...
if not exist "mcp-servers\memory-mcp-server\build" (
    echo [WARNING] Memory server not built
    set /a ERRORS+=1
)

if not exist "mcp-servers\git-mcp-server\build" (
    echo [WARNING] Git server not built
    set /a ERRORS+=1
)

if not exist "mcp-servers\design-tools-mcp-server\build" (
    echo [WARNING] Design tools server not built
    set /a ERRORS+=1
)
