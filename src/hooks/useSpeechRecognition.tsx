 ...
 *
 * MIGRATION NOTE: Logic aligned with master-update (getUserMedia before start, error handling).
 * No UI impact; API unchanged for VoiceChatModal/InputArea.
 */
export const useSpeechRecognition = (
  ...
  const initializeRecognition = useCallback(() => {
    ...
    if (!SpeechRecognitionAPI) {
      const errorMsg =
        "Browser does not support the Web Speech API. Please use Chrome or Edge.";
      setError(errorMsg);
      onError?.(errorMsg);
      return null;
    }
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      const errorMsg =
        permissionError?.message || "Microphone permission was denied";
      setError(errorMsg);
      onError?.(errorMsg);
      return;
    }
