...
export const ImpactHeatmap: React.FC<ImpactHeatmapProps> = ({
  ...
  useEffect(() => {
    ...
    moduleList.forEach((sourceModule: any, i: any) => {
      moduleList.forEach((targetModule: any, j: any) => {
        ...
        g.append('rect')
          ...
          .on('mouseover', function() {
            ...
            d3.select(event?.currentTarget || event?.target).attr('stroke-width', 2).attr('stroke', '#3b82f6');
          })
          .on('mouseout', function() {
            setHoveredCell(null);
            d3.select(event?.currentTarget || event?.target).attr('stroke-width', 0.5).attr('stroke', '#fff');
          });
    ...
    g.append('g')
      ...
      .attr('text-anchor', 'middle')
      .attr('font-size', '10px')
      .attr('transform', (d: any, i: number) => `rotate(-45, ${i * cellSize + cellSize / 2}, -5)`);
