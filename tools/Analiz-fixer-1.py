    ...
    def show_menu(self):
        ...
        while True:
            ...
            menu_items = [
                ...
                ("2", "🗑️", "Unused Components", "Find unused React components"),
                ("3", "🔌", "Unwired Components", "Components not imported anywhere"),
                ("4", "📝", "Duplicate Components", "Multiple component definitions"),
                ("5", "🪝", "Hook Analysis", "Custom hooks usage analysis"),
                ("6", "📦", "Orphan Files", "Files with no dependents"),
                ("7", "🔄", "Duplicate Files", "Identical file content"),
                ("8", "📊", "Project Statistics", "Metrics and complexity"),
                ("9", "🎯", "Refactoring Plan", "Step-by-step roadmap"),
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .badge {{ display: inline-block; padding: 4px 10px; border-radius: 4px; font-size: 11px; margin-left: 10px; }}
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            {self._render_items_html(report['unused_components'], 'unused')}
        </div>
        
        <div class="section">
            <h2>🔌 Unwired Components</h2>
            {self._render_items_html(report['unwired_components'], 'unwired')}
        </div>
        
        <div class="section">
            <h2>📝 Duplicate Components</h2>
            {self._render_items_html(report['duplicate_components'], 'duplicate')}
        </div>
    </div>
</body>
</html>'''
    ...
    def _render_items_html(self, items: List[Dict], item_type: str) -> str:
        """Render items as HTML"""
        if not items:
            return '<p style="color: #64748b;">No items found</p>'
        
        html = ''
        for item in items[:30]:
            if item_type == 'unused':
                badge = f'<span class="badge badge-danger">{item["severity"]}</span>'
                html += f'<div class="item">{item["name"]} - {item["file"]}{badge}</div>'
            elif item_type == 'unwired':
                html += f'<div class="item">{item["component"]} - {item["file"]}<br><small>{item["reason"]}</small></div>'
            elif item_type == 'duplicate':
                html += f'<div class="item">{item["name"]} - {item["count"]} copies</div>'
