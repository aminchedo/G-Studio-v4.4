...
const DetailedIcons = {
  ...
  Wifi: ({
    ...
    <svg viewBox="0 0 24 24" fill="none" className={className} style={style}>
      <path
        d="M5 12.55a11 11 0 0 1 5.17-2.39"
        stroke="currentColor"
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
...
export const InputArea: FC<InputAreaProps> = ({
  ...
  // Ref so speech onResult callback always sees latest handleSend/setInput (handleSend is defined later)
  const speechResultRef = useRef<{
    handleSend: (text: string, image?: string) => void;
    setInput: React.Dispatch<React.SetStateAction<string>>;
    voiceCommandMode: boolean;
  }>({ handleSend: () => {}, setInput: () => {}, voiceCommandMode: false });

  // Use simple Web Speech API hook (lang, continuous, interimResults, onResult, onError).
  // Hook is source of truth for listening state; we sync to parent via onListeningChange.
  const speechState = useSpeechRecognition({
    ...
    onResult: (text, isFinal) => {
      ...
      const {
        handleSend: send,
        setInput: setIn,
        voiceCommandMode: vcm,
      } = speechResultRef.current;
      if (vcm) {
        send(text);
      } else {
        setIn((prev) => (prev + (prev ? " " : "") + text).trim());
      }
    },
    onError: (err) => console.error("[InputArea] Speech error:", err),
  });

  // Sync hook listening state to parent for external UI
  useEffect(() => {
    onListeningChange(speechState.isListening);
  }, [speechState.isListening, onListeningChange]);

  const error = speechState.error ?? null;
  const setError = () => {}; // simple hook does not expose setError
  const interimTranscript = speechState.transcript ?? "";
  const hasSpeechText = !!(speechState.transcript ?? "").trim();
  const clearSpeechState = speechState.reset;

  // Backend health check: only run when a dedicated health URL is configured.
  // Port 7242 is the debug log ingest server and does not expose /health; skip to avoid 404.
  useEffect(() => {
    const healthUrl =
      typeof import.meta.env?.VITE_BACKEND_HEALTH_URL === "string" &&
      import.meta.env.VITE_BACKEND_HEALTH_URL.trim() !== ""
        ? import.meta.env.VITE_BACKEND_HEALTH_URL.trim()
        : null;

    if (!healthUrl) {
      setBackendAvailable(true); // No backend configured; don't block chat
      return;
    }
  ...
  const handleSend = useCallback(
    async (textOverride?: string) => {
      ...
      try {
        ...
        const { ModelValidationStore } =
          await import("@/services/modelValidationStore");
        ...
        console.warn(
          "[InputArea] Failed to check provider availability:",
          error,
        );
      }
  ...
  useEffect(() => {
    speechResultRef.current = { handleSend, setInput, voiceCommandMode };
  }, [handleSend, setInput, voiceCommandMode]);

  // Speech recognition: hook is source of truth; mic button starts/stops the recognizer
  ...
  const listening = speechState.isListening;
    ...
    <div className="px-0 py-0 bg-transparent relative z-20">
      ...
      {detectedLanguage && (input ?? "").trim() && (
        <div className="absolute -top-8 left-6 text-[10px] text-slate-400 bg-slate-800/80 px-2 py-1 rounded-lg border border-slate-700">
          Language: {detectedLanguage.name} ({detectedLanguage.confidence}%
          confidence)
        </div>
      )}
        ...
        <div className="absolute -top-24 left-6 animate-fade-in z-10">
          <div className="relative group">
            <img
              src={selectedImage}
              alt="preview"
              className="h-20 w-auto rounded-xl border border-slate-200 shadow-lg"
            />
            <button
      ...
      <div
        className={combinePatterns(
          ...
          "flex items-center gap-1.5 border-0 rounded-lg transition-all focus-within:ring-2 focus-within:ring-purple-500/20 focus-within:border-purple-400/30 relative overflow-hidden min-h-[38px] bg-slate-900/90 border border-slate-800/50 px-2 py-1.5",
        )}
      >
        ...
        {backendAvailable === false && (
          <div
            ...
            title="Backend ingest service unavailable - chat will continue normally"
          >
            <DetailedIcons.WifiOff
        ...
        <div className="flex gap-1 relative z-10">
          <button
            ...
            className="group/attach relative p-1 text-orange-500 hover:text-orange-400 bg-transparent hover:bg-orange-500/15 active:scale-95 rounded-md transition-all duration-200"
            title="Attach File"
          >
            <div className="absolute inset-0 bg-gradient-to-br from-orange-500/10 via-transparent to-orange-400/10 opacity-0 group-hover/attach:opacity-100 transition-opacity duration-200 rounded-md" />
            <DetailedIcons.Paperclip
              className="w-5 h-5 text-orange-500 relative z-10 transition-transform duration-200 group-hover/attach:scale-105"
              style={{
                shapeRendering: "geometricPrecision",
                textRendering: "optimizeLegibility",
                WebkitFontSmoothing: "antialiased",
          ...
          <button
            onClick={(e) => {
              ...
              if (listening) {
                ...
                if (interimTranscript && !voiceCommandMode) {
                  setInput((prev) => {
                    const it = interimTranscript ?? "";
                    const cleanPrev = (prev ?? "").replace(it, "").trim();
                    return cleanPrev ? cleanPrev + " " + it.trim() : it.trim();
                  });
                }
                clearSpeechState();
                ...
                console.log(
                  "[InputArea] Click - starting listening (real-time typing)",
                );
                onListeningChange(true);
              }
            }}
            // Allow voice capture even if chat is disabled (e.g. missing API key);
            // sending will be handled by the parent and can show a proper error.
            disabled={isLoading}
            className={`group/mic relative p-1 rounded-md transition-all duration-200 border ${
              listening
            ...
            title={
              listening
                ? voiceCommandMode
                  ? "Listening... (Voice Command Mode - Auto-send)"
                  : "Listening... (Text will be added to input)"
                : isLoading
                  ? "Please wait..."
          ...
          >
            ...
            {listening ? (
              <DetailedIcons.MicOff
                className="w-5 h-5 relative z-10 transition-transform duration-200 group-hover/mic:scale-105"
                style={{
                  shapeRendering: "geometricPrecision",
                  textRendering: "optimizeLegibility",
                  WebkitFontSmoothing: "antialiased",
        ...
        <textarea
          ...
          placeholder={
            listening
              ? voiceCommandMode
                ? "Listening... (Voice command mode - auto-send)"
                : "Listening... (Text will be added to input)"
              : (placeholder ?? "Ask Gemini to analyze code or run commands...")
          }
          rows={1}
          ...
          className={combinePatterns(
            ...
            "flex-1 bg-transparent resize-none py-1.5 px-1.5 text-[12px] max-h-28 leading-relaxed relative z-10 placeholder:text-slate-500 placeholder:opacity-80 no-scrollbar",
            (listening && interimTranscript && !voiceCommandMode) ||
              hasSpeechText
              ? "text-amber-400 font-bold" // Golden glowing text for speech-to-text
              : "text-slate-100", // Normal text - matches message body
          )}
          style={{
            fontFamily:
              (listening && interimTranscript && !voiceCommandMode) ||
        ...
        <button
          ...
          className={`group/send relative p-1 rounded-md transition-all duration-200 active:scale-95 ${
            isLoading
        ...
        >
          ...
          {!isLoading &&
            ((input ?? "").trim() ||
              (interimTranscript ?? "").trim() ||
              selectedImage) && (
              <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 via-transparent to-blue-400/10 opacity-0 group-hover/send:opacity-100 transition-opacity duration-200 rounded-md" />
            )}
          {isLoading ? (
            <Loader2
              strokeWidth={2}
              strokeLinecap="round"
              strokeLinejoin="round"
              className="w-5 h-5 text-blue-500 animate-spin relative z-10"
              style={{
                shapeRendering: "geometricPrecision",
                textRendering: "optimizeLegibility",
                WebkitFontSmoothing: "antialiased",
            ...
            <DetailedIcons.Send
              className="w-5 h-5 text-blue-500 relative z-10 transition-transform duration-200 group-hover/send:scale-105 group-hover/send:translate-x-0.5"
              style={{
                shapeRendering: "geometricPrecision",
                textRendering: "optimizeLegibility",
                WebkitFontSmoothing: "antialiased",
