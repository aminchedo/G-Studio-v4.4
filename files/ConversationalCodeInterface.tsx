    ...
    this.registerTool({
      ...
      inputSchema: {
        ...
        properties: {
          ...
          scheme: { 
            type: 'string', 
            enum: ['complementary', 'analogous', 'triadic', 'tetradic', 'monochromatic', 'split-complementary']
          },
          count: { type: 'number' }
        },
...
Available MCP Tools:
${this.mcpTools.getTools().map(t => `- ${t.name}: ${t.description}`).join('\n')}

Guidelines:
1. Always provide complete, runnable code
2. Include comments for complex logic
3. Follow language-specific best practices
4. Use MCP tools when appropriate (storing context, git operations, etc.)
5. Format code blocks with language tags
...
export const ConversationalCodeInterface: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await geminiAPIRef.current.chat([...messages, userMessage]);
      ...
      if (response.functionCalls && response.functionCalls.length > 0) {
        ...
        if (toolCalls.length > 0) {
          ...
          const toolResultsMessage = `Tool execution results:\n${toolCalls.map(tc => 
            `${tc.tool}: ${JSON.stringify(tc.result)}`
          ).join('\n')}`;
      ...
      <div className="flex items-center justify-center h-full bg-gradient-to-br from-gray-900 to-gray-800 p-6">
        <div className="bg-gray-800 rounded-lg p-8 max-w-md w-full border border-gray-700 shadow-2xl">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-bold text-white">Configure Gemini API</h2>
          </div>
          
          <p className="text-gray-300 mb-6">
            Enter your Google Gemini API key to enable conversational code writing with full MCP tools access.
          </p>
          ...
          <div className="mt-6 p-4 bg-gray-900 rounded-lg border border-gray-700">
            <h3 className="text-sm font-medium text-gray-300 mb-2">Available MCP Tools:</h3>
    ...
    <div className="flex flex-col h-full bg-gray-900">
      ...
      <div className="bg-gray-800 border-b border-gray-700 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-white">Conversational Code Writer</h1>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">Ready to Write Code</h2>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 max-w-2xl">
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700 text-left">
                <p className="text-sm text-gray-300">"Create a React component for user authentication"</p>
              </div>
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700 text-left">
                <p className="text-sm text-gray-300">"Write a Python API client with error handling"</p>
              </div>
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700 text-left">
                <p className="text-sm text-gray-300">"Generate a color palette for my app using #3498db"</p>
              </div>
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700 text-left">
                <p className="text-sm text-gray-300">"Show me the git status and create a commit"</p>
              </div>
            </div>
          </div>
        )}
                  ...
                  {message.toolCalls.map((tc, idx) => (
                    <div key={idx} className="bg-gray-900 p-3 rounded border border-gray-600">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-purple-300">{tc.tool}</span>
                        <span className={`text-xs px-2 py-0.5 rounded ${
                          tc.status === 'success' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'
                        }`}>
                          {tc.status}
                        </span>
                      </div>
              ...
              {message.codeBlocks && message.codeBlocks.length > 0 && (
                <div className="mt-3 space-y-3">
                  {message.codeBlocks.map((block, idx) => (
                    <div key={idx} className="bg-gray-950 rounded-lg overflow-hidden border border-gray-700">
                      <div className="flex items-center justify-between bg-gray-900 px-4 py-2 border-b border-gray-700">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => applyCode(block.code, block.language)}
      ...
      <div className="bg-gray-800 border-t border-gray-700 px-6 py-4">
        ...
        <div className="mt-2 text-xs text-gray-400">
          Press Enter to send • Shift+Enter for new line • Full MCP tools access enabled
        </div>
      </div>
    </div>
  );
