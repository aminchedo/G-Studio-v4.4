...
import { ProductivityMetrics, MetricAggregation } from './productivityMetrics';
import { ContextDatabaseBridge } from './contextDatabaseBridge';
import { TaskDecompositionEngine, DecomposedStep } from './taskDecompositionEngine';
...
export class PlanningFeedbackService {
   ...
   */
  static async analyzeAndGenerateFeedback(taskType: string): Promise<PlanningFeedback[]> {
    ...
    if (!efficiencyMetrics || efficiencyMetrics.count < this.MIN_SAMPLES_FOR_FEEDBACK) {
      return feedbacks; // Not enough data
    }

    // Analyze step granularity
    const granularityFeedback = await this.analyzeStepGranularity(taskType, efficiencyMetrics);
    ...
    for (const feedback of feedbacks) {
      ...
      console.log(`[PLANNING_FEEDBACK]: APPLIED (${feedback.adjustmentType} for ${taskType})`);
    }
  ...
  private static async analyzeStepGranularity(
    ...
    if (efficiencyMetrics.average > 1.3) {
      return {
        ...
        taskType,
        adjustmentType: 'STEP_GRANULARITY',
   ...
   * Analyze risk estimation
   */
  private static async analyzeRiskEstimation(taskType: string): Promise<PlanningFeedback | null> {
    ...
    if (retryMetrics.average > 2) {
      return {
        ...
        taskType,
        adjustmentType: 'RISK_ESTIMATION',
   ...
   * Analyze time estimation
   */
  private static async analyzeTimeEstimation(taskType: string): Promise<PlanningFeedback | null> {
    ...
    return {
      ...
      taskType,
      adjustmentType: 'TIME_ESTIMATION',
      adjustmentValue: { 
   ...
   * Analyze model selection
   */
  private static async analyzeModelSelection(taskType: string): Promise<PlanningFeedback | null> {
    ...
    if (roiMetrics.average < 0.5) {
      return {
        ...
        taskType,
        adjustmentType: 'MODEL_SELECTION_HINT',
        adjustmentValue: { 
   ...
   */
  static async getFeedbackForTaskType(taskType: string): Promise<PlanningFeedback[]> {
    return await ContextDatabaseBridge.getPlanningFeedback(taskType, 20);
  }
          ...
          for (let i = 0; i < adjustedSteps.length; i += 2) {
            if (i + 1 < adjustedSteps.length) {
              merged.push({
                ...
                risk: adjustedSteps[i].risk === 'risky' || adjustedSteps[i + 1].risk === 'risky' 
                  ? 'risky' 
                  : adjustedSteps[i].risk === 'moderate' || adjustedSteps[i + 1].risk === 'moderate'
                  ? 'moderate'
                  : 'safe',
                estimatedTime: adjustedSteps[i].estimatedTime + adjustedSteps[i + 1].estimatedTime,
                dependencies: [...(adjustedSteps[i].dependencies || []), ...(adjustedSteps[i + 1].dependencies || [])],
              });
            } else {
