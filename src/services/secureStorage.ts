...
export class SecureStorage {
  ...
  private static async clearOldData(): Promise<void> {
    ...
    // If still over 4MB after cleanup, try to compress essential keys
    ...
    if (remainingMB > 4) {
      console.warn(`[SecureStorage] Still over quota (${remainingMB.toFixed(2)}MB). Attempting to compress essential keys...`);
      
      // Try to compress secure_gstudio_agent_config if it exists and is large
      ...
      if (agentConfig) {
        ...
        if (agentConfigSize > 1024 * 1024) { // If over 1MB
          try {
            const parsed = JSON.parse(agentConfig);
            // Remove old conversation history or large arrays
            if (parsed && typeof parsed === 'object') {
              ...
              // Remove large arrays like conversations, history, etc.
              const compressedStr = JSON.stringify(compressed);
              if (new Blob([compressedStr]).size < agentConfigSize) {
                localStorage.setItem(agentConfigKey, compressedStr);
                console.log(`[SecureStorage] Compressed ${agentConfigKey} from ${(agentConfigSize / 1024).toFixed(2)}KB to ${(new Blob([compressedStr]).size / 1024).toFixed(2)}KB`);
              }
            }
          } catch (e) {
  ...
  static async setItem(key: string, value: any): Promise<void> {
    ...
    try {
      ...
      // If data is very large (>2MB), try to compress it
      if (sizeInMB > 2 && typeof value === 'object' && value !== null) {
        ...
        if (key === 'gstudio_agent_config') {
          ...
          if (value.conversations && Array.isArray(value.conversations)) {
            compressed.conversations = value.conversations.slice(-10); // Keep only last 10
          }
          if (value.history && Array.isArray(value.history)) {
            compressed.history = value.history.slice(-50); // Keep only last 50
          }
          serialized = JSON.stringify(compressed);
          console.log(`[SecureStorage] Compressed ${key} before saving`);
        }
      }
      ...
      // If storage is over 70% full (3.5MB of typical 5MB), clean up proactively
      if (currentMB > 3.5) {
