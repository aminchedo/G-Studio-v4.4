...
import type { UsageMetadata } from "@/services/geminiService/types";
import { RequestCoalescer } from "../network/requestCoalescer";
import { DegradedMode } from "../network/degradedMode";

interface QueuedRequest {
  resolve: (value: any) => void;
  reject: (error: any) => void;
  request: () => AsyncGenerator<
    { text?: string; toolCalls?: ToolCall[]; usage?: UsageMetadata },
    void,
    unknown
  >;
}
...
export class GeminiServiceOptimized {
  ...
  private static readonly RATE_LIMIT_RPM: number = parseInt(
      ...
      if (queuedRequest) {
        ...
        try {
          ...
          if (isFatalError(error)) {
            console.error(
              "[GeminiServiceOptimized][processQueue] FATAL_ERROR_DETECTED - terminating pipeline",
  ...
  > {
    ...
    try {
      ...
      if (isFatalError(error)) {
        console.error(
          "[GeminiServiceOptimized][queueRequest] FATAL_ERROR_DETECTED - terminating pipeline",
          (error as any).message,
        );
  ...
  > {
    ...
    if (!requestId) {
      throw new Error(
        "CODE_BUG: requestId is required in generateContentStream(). It must be generated in UI layer and passed explicitly.",
      );
    }
    ...
    try {
      ...
      if (isFatalError(error)) {
        console.error(
          "[GeminiServiceOptimized] FATAL_ERROR_DETECTED - terminating pipeline",
          (error as any).message,
        );
        throw error; // ⛔️ Stop everything - no fallback, no retry
      }
      ...
      console.warn(
        "LLM Gateway optimization failed, using base service:",
        error,
      );
      const lastMessage = this.extractUserMessage(messages);
      yield* this.baseService.streamChat(
  ...
  > {
    ...
    if (!requestId) {
      throw new Error(
        "CODE_BUG: requestId is required in GeminiServiceOptimized.streamChat(). It must be generated in UI layer (App.tsx) and passed explicitly. No service may generate requestId.",
      );
    }
  ...
  getTokenUsage(userId?: string): { prompt: number; response: number } {
    ...
    try {
      ...
      return {
        prompt: stats.metrics["totalPromptTokens"]?.sum || 0,
        response: stats.metrics["totalCompletionTokens"]?.sum || 0,
      };
    } catch (error) {
      return { prompt: 0, response: 0 };
    }
