...
import {
  categorizeError,
  getObservabilityError,
} from "@/services/gemini/errorHandler";
...
export async function runModelDiagnosticTest(
  ...
  try {
    ...
    // 2. Minimal valid request payload (NEW SDK: model, contents, config)
    ...
    // 3. Execute model call (uses @google/genai NEW SDK)
...
export class GeminiService {
  ...
  async generateContent(prompt: string): Promise<string> {
    ...
    const doFetch = async () => {
      ...
      if (data?.usageMetadata) {
        ...
        this.usageInstance.total =
          um.totalTokenCount ||
          this.usageInstance.prompt + this.usageInstance.response;
      }
      return text;
    };
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (obj && typeof obj.text === "string") {
              ...
              const t = obj.candidates[0].content.parts
                .map((p: any) => p.text)
                .join("");
              onChunk(t);
            }
  ...
  getTokenUsage() {
    return {
      total: this.usageInstance.total,
      prompt: this.usageInstance.prompt,
      response: this.usageInstance.response,
    };
  }
  ...
  private static formatHistory(
    ...
    const result = validMessages
      .map((msg) => {
        ...
        if (msg.image) {
          const base64Data =
            typeof (msg as any).image === "string"
              ? ((msg as any).image as string).split(",")[1] ||
                (msg as any).image
              : (msg as any).image;
          parts.push({
            inlineData: {
  ...
  private static modelIdToApiName(modelId: ModelId): string {
    ...
    return _modelMap[modelId] || modelId.toString();
  }
  ...
  > {
    ...
    const { StreamLifecycleManager } =
      await import("../streamLifecycleManager");
    ...
    // #region agent log
    safeFetch(`${API_BASE}/ingest/e18d953a-ffb4-4ced-9fd1-6cbd5a7207b7`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        location: "geminiService.ts:streamChat",
        message: "streamChat usableModels check",
        data: {
          requestId: finalRequestId,
          usableModelsCount: usableModels.length,
          usableModels: usableModels.slice(0, 5),
    ...
    if (usableModels.length === 0) {
      // #region agent log
      safeFetch(
        `${API_BASE}/ingest/e18d953a-ffb4-4ced-9fd1-6cbd5a7207b7`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            location: "geminiService.ts:streamChat",
            message: "throwing ZERO_USABLE_MODELS",
            data: {
              requestId: finalRequestId,
              providerStatus: ModelValidationStore.getProviderStatus(apiKey),
              hasTestBeenExecuted:
                ModelValidationStore.hasTestBeenExecuted(apiKey),
            },
            timestamp: Date.now(),
            hypothesisId: "H1",
          }),
    ...
    try {
      ...
      while (attemptNumber <= maxAttempts) {
        ...
        try {
          ...
          if (!preflight.isValid) {
            ...
            try {
              const { CompletionReporter } =
                await import("../monitoring/completionReporter");
          ...
          // Streaming mode (uses @google/genai NEW SDK)
          ...
          try {
            // TRACE: NEW SDK client has ai.models; this call would throw if using OLD SDK
            console.log(
              "[Trace] GeminiService.streamChat → ai.models.generateContentStream",
              {
                requestId: finalRequestId,
                model: currentModel,
                hasModels: !!ai.models,
          ...
          if (error) {
            ...
            logObservabilityError(
              getObservabilityError(errorInfo, {
                requestId: finalRequestId,
                model: currentModel,
                attempt: attemptNumber,
              }),
      ...
      if (error.message && !error.message.includes("Your API key is valid")) {
        ...
        if (
          ...
          throw new Error(
            "Function call error: thought signature is required.\n\nThis error occurs with Gemini 3.0 models when using tools. Please use a different model or contact support.",
          );
        }
      }
  ...
  static async chatNonStreaming(
    ...
    try {
      ...
      // REST API expects top-level fields: contents, generationConfig, systemInstruction, tools (no "config" wrapper)
      ...
      const cAny = c as any;
      const responseText =
        cAny?.candidates?.[0]?.content?.parts
          ?.map((p: any) => p.text)
          ?.filter((t: any) => t)
          ?.join("") || "";

      if (!responseText && !cAny?.functionCalls?.length) {
        console.error(
          `[GeminiService][requestId=${finalRequestId}]: Empty model response`,
          {
            hasCandidates: !!cAny?.candidates,
            candidatesLength: cAny?.candidates?.length || 0,
            hasContent: !!cAny?.candidates?.[0]?.content,
            hasParts: !!cAny?.candidates?.[0]?.content?.parts,
            partsLength: cAny?.candidates?.[0]?.content?.parts?.length || 0,
          },
        );
        throw new Error(
      ...
      const text = cAny.text || "";
      const toolCalls: ToolCall[] = [];

      if (cAny.functionCalls) {
        if (!Array.isArray(cAny.functionCalls)) {
          console.error(
            `[GeminiService][requestId=${finalRequestId}]: Invalid functionCalls type`,
            { functionCallsType: typeof cAny.functionCalls },
          );
          throw new Error("Invalid response format. Please try again.");
        }
        for (const fc of cAny.functionCalls) {
          if (!fc || typeof fc !== "object" || !fc.name) {
            console.error(
              `[GeminiService][requestId=${finalRequestId}]: Invalid function call`,
              { fc },
      ...
      if (AIBehaviorValidation.is400InvalidArgument(error)) {
        ...
        safeFetch(
          `${API_BASE}/ingest/e18d953a-ffb4-4ced-9fd1-6cbd5a7207b7`,
          {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              location: "geminiService.ts:chatNonStreaming",
              message: "400 in non-streaming path",
              data: {
                requestId: finalRequestId,
                modelId,
                errorMessage: (error && error.message) || "",
      ...
      try {
        ...
        console.error("Gemini API Error (non-streaming):", {
          ...
          stack: error?.stack,
          status:
            error?.status ?? AIBehaviorValidation.getHttpStatusCode(error),
