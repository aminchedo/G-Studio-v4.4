/**
 * Modern API Keys Settings with Model Discovery
 * Enhanced version with automatic model detection and user feedback
 */

import React, { useState } from 'react';
import { useSettingsStore } from '../settingsStore';
import { useSettingsIntegration } from '@/services/settingsIntegration';
import { 
  SettingGroup, 
  SettingRow, 
  SecretInput,
...
import { ModelDiscoveryProgressModal } from '@/components/modals/ModelDiscoveryProgress';

const APIKeysSettingsEnhanced: React.FC = () => {
  ...
  const handleAPIKeyChange = (provider: keyof typeof apiKeys, value: string) => {
    if (typeof apiKeys[provider] === 'string') {
      ...
      // Auto-discover models for Google API key
      if (provider === 'google' && value && value.length > 20) {
  ...
  const providers = [
    {
      ...
      description: 'Gemini 2.5, 2.0, Flash, Pro, and other Google models',
      icon: (
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm leading-relaxed text-amber-800 dark:text-amber-200">
              API keys are stored securely in your browser's encrypted local storage. They never leave your device
              and are not sent to any third-party servers. Always keep your API keys confidential.
            </p>
          </div>
        </div>
      </div>

      {/* Model Discovery Status (Google) */}
      {apiKeys.google && discoveredModels.length > 0 && (
        <div className="rounded-2xl border border-green-200/60 bg-gradient-to-br from-green-50 to-green-100/50 p-6 dark:border-green-900/60 dark:from-green-900/20 dark:to-green-800/20">
          <div className="flex items-start justify-between gap-4">
            <div className="flex gap-4">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-green-500 text-white shadow-lg">
                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-green-800 dark:text-green-200">
                  {discoveredModels.length} Google AI models available for use.
                  {activeModel && (
                    <span className="ml-1">
                      Currently using: <span className="font-semibold">{activeModel.label}</span>
                    </span>
                  )}
                </p>
              </div>
            ...
            <button
              ...
              className="rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow-md transition-all hover:bg-green-700 hover:shadow-lg disabled:opacity-50"
            >
      ...
      {providers.map((provider) => (
        ...
        >
          <SettingRow
            ...
            badge={provider.hasModelDiscovery ? 'Auto-Discover' : undefined}
          >
            <SecretInput
...
export default APIKeysSettingsEnhanced;
