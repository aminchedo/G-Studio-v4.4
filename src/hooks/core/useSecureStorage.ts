...
export const secureStorageOperations = {
  setItem: async <T>(key: string, value: T, options: SecureStorageOptions = {}): Promise<void> => {
    ...
    try {
      ...
      if (error.name === 'QuotaExceededError' || error.code === 22) {
        console.warn('[SecureStorage] Storage quota exceeded, attempting cleanup...');
        await secureStorageOperations.cleanup(options);
        
        // Retry once
        const serialized = JSON.stringify(value);
        const data = finalOptions.forceEncryption ? encrypt(serialized, getEncryptionKey()) : serialized;
  ...
  getItem: async <T>(key: string, options: SecureStorageOptions = {}): Promise<T | null> => {
    const finalOptions = { ...DEFAULT_OPTIONS, ...options };
    const prefixedKey = `${finalOptions.keyPrefix}${key}`;
  ...
  removeItem: async (key: string, options: SecureStorageOptions = {}): Promise<void> => {
    const finalOptions = { ...DEFAULT_OPTIONS, ...options };
    localStorage.removeItem(`${finalOptions.keyPrefix}${key}`);
  },
  
  hasItem: async (key: string, options: SecureStorageOptions = {}): Promise<boolean> => {
      ...
      for (const key of keysToMigrate) {
        ...
        if (value) {
          try {
            ...
            await secureStorageOperations.setItem(key, parsed, options);
          } catch {
            // Skip invalid data
          }
        }
