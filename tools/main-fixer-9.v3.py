...
class ReportGenerator:
    ...
    def generate_dependency_graph_json(self):
        ...
        output_path = self.report_folder / 'dependency_graph.json'
        ...
        for path, file_data in self.report.files.items():
            node_id = file_data['relative_path']
            node_ids.add(node_id)
            
            # Determine node color based on status
            if path in self.report.unused_candidates:
                color = '#ef4444'  # red - unused
                status = 'unused'
            elif path in self.report.unwired_candidates:
                color = '#f59e0b'  # orange - unwired
                status = 'unwired'
            elif file_data['risk_level'] == 'CRITICAL':
                color = '#dc2626'  # dark red - critical
                status = 'critical'
            elif file_data['risk_level'] == 'HIGH':
                color = '#f97316'  # orange - high risk
                status = 'high_risk'
            elif file_data['dependents_count'] > 0:
                color = '#10b981'  # green - used
                status = 'used'
            else:
            ...
            size = 10 + min(file_data['dependents_count'] * 2, 40)
            
            nodes.append({
                'id': node_id,
                'label': file_data['relative_path'].split('/')[-1],
                'title': f"{file_data['relative_path']}\nDependents: {file_data['dependents_count']}\nRisk: {file_data['risk_level']}\nStatus: {status}",
                'color': color,
                'size': size,
                'status': status,
        ...
        for path, file_data in self.report.files.items():
            source_id = file_data['relative_path']
            for dependent in file_data['dependents']:
                if dependent in self.report.files:
                    target_id = self.report.files[dependent]['relative_path']
        ...
        self.logger.info(f"Dependency graph JSON: {output_path.name}")
...
"""
    ...
    def _build_comprehensive_html(self) -> str:
        ...
        graph_file = self.report_folder / 'dependency_graph.json'
        graph_data = {}
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Enterprise Code Intelligence Dashboard v3 — {self.report.metadata['start_time']}</title>
...
<body>
    <div class="dashboard">
        {self._build_header()}
        {self._build_agent_action_bar()}
        {self._build_safety_banner()}
        {self._build_executive_summary()}
        {self._build_dependency_graph_section()}
...
"""
    ...
    def _build_header(self) -> str:
        ...
        return f"""
<div class="header">
    <h1>🔬 Enterprise Code Intelligence Dashboard v3</h1>
    <div class="subtitle">Professional Static Analysis & Refactoring Intelligence Platform v{Config.TOOL_VERSION}</div>
    <div class="subtitle" style="color: #10b981; margin-top: 8px;">
        ✨ Enhanced: Interactive Dependency Graph • Agent-Friendly Actions • Real-time Analysis
    </div>
    <div class="meta">
        <div class="meta-item">
...
"""
    ...
    def _build_agent_action_bar(self) -> str:
        """Build agent-friendly action bar with quick actions"""
        return f"""
<div class="agent-action-bar" style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 12px; padding: 20px; margin: 24px 0;">
    <h3 style="margin: 0 0 16px 0; color: #10b981;">🤖 Agent Quick Actions</h3>
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px;">
        <button class="agent-btn" onclick="copyUnusedFiles()" style="padding: 12px; background: #ef4444; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
            📋 Copy {len(self.report.unused_candidates)} Unused Files
        </button>
        <button class="agent-btn" onclick="copyDuplicates()" style="padding: 12px; background: #f59e0b; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
            📋 Copy {len(self.report.duplicate_clusters)} Duplicate Clusters
        </button>
        <button class="agent-btn" onclick="copyHighRisk()" style="padding: 12px; background: #dc2626; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
            📋 Copy High Risk Files
        </button>
        <button class="agent-btn" onclick="exportActionableJSON()" style="padding: 12px; background: #6366f1; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
            💾 Export Actionable JSON
        </button>
        <button class="agent-btn" onclick="showQuickStats()" style="padding: 12px; background: #3b82f6; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
            📊 Show Quick Stats
        </button>
        <button class="agent-btn" onclick="focusGraph()" style="padding: 12px; background: #8b5cf6; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
            🗺️ Focus Dependency Graph
        </button>
    </div>
</div>
"""
...
"""
    ...
    def _build_dependency_graph_section(self) -> str:
        """Build interactive dependency graph section"""
        return """
<div class="section">
    ...
    <div class="section-content" style="max-height: 5000px;">
        <div class="section-description">
            Visual representation of file dependencies. Color-coded by status:
            <span style="color: #ef4444;">● Unused</span> |
            <span style="color: #f59e0b;">● Unwired</span> |
            <span style="color: #dc2626;">● Critical Risk</span> |
            <span style="color: #10b981;">● Used/Healthy</span> |
            <span style="color: #94a3b8;">● Neutral</span>
        </div>
        <div class="graph-controls" style="margin: 16px 0; display: flex; gap: 12px; flex-wrap: wrap;">
            <button onclick="filterGraphByStatus('unused')" style="padding: 8px 16px; background: #ef4444; color: white; border: none; border-radius: 6px; cursor: pointer;">
                Show Unused Only
            </button>
            <button onclick="filterGraphByStatus('unwired')" style="padding: 8px 16px; background: #f59e0b; color: white; border: none; border-radius: 6px; cursor: pointer;">
                Show Unwired Only
            </button>
            <button onclick="filterGraphByStatus('critical')" style="padding: 8px 16px; background: #dc2626; color: white; border: none; border-radius: 6px; cursor: pointer;">
                Show Critical Only
            </button>
            <button onclick="resetGraph()" style="padding: 8px 16px; background: #6366f1; color: white; border: none; border-radius: 6px; cursor: pointer;">
                Reset View
            </button>
            <button onclick="zoomIn()" style="padding: 8px 16px; background: #3b82f6; color: white; border: none; border-radius: 6px; cursor: pointer;">
                🔍 Zoom In
            </button>
            <button onclick="zoomOut()" style="padding: 8px 16px; background: #3b82f6; color: white; border: none; border-radius: 6px; cursor: pointer;">
                🔍 Zoom Out
            </button>
        </div>
        <div id="dependencyGraph" style="width: 100%; height: 600px; border: 1px solid var(--border); border-radius: 12px; background: var(--bg-secondary);"></div>
        <div id="graphInfo" style="margin-top: 16px; padding: 16px; background: rgba(99, 102, 241, 0.1); border-radius: 8px; display: none;">
            <h4 style="margin: 0 0 12px 0;">Selected Node Info:</h4>
...
function initDependencyGraph() {
    ...
    console.log('✓ Dependency graph initialized:', graphData.metadata);
}
...
function showNodeDetails(node) {
    ...
    detailsDiv.innerHTML = `
        <div style="display: grid; gap: 12px;">
            <div><strong>File:</strong> <code style="background: rgba(99,102,241,0.2); padding: 4px 8px; border-radius: 4px;">${node.id}</code></div>
            <div><strong>Status:</strong> <span style="color: ${node.color}; font-weight: 600;">${node.status.toUpperCase()}</span></div>
            <div><strong>Category:</strong> ${node.category}</div>
            <div><strong>Risk Level:</strong> ${node.risk}</div>
            <div><strong>Dependents:</strong> ${node.dependents} file(s)</div>
            <div><strong>Lines:</strong> ${node.lines}</div>
            <button onclick="copyFilePath('${node.id}')" style="margin-top: 8px; padding: 8px 16px; background: #6366f1; color: white; border: none; border-radius: 6px; cursor: pointer;">
                📋 Copy File Path
            </button>
        </div>
    `;
...
function highlightConnectedNodes(nodeId) {
    ...
    const updates = allNodeIds.map(id => {
        const opacity = id === nodeId || connectedNodes.includes(id) ? 1.0 : 0.3;
        return { id: id, opacity: opacity };
    });
    
    network.body.data.nodes.update(updates);
}

function filterGraphByStatus(status) {
    if (!network) return;
    
    const filteredNodes = allNodes.filter(n => n.status === status);
    ...
    console.log(`Filtered to ${status}:`, filteredNodes.length, 'nodes');
}
...
function focusGraph() {
    const graphSection = document.getElementById('dependencyGraph');
    if (graphSection) {
...
// === AGENT-FRIENDLY ACTION FUNCTIONS ===
function copyUnusedFiles() {
    const unused = Object.values(filesData).filter(f => {
        return Object.keys(filesData).some(path => {
            const fileData = filesData[path];
            return fileData.dependents_count === 0 && fileData.dependents.length === 0;
        });
    });
    
    const paths = Object.keys(filesData)
        .filter(path => filesData[path].dependents_count === 0 && filesData[path].dependents.length === 0)
        .map(path => filesData[path].relative_path);
    
    copyToClipboard(paths.join('\\n'));
    alert(`Copied ${paths.length} unused file paths to clipboard`);
}

function copyDuplicates() {
    const duplicateInfo = duplicatesData.map(cluster => {
        return `Cluster ${cluster.cluster_id}: ${cluster.files.join(', ')}`;
    });
    
    copyToClipboard(duplicateInfo.join('\\n\\n'));
    alert(`Copied ${duplicatesData.length} duplicate clusters to clipboard`);
}

function copyHighRisk() {
    const highRisk = Object.values(filesData)
        .filter(f => f.risk_level === 'HIGH' || f.risk_level === 'CRITICAL')
        .map(f => f.relative_path);
    
    copyToClipboard(highRisk.join('\\n'));
    alert(`Copied ${highRisk.length} high-risk file paths to clipboard`);
}

function exportActionableJSON() {
    const actionable = {
        unused_files: Object.keys(filesData)
            .filter(path => filesData[path].dependents_count === 0)
            .map(path => filesData[path].relative_path),
        duplicate_clusters: duplicatesData.map(c => ({
            id: c.cluster_id,
            files: c.files,
            suggested_base: c.suggested_base_file
        })),
        high_risk_files: Object.values(filesData)
            .filter(f => f.risk_level === 'HIGH' || f.risk_level === 'CRITICAL')
            .map(f => ({
        ...
        recommendations: recommendationsData.slice(0, 10)
    };
    
    const json = JSON.stringify(actionable, null, 2);
    const blob = new Blob([json], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'actionable_analysis.json';
    a.click();
    
    alert('Exported actionable JSON for agent processing');
}

function showQuickStats() {
    const unused = Object.values(filesData).filter(f => f.dependents_count === 0).length;
    const highRisk = Object.values(filesData).filter(f => f.risk_level === 'HIGH' || f.risk_level === 'CRITICAL').length;
    const duplicates = duplicatesData.length;
    
    const stats = `
📊 Quick Stats:
• Total Files: ${Object.keys(filesData).length}
• Unused: ${unused}
• High Risk: ${highRisk}
...
window.dashboardAPI = {
    ...
    getUnusedFiles: () => {
        return Object.keys(filesData).filter(path => filesData[path].dependents_count === 0);
    },
    exportActionable: exportActionableJSON
};
"""
...
class CodeIntelligencePlatform:
    ...
    def __init__(self, project_path: str, scope_path: Optional[str] = None, report_folder_override: Optional[str] = None):
        self.project_path = Path(project_path).resolve()
        self.scope_path = scope_path
        self.report_folder_override = report_folder_override
    ...
    def analyze(self) -> AnalysisReport:
        ...
        # Create report folder (static override supported)
        if self.report_folder_override:
            rf = Path(self.report_folder_override)
            if not rf.is_absolute():
                rf = (self.project_path / rf).resolve()
            rf.mkdir(parents=True, exist_ok=True)
            # Maintain expected subdirectories
            (rf / "archives").mkdir(exist_ok=True)
            self.report_folder = rf
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Write reports into this folder (relative to project root unless absolute). Example: reports/latest"
    )
    parser.add_argument(
        '--non-interactive',
        '-n',
    ...
    platform = CodeIntelligencePlatform(project_path, scope_path, report_folder_override=args.report_folder)
