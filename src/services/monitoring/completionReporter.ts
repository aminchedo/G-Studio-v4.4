...
import { CompletionVerificationResult } from "../security/aiBehaviorValidation";
...
export class CompletionReporter {
  ...
  static recordErrorRootCause(
    ...
    if (error) {
      ...
      if (classification === "CODE_BUG") {
        rootCause =
          "Code bug: JS exception before network call or invalid request structure";
        classificationProof.jsException = true;
      } else if (classification === "CONFIG_FAILURE") {
        rootCause = "Configuration failure: API key missing or invalid";
      } else if (classification === "INFRA_FAILURE") {
        rootCause =
          "Infrastructure failure: Network error, timeout, or server error";
      ...
      } else if (classification === "MODEL_FAILURE") {
        rootCause =
          "Model failure: Empty response, refusal, or malformed output";
      }
    }
  ...
  static generateReport(
    ...
    const requestFixes = this.runtimeFixes.filter(
      (f) => f.requestId === requestId,
    );
    const requestErrors = this.errorRootCauses.filter(
  ...
  static formatReportAsMarkdown(report: CompletionReport): string {
    ...
    if (report.runtimeFixes.length === 0) {
      ...
      report.runtimeFixes.forEach((fix) => {
        lines.push(
          `- **${fix.type}:** ${fix.description} (${new Date(fix.timestamp).toISOString()})`,
        );
      });
    }
    lines.push("");
    ...
    lines.push("");
    lines.push(
      `- **UI Interactions:** ${report.liveVerification.uiInteractions ? "✅" : "❌"}`,
    );
    lines.push(
      `- **AI Request Sent:** ${report.liveVerification.aiRequestSent ? "✅" : "❌"}`,
    );
    lines.push(
      `- **AI Response Received:** ${report.liveVerification.aiResponseReceived ? "✅" : "❌"}`,
    );
    lines.push(
      `- **Network Reachable:** ${report.liveVerification.networkReachable ? "✅" : "❌"}`,
    );
    ...
    lines.push("");
    lines.push(
      `- **Code Detected:** ${report.codeDelivery.codeDetected ? "✅" : "❌"}`,
    );
    if (report.codeDelivery.languageDetected) {
      lines.push(
        `- **Language Detected:** ${report.codeDelivery.languageDetected}`,
      );
    }
    lines.push(`- **Code Block Count:** ${report.codeDelivery.codeBlockCount}`);
    lines.push(
      `- **UI Rendered:** ${report.codeDelivery.uiRendered ? "✅" : "❌"}`,
    );
    ...
    lines.push("");
    lines.push(
      `- **Is Complete:** ${report.completionVerification.isComplete ? "✅" : "❌"}`,
    );
    lines.push("");
    lines.push("**Criteria:**");
    lines.push(
      `- Model Responded: ${report.completionVerification.criteria.modelResponded ? "✅" : "❌"}`,
    );
    lines.push(
      `- Response Non-Empty: ${report.completionVerification.criteria.responseNonEmpty ? "✅" : "❌"}`,
    );
    lines.push(
      `- Contains Code Block: ${report.completionVerification.criteria.containsCodeBlock ? "✅" : "❌"}`,
    );
    lines.push(
      `- Code Syntactically Valid: ${report.completionVerification.criteria.codeSyntacticallyValid ? "✅" : "❌"}`,
    );
    lines.push(
      `- Contains Explanation: ${report.completionVerification.criteria.containsExplanation ? "✅" : "❌"}`,
    );
    lines.push(
      `- No Unclassified Errors: ${report.completionVerification.criteria.noUnclassifiedErrors ? "✅" : "❌"}`,
    );
    lines.push("");
