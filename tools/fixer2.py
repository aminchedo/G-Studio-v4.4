#!/usr/bin/env python3
"""
G-Studio Project Architecture Analyzer
Advanced codebase analysis with interactive menu and comprehensive reporting
"""
...
class ProjectAnalyzer:
    ...
    def show_menu(self):
        ...
        while True:
            self._clear_screen()
            print("╔" + "═" * 58 + "╗")
            print("║" + " " * 10 + "G-STUDIO ARCHITECTURE ANALYZER" + " " * 18 + "║")
            print("╚" + "═" * 58 + "╝")
            print()
            print("📁 Project:", str(self.root))
            print("📂 Reports:", str(self.report_dir))
            print()
            print("┌" + "─" * 58 + "┐")
            print("│  [1] 🔍 Full Analysis (Recommended)              │")
            print("│  [2] 🗑️  Find Unused Files                        │")
            print("│  [3] 📝 Find Duplicate Files                      │")
            print("│  [4] ⚛️  Find Duplicate Components & Hooks        │")
            print("│  [5] 🔧 Architecture Issues Analysis              │")
            print("│  [6] 📊 Project Statistics                        │")
            print("│  [7] 🎯 Generate Refactoring Plan                 │")
    ...
    def find_unused_files(self):
        ...
        for file in unused[:20]:
            risk = "LOW" if file['potential_value']['score'] < 50 else "MEDIUM"
            print(f"{'🟢' if risk == 'LOW' else '🟡'} {file['path']}")
            print(f"   Lines: {file['lines']} | Size: {file['size']} bytes")
            print(f"   Recommendation: {file['potential_value']['recommendation']}")
            print()
        
        if len(unused) > 20:
            print(f"... and {len(unused) - 20} more files")
    ...
    def find_duplicate_files(self):
        ...
        for group in duplicates[:10]:
            print(f"📝 {len(group['files'])} identical files (save {group['savings_potential']:,} bytes)")
    ...
    def find_duplicate_components(self):
        ...
        for comp in dup_components[:10]:
            print(f"• {comp['name']} ({comp['count']} copies)")
        ...
        for hook in dup_hooks[:10]:
            print(f"• {hook['name']} ({hook['count']} copies)")
    ...
    def export_dashboard(self):
        ...
        print(f"\n✅ Dashboard exported to: {self.report_dir / 'dashboard.html'}")
        input("\nPress Enter to continue...")
    ...
    def settings_menu(self):
        ...
        print("\n⚙️  SETTINGS")
        print("=" * 60)
        print(f"Current project: {self.root}")
        print(f"Reports location: {self.report_dir.parent}")
    ...
    def _create_refactor_plan(self, results: Dict) -> List[Dict]:
        ...
        phase1_tasks = []
        if results['unused']:
            phase1_tasks.append(f"Remove {len(results['unused'])} unused files")
        phase1_tasks.append("Add TODO comments to suspicious files")
        phase1_tasks.append("Update documentation")
        ...
        # Phase 2: Merge Similar Files
        phase2_tasks = []
        if results['similar']:
            phase2_tasks.append(f"Merge {len(results['similar'])} similar file groups")
        if results['duplicates']:
            phase2_tasks.append(f"Consolidate {len(results['duplicates'])} duplicate groups")
...
<head>
    <meta charset="UTF-8">
    <title>G-Studio Architecture Dashboard</title>
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .stat-card:hover {{ transform: translateY(-5px); box-shadow: 0 10px 30px rgba(99,102,241,0.3); }}
        .stat-value {{ font-size: 42px; font-weight: bold; color: #60a5fa; margin-bottom: 8px; }}
        .stat-label {{ color: #94a3b8; font-size: 13px; text-transform: uppercase; letter-spacing: 1px; }}
        .section {{ background: #1e293b; padding: 30px; border-radius: 12px; margin-bottom: 25px; }}
        .section h2 {{ margin-bottom: 25px; color: #60a5fa; font-size: 22px; display: flex; align-items: center; gap: 12px; }}
        .file-list {{ list-style: none; }}
        .file-item {{ 
            padding: 18px; margin-bottom: 12px; background: #0f172a;
            border-radius: 8px; display: flex; justify-content: space-between;
            align-items: center; transition: all 0.2s; border-left: 3px solid transparent;
        }}
        .file-item:hover {{ background: #1e293b; transform: translateX(5px); border-left-color: #6366f1; }}
        .file-path {{ font-family: 'Monaco', monospace; color: #f1f5f9; font-size: 13px; }}
        .file-meta {{ display: flex; gap: 15px; color: #94a3b8; font-size: 12px; align-items: center; }}
        .badge {{ 
        ...
        .footer {{ 
            margin-top: 50px; padding: 25px; text-align: center; 
            border-top: 2px solid #334155; color: #94a3b8;
        }}
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1>🏗️ G-Studio Architecture Dashboard</h1>
            <p>📁 {report['project_metadata']['root']}</p>
            <p>📅 Generated: {report['project_metadata']['analysis_date']}</p>
            <p style="margin-top: 10px;">
                <span class="badge risk-{report['risk_assessment']['overall_risk'].lower()}">
                    Risk: {report['risk_assessment']['overall_risk']} (Score: {report['risk_assessment']['risk_score']})
                </span>
            </p>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-value">{report['project_metadata']['total_files']}</div>
            ...
            <div class="stat-card">
                <div class="stat-value">{len(report['duplicate_hooks'])}</div>
                <div class="stat-label">Duplicate Hooks</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">{report['statistics']['complexity_analysis']['average_maintainability']:.0f}</div>
            ...
            <ul class="file-list">
                ...
                <li class="file-item">
                    ...
                    <div class="file-meta">
                        ...
                        <span class="badge {'badge-success' if f['potential_value']['score'] < 50 else 'badge-warning'}">
                            {f['potential_value']['recommendation']}
                        </span>
                    </div>
                </li>
                ''' for f in report['unused_files'][:25])}
            </ul>
        </div>
        ...
        <div class="section">
            ...
            <ul class="file-list">
                ...
                ''' for g in report['duplicate_files'][:15])}
            </ul>
        </div>
        ...
        <div class="section">
            ...
            <ul class="file-list">
                ...
                ''' for comp in report['duplicate_components'][:20])}
            </ul>
        </div>
        ...
        <div class="section">
            ...
            <ul class="file-list">
                ...
                ''' for hook in report['duplicate_hooks'][:20])}
            </ul>
        </div>
        ...
        <div class="section">
            ...
            {"".join(f'''
            <div style="background: #0f172a; padding: 20px; border-radius: 8px; margin-bottom: 15px; border-left: 4px solid #6366f1;">
                <div style="font-size: 18px; font-weight: bold; margin-bottom: 10px;">
                    Phase {phase['phase']}: {phase['name']}
                </div>
                <div style="color: #94a3b8; font-size: 13px; margin-bottom: 15px;">
                    Duration: {phase['duration']} | Risk: <span class="badge risk-{phase['risk'].lower()}">{phase['risk']}</span>
                </div>
                <ul style="padding-left: 20px; color: #cbd5e1;">
                    {"".join(f"<li style='margin: 8px 0;'>{task}</li>" for task in phase['tasks'])}
                </ul>
            </div>
    ...
    def _display_summary(self, report: Dict):
        ...
        print(f"🌐 Dashboard:    {self.report_dir / 'dashboard.html'}")


def main():
    import argparse
    
    parser = argparse.ArgumentParser(description="G-Studio Project Architecture Analyzer")
