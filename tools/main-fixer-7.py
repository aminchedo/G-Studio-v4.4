...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.placeholder = 'Search files, recommendations, or clusters...';
    ...
    searchInput.addEventListener('input', function(e) {
        ...
        const cards = document.querySelectorAll('.cluster, .recommendation-card, .file-card, tr');
        ...
        button.addEventListener('click', function() {
            ...
            sections.forEach(section => {
                if (filter.value === 'all') {
                    ...
                    if (sectionText.includes(filter.value)) {
                        section.style.display = '';
                    } else {
    ...
    exportButton.addEventListener('click', function() {
        const summary = {
            ...
            files_analyzed: Object.keys(filesData).length,
            duplicates: duplicatesData.length,
            recommendations: recommendationsData.length,
            critical_files: Object.values(filesData).filter(f => f.risk_level === 'CRITICAL').length,
            high_risk_files: Object.values(filesData).filter(f => f.risk_level === 'HIGH').length
        };
...
function highlightCode(code) {
    ...
    return code
        .replace(/\b(const|let|var|function|return|if|else|for|while)\b/g, '<span style="color: var(--accent)">$1</span>')
        .replace(/\b(true|false|null|undefined)\b/g, '<span style="color: var(--warning)">$1</span>')
...
class CodeIntelligencePlatform:
    ...
    def analyze(self) -> AnalysisReport:
        ...
        try:
            ...
            self.logger.info("Phase 6: Recommendation Generation")
            recommendation_engine = RecommendationEngine()
            
            for path, meta in self.files.items():
                rec, reasoning, confidence = recommendation_engine.generate_comprehensive_recommendation(
            ...
            self.logger.info("Phase 7: Archive Eligibility Evaluation")
            ...
            # Phase 8: Generate merge strategies for duplicates
            self.logger.info("Phase 8: Merge Strategy Generation")
            for cluster in duplicates:
                cluster.merge_strategy = recommendation_engine.generate_merge_strategy(
                    cluster, self.files
                )
            
            # Phase 9: Calculate quality metrics
            self.logger.info("Phase 9: Quality Metrics Calculation")
            quality_metrics = self._calculate_quality_metrics()
            
            # Phase 10: Compile recommendations
            self.logger.info("Phase 10: Compiling Final Recommendations")
            recommendations = self._compile_recommendations(duplicates, usage_analyzer)
            ...
            self.report = AnalysisReport(
                ...
                files={
                    path: {
                        ...
                        'detailed_reasoning': meta.detailed_reasoning,
                        'last_modified_days': meta.last_modified_days,
            ...
            # Generate reports
            self.logger.info("Phase 11: Report Generation")
    ...
    def _calculate_quality_metrics(self) -> Dict[str, Any]:
        ...
        total_types = sum(f.interface_count + f.type_count for f in self.files.values())
        ...
        type_safety_score = 100 - min((total_any / total_files) * 10, 100)
        
        test_files = sum(1 for f in self.files.values() if f.is_test_file)
        test_coverage_estimate = (test_files / total_files) * 100
        ...
        return {
            ...
            'total_any_usage': total_any,
            'total_type_definitions': total_types
        }
    
    def _compile_recommendations(self, duplicates: List[DuplicateCluster], usage_analyzer: UsageAnalyzer) -> List[Dict[str, Any]]:
        ...
        for cluster in duplicates:
            if cluster.risk_level in {RiskLevel.HIGH, RiskLevel.CRITICAL}:
                recommendations.append({
                    'title': f"Consolidate Duplicate Cluster {cluster.cluster_id}",
                    'priority': 'HIGH',
                    'action': cluster.recommendation.value,
                    'reasoning': cluster.diff_summary,
                    'confidence': cluster.confidence_score,
                    'impact': f"Affects {cluster.cluster_size} files",
                    'effort': 'Medium',
                    'type': 'duplicate'
                })
        
        # Medium priority: Unused files
        for file_path in usage_analyzer.unused[:10]:
            meta = self.files[file_path]
            recommendations.append({
                'title': f"Review Unused File: {meta.name}",
                'priority': 'MEDIUM',
                'action': 'INVESTIGATE',
                'reasoning': meta.detailed_reasoning,
                'confidence': 0.70,
                'impact': 'Low - no active dependents',
                'effort': 'Low',
                'type': 'unused'
            })
        
        # Medium priority: Unwired features
        for file_path in usage_analyzer.unwired[:5]:
            meta = self.files[file_path]
            recommendations.append({
                'title': f"Wire Feature: {meta.name}",
                'priority': 'MEDIUM',
                'action': 'WIRE_TO_APPLICATION',
                'reasoning': [
                    f"Complete implementation ({meta.lines} lines)",
                    "Not integrated into application",
                    "Consider adding to feature module"
                ],
                'confidence': 0.75,
                'impact': 'Medium - adds functionality',
                'effort': 'Medium',
                'type': 'unwired'
            })
        ...
        weak_typed_files = [
            (path, meta) for path, meta in self.files.items()
            if meta.any_count > 5
        ]
        
        for file_path, meta in weak_typed_files[:5]:
            recommendations.append({
                'title': f"Improve Type Safety: {meta.name}",
                'priority': 'LOW',
                'action': 'ADD_TYPE_SAFETY',
                'reasoning': [
                    f"Contains {meta.any_count} 'any' usages",
...
class InteractiveCLI:
    """Interactive command-line interface with comprehensive features"""
    ...
    def _view_file_details(self):
        ...
        if data.get('detailed_reasoning'):
            print(f"\n{Colors.BOLD}Detailed Reasoning:{Colors.ENDC}")
            for reason in data['detailed_reasoning']:
                print(f"  • {reason}")
    ...
    def _show_recommendations(self):
        ...
        if not self.report.recommendations:
            print(f"{Colors.OKGREEN}✓ No critical recommendations{Colors.ENDC}")
            return
        
        for i, rec in enumerate(self.report.recommendations[:10], 1):
            priority_color = Colors.FAIL if rec['priority'] == 'HIGH' else Colors.WARNING if rec['priority'] == 'MEDIUM' else Colors.OKBLUE
            
            print(f"\n{priority_color}[{rec['priority']}]{Colors.ENDC} {Colors.BOLD}{i}. {rec['title']}{Colors.ENDC}")
        ...
        if len(self.report.recommendations) > 10:
            print(f"\n{Colors.WARNING}... and {len(self.report.recommendations) - 10} more recommendations{Colors.ENDC}")
            print(f"View full list in optimization_dashboard.html")
...
def main():
    ...
    if len(sys.argv) >= 2:
        ...
        scope_path = None
        print(f"{Colors.OKCYAN}No path specified, using current directory: {project_path}{Colors.ENDC}\n")
