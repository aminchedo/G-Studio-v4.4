#!/usr/bin/env python3
"""
G-Studio Intelligent Code Analyzer v6.1.0-beta
===============================================
Main Purpose:
...
What's new in v6.1.0-beta:
    ✓ **Much stronger tree‑sitter usage** – precise capture of import details, 
      export details, React components, custom hooks, and context providers.
      New fields `import_details`, `export_details`, `is_functional_component`,
      `is_custom_hook`, `is_context_provider`, `has_create_context` stored in FileInfo.
    ✓ **Content‑aware layer classification** – now uses code signals (createContext,
      custom hooks, service patterns) in addition to folder heuristics.
    ✓ **Numeric severity score (0–100)** for each WiringIssue – used for sorting
      and prioritisation. Severity enum automatically mapped to impact score.
    ✓ **New CLI flag `--since-last-commit` (`-c`)** – only analyse files changed
      since the last Git commit. Falls back to full scan if Git unavailable.
    ✓ **CSV export** for `valuable_unused` and `wiring_issues` via `--csv` flag.
      Two files are generated in the output directory.
    ✓ **HTML report polish** – sortable tables, refined severity badge colours,
      collapsible `<details>` blocks for refactoring samples, and improved
      progress bar styling.
    ✓ Full backwards compatibility – all v6.0.0 features remain intact.
...
class FileInfo:
    ...
    import_details: List[Dict] = field(default_factory=list)   # list of {'source': str, 'imported': list of {'name': str, 'alias': str, 'type_only': bool}}
    export_details: List[Dict] = field(default_factory=list)   # list of {'kind': str, 'source': str (if re-export), 'names': list of {'name': str, 'alias': str}}
...
# =============================================================================
# CACHE MANAGER (with compatibility for new fields)
# =============================================================================
class CacheManager:
...
# =============================================================================
# FILE PARSER (with real tree‑sitter support – enhanced v6.1.0)
# =============================================================================
class FileParser:
    ...
    # -------------------------------------------------------------------------
    # ENHANCED in v6.1.0: Much richer AST extraction
    # -------------------------------------------------------------------------
    def _parse_with_tree_sitter(self, content: str, parser: Any) -> Dict[str, Any]:
        ...
        def walk(node: Node):
                ...
                if source_node:
                    ...
                    if import_clause:
                        for spec in import_clause.children:
                            # import default, { a, b as c }, * as name
                            if spec.type == 'identifier':
                                imported_names.append({'name': get_node_text(spec), 'alias': None, 'type_only': False})
                            elif spec.type == 'namespace_import':
                                for ns in spec.children:
                                    if ns.type == 'identifier':
                                        imported_names.append({'name': '*', 'alias': get_node_text(ns), 'type_only': False})
                                ...
                                for n in spec.children:
                                    if n.type == 'import_specifier':
                                        ...
                                        imported_names.append({'name': name, 'alias': alias, 'type_only': False})
            ...
            # ----- EXPORT statements -----
                ...
                # Detect re-exports (export ... from 'module')
                ...
                if is_reexport and source_node:
                    ...
                    # Also add to exports list for backward compatibility
                    for n in names:
                        result['exports'].append(n['alias'] or n['name'])
                else:
                    # local export: export { a, b }, export const x, etc.
                    names = []
                    for child in node.children:
            ...
            # Functional component: function/arrow that returns JSX
            if node.type in {'function_declaration', 'arrow_function'}:
                ...
                if has_jsx(node):
                    ...
                    # If it's a custom hook (starts with 'use')
            ...
            if node.type == 'call_expression':
                ...
                if func and func.type == 'identifier':
                    ...
                    if name == 'createContext':
                        ...
                        # Also check if this file exports a Provider component
                        # We'll detect context provider by looking for a component that returns JSX containing .Provider
                        # (handled elsewhere)
            
            # ----- Custom hook definition (standalone) -----
            ...
            # ----- Hooks usage (call expressions starting with 'use') -----
            if node.type == 'call_expression':
                func = node.child_by_field_name('function')
                if func and func.type == 'identifier':
                    name = get_node_text(func)
        ...
        # Context provider detection: has createContext and returns a component with .Provider
        if result['has_create_context']:
            # Simple heuristic: file exports a component that uses .Provider in JSX
    ...
    def _parse_with_regex(self, content: str) -> Dict[str, Any]:
        ...
        result = {
            ...
            'has_interface': False,
            'has_type_export': False,
            # NEW fields – empty for regex fallback
            'import_details': [],
            'export_details': [],
            'is_functional_component': False,
            'is_custom_hook': False,
        ...
        # Basic React component detection (heuristics)
...
# =============================================================================
# WIRING ISSUE DETECTOR (5+ real rules) – ENHANCED with severity scoring
# =============================================================================
class WiringIssueDetector:
    ...
    # NEW in v6.1.0: compute severity score based on Severity enum
    @staticmethod
    def _severity_to_score(severity: Severity) -> int:
        low, high = Config.SEVERITY_SCORES[severity]
        # Use mid-point for simplicity
        return (low + high) // 2
...
class GitHelper:
    ...
    # NEW in v6.1.0: get list of files changed since last commit
    @staticmethod
    def get_changed_files_since_last_commit(project_root: Path) -> Optional[Set[str]]:
        """Return set of relative paths of files changed since last commit."""
        try:
            # Try HEAD~1 (previous commit); if that fails (first commit), use HEAD
            ...
            if result.returncode != 0:
                # Probably no previous commit; try HEAD
                result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                # Filter only existing files (some may be deleted)
                existing = set()
                for f in files:
                    full = project_root / f
                    if full.exists():
...
# =============================================================================
# HTML REPORT GENERATOR (enhanced with real tables & styling – v6.1.0)
# =============================================================================
class HTMLReportGenerator:
    ...
    def generate(result: AnalysisResult, output_path: Path, with_git: bool = False):
        ...
        wiring_rows = ""
        # NEW: sort by severity_score descending
        for issue in sorted(result.wiring_issues, key=lambda x: x.severity_score, reverse=True)[:50]:
            ...
            # NEW: collapsible details for reasoning & refactor_sample
            details_html = ""
        ...
        # NEW: refined severity badge colours
        css_severity = """
...
<head>
    ...
    <title>G-Studio Code Analysis Report v6.1</title>
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        {css_severity}
        .badge-high {{ background: #10b981; color: white; }} /* kept for compatibility, but overridden above */
    ...
    <!-- NEW: simple table sorting script -->
    <script>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🎯 G-Studio Code Analysis v6.1</h1>
        ...
        <div class="footer">
            Generated by G-Studio Code Intelligence v6.1.0-beta<br>
...
# =============================================================================
# MAIN ANALYZER (with new features: content-based layer, changed-only, CSV export)
# =============================================================================
class GStudioAnalyzer:
    ...
    def __init__(self, project_path: Path, output_dir: Path, 
                 ...
                 changed_only: bool = False,  # NEW in v6.1.0
                 csv_export: bool = False):   # NEW in v6.1.0
        self.project_path = project_path
        ...
        # NEW: track skipped files count
        self.skipped_files = 0
    
    def analyze(self) -> AnalysisResult:
        """Run complete analysis."""
        log_header("G-STUDIO INTELLIGENT CODE ANALYZER v6.1.0-beta")
        ...
        # NEW: filter to changed files only if requested
        if self.changed_only:
            ...
            if changed_set is not None:
                # Convert to relative paths (as string) for matching
    ...
    def _analyze_single_file(self, file_path: Path):
        ...
        try:
            ...
            if cached:
                # Use cached data, but fill missing new fields with defaults for compatibility
                cached_info = cached['file_info']
                # Create a dict with default values for all FileInfo fields
                ...
                # Determine layer using enhanced content-aware method
    ...
    # NEW in v6.1.0: content-aware layer classification
    def _classify_layer_with_signals(self, file_path: Path, content: str, parsed: Dict) -> LayerType:
        """Determine layer using content signals, fallback to folder heuristics."""
        # 1. Strong signals from content
        if parsed.get('has_create_context', False) or 'createContext' in content:
            # Context file – likely in contexts/ but could be elsewhere
        ...
        if parsed.get('is_functional_component', False):
            # Component file
            # But if it's a provider with createContext, it's context layer
            if parsed.get('is_context_provider', False):
                return LayerType.CONTEXTS
            return LayerType.COMPONENTS
        
        # 2. Service detection: name contains Service/Manager/Engine or exports service-like object
        name = file_path.stem
        if 'Service' in name or 'Manager' in name or 'Engine' in name:
            return LayerType.SERVICES
        # Look for export of object with methods
        ...
        # 4. Fallback to folder-based
        return self._determine_layer(file_path)
    
    # (existing folder-based detection kept for fallback)
    def _determine_layer(self, file_path: Path) -> LayerType:
        """Determine architectural layer from file path (folder heuristic)."""
...
<{name} />"""
    ...
    # NEW in v6.1.0: CSV export
    def _export_csv(self):
        """Export valuable_unused and wiring_issues to CSV files."""
        if not self.result.valuable_unused and not self.result.wiring_issues:
            return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="G-Studio Intelligent Code Analyzer v6.1.0-beta",
...
Examples:
  ...
  %(prog)s . --csv                       # export CSV files alongside JSON/HTML
    ...
    parser.add_argument(
        '--since-last-commit', '-c',
        action='store_true',
        help='Only analyze files changed since the last Git commit'
    )
    
    parser.add_argument(
        '--csv',
        action='store_true',
        help='Export valuable_unused and wiring_issues as CSV files'
    )
