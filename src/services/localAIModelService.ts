...
export type ModelStatus = 'NOT_INSTALLED' | 'DOWNLOADING' | 'READY' | 'ERROR' | 'LOADING' | 'UNLOADED';
...
async function getUserDataDir(): Promise<string> {
  if (typeof window !== 'undefined' && (window as any).electron?.ipcRenderer) {
    ...
    try {
      const result = await (window as any).electron.ipcRenderer.invoke('local-ai:get-user-data-dir');
      if (result && result.path) {
        return result.path;
      }
    } catch (e) {
      console.warn('[LocalAIModelService] Could not get user data dir via IPC, using fallback:', e);
    }
  }
...
async function getActualModelPath(): Promise<string> {
  ...
  const fallbackExists = await LocalAIModelService.checkModelExists(fallbackPath);
  if (fallbackExists) {
    console.log('[LocalAIModelService] Using fallback model:', FALLBACK_MODEL_FILE);
    return fallbackPath;
  }
...
export class LocalAIModelService {
  ...
  private static session: LlamaChatSession | null = null;
  ...
  static async initialize(): Promise<void> {
    ...
    try {
      ...
      if (exists) {
        ...
        console.log('[LocalAIModelService] Model file found, status: UNLOADED (not loaded)');
      } else {
        this.status = 'NOT_INSTALLED';
        console.log('[LocalAIModelService] Model file not found, status: NOT_INSTALLED');
   ...
   */
  private static async checkModelExists(path: string): Promise<boolean> {
    // In Electron, use IPC to check file existence
    if (typeof window !== 'undefined' && (window as any).electron?.ipcRenderer) {
      try {
        const result = await (window as any).electron.ipcRenderer.invoke('local-ai:check-model', path);
    ...
    try {
      ...
      if (response.ok) {
        console.log('[LocalAIModelService] Model found in public folder:', modelUrl);
        console.log('[LocalAIModelService] Model path: public/' + MODEL_FILE);
        return true;
      }
    } catch (e) {
      // Model not in public folder, try IndexedDB
      console.log('[LocalAIModelService] Model not in public folder, checking IndexedDB...');
    }
  ...
  private static async downloadModelElectron(
    ...
    const modelDir = lastSlash > 0 ? modelPath.substring(0, lastSlash) : modelPath;

    // Create directory if needed (via IPC in Electron)
    await (window as any).electron.ipcRenderer.invoke('local-ai:ensure-dir', modelDir);

    // Get existing file size for resume
    let startByte = 0;
    try {
      const fileInfo = await (window as any).electron.ipcRenderer.invoke('local-ai:get-file-size', modelPath);
    ...
    const fileHandle = await (window as any).electron.ipcRenderer.invoke('local-ai:open-file', modelPath, startByte > 0 ? 'append' : 'write');
    ...
    while (true) {
      ...
      // Write chunk (via IPC)
      await (window as any).electron.ipcRenderer.invoke('local-ai:write-chunk', fileHandle, Array.from(value));

      bytesDownloaded += value.length;
      
      const now = Date.now();
      if (now - lastUpdateTime > 500) { // Update every 500ms
        const speed = ((bytesDownloaded - lastBytes) / (now - lastUpdateTime)) * 1000;
    ...
    // Close file
    await (window as any).electron.ipcRenderer.invoke('local-ai:close-file', fileHandle);
  ...
  private static async downloadModelBrowser(
    ...
    try {
      ...
      if (result && result.chunks) {
        ...
        startByte = existingChunks.reduce((sum, chunk) => sum + chunk.length, 0);
        console.log(`[LocalAIModelService] Resuming from ${startByte} bytes`);
      }
    } catch (e) {
      console.log('[LocalAIModelService] No existing download found, starting fresh');
    }
    ...
    while (true) {
      ...
      if (now - lastUpdateTime > 500) { // Update every 500ms
        ...
        if (newChunks.length > 0 && bytesDownloaded % (10 * 1024 * 1024) < value.length) {
          await this.saveChunksToIndexedDB(db, MODEL_FILE, [...existingChunks, ...newChunks]);
    ...
    const modelBlob = new Blob(allChunks);
  ...
  private static async verifyModelIntegrity(path: string): Promise<void> {
    try {
      if (typeof window !== 'undefined' && (window as any).electron?.ipcRenderer) {
        ...
        const fileInfo = await (window as any).electron.ipcRenderer.invoke('local-ai:get-file-size', path);
        ...
        const hashResult = await (window as any).electron.ipcRenderer.invoke('local-ai:compute-sha256', path);
        ...
        if (hashResult.success && hashResult.hash) {
          ...
          if (storedHash) {
            ...
            if (hashResult.hash !== storedHash) {
              throw new Error('Model file hash mismatch - file may be corrupted');
            }
            console.log('[LocalAIModelService] Model integrity verified (SHA-256 match)');
          } else {
            // First time - store the hash for future verification
            localStorage.setItem(MODEL_SHA256_STORAGE_KEY, hashResult.hash);
            console.log('[LocalAIModelService] Model integrity verified (SHA-256 computed and stored)');
          }
        } else {
          // Fallback to size check if hash computation fails
          console.warn('[LocalAIModelService] SHA-256 computation failed, using size check only');
          console.log('[LocalAIModelService] Model integrity verified (size check)');
        }
      }
    } catch (error) {
      console.warn('[LocalAIModelService] Integrity verification skipped:', error);
    }
  }
  ...
  static async loadModel(): Promise<void> {
    ...
    if (typeof navigator !== 'undefined' && (navigator as any).deviceMemory) {
      ...
      if (deviceMemory && deviceMemory < 4) {
        console.warn('[LocalAIModelService] Low device memory detected, model may not load properly');
      }
    }
    ...
    try {
      ...
      try {
        ...
        console.warn('[LocalAIModelService] Pre-warm failed (non-critical):', error);
      }
  ...
  private static async loadModelElectron(): Promise<void> {
    ...
    if (!exists) {
      throw new Error(`Model file not found. Expected at:\n  ${primaryPath}\nor:\n  ${fallbackPath}\n\nPlease place your model file in the models directory.`);
    }

    // Load model using node-llama-cpp
    await loadLlamaModule();
    if (!getLlama) {
      throw new Error('node-llama-cpp not available. This feature requires Electron or Node.js environment.');
    }
  ...
  private static async loadModelBrowser(): Promise<void> {
    ...
    if (!exists) {
      throw new Error(`Model file not found. Please ensure the model file (${MODEL_FILE}) exists in the public folder or download it first.`);
    }
    ...
    let modelBlob: Blob | null = null;
      ...
      try {
        ...
        if (response.ok) {
          ...
          modelPath = modelUrl;
          console.log('[LocalAIModelService] Model loaded from public folder:', modelUrl, 'size:', modelBlob.size);
        } else {
          throw new Error('Model not found in public folder');
        }
      } catch (e) {
      // Fallback to IndexedDB
      console.log('[LocalAIModelService] Model not in project root, loading from IndexedDB...');
      const db = await this.openIndexedDB();
      const transaction = db.transaction(['models'], 'readonly');
      const store = transaction.objectStore('models');
      const request = store.get(MODEL_FILE);
      ...
      console.log('[LocalAIModelService] Model loaded from IndexedDB, size:', modelBlob.size);
    }
    ...
    try {
      ...
      console.log('[LocalAIModelService] Browser model session initialized successfully');
    } catch (error) {
      console.error('[LocalAIModelService] Error initializing browser model:', error);
  ...
  static async infer(
    ...
    try {
      ...
      if (this.consecutiveFailures >= 3) {
        ...
        throw new Error('Model health degraded. Too many consecutive failures.');
  ...
  static async calculateImportance(content: string): Promise<number> {
    ...
    try {
      ...
      console.warn('[LocalAIModelService] Importance calculation failed, using default');
      return 0.5;
    }
  }
   ...
   */
  static async createSummary(entries: Array<{ role: string; content: string }>): Promise<string> {
    if (this.status !== 'READY') {
  ...
  private static startRecoveryMonitoring(): void {
    ...
    (this as any).recoveryInterval = setInterval(async () => {
      ...
      try {
        ...
        console.log('[LOCAL_AI_RECOVERY]: FAILED (attempt ' + this.recoveryAttempts + ')');
      }
