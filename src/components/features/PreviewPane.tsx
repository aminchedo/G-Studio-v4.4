...
import { Play, RefreshCw, Maximize2, Minimize2, Download, Copy, ExternalLink, AlertCircle, CheckCircle2, XCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
export const PreviewPane: React.FC<PreviewPaneProps> = ({ files, activeFile }) => {
  const [output, setOutput] = useState('');
  const [isRunning, setIsRunning] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [isFullscreen, setIsFullscreen] = useState(false);
  const [previewMode, setPreviewMode] = useState<'html' | 'react' | 'console'>('html');
  const iframeRef = useRef<HTMLIFrameElement>(null);
  const [logs, setLogs] = useState<Array<{ type: 'log' | 'error' | 'warn'; message: string; timestamp: number }>>([]);
    ...
    window.onerror = (msg, url, line, col, error) => {
      ...
      // JavaScript Console Preview
        ...
        try {
          ...
          setLogs(capturedLogs.map((log, i) => ({
            ...log,
...
<body>
  ...
  <div class="container">
    ...
    <div class="box">Box 2</div>
        ...
        throw new Error(`Preview not supported for .${ext} files. Supported: html, jsx, tsx, js, ts, css`);
      }
  ...
  return (
    <div className={`flex flex-col h-full bg-slate-900 ${isFullscreen ? 'fixed inset-0 z-[9999]' : ''}`}>
            ...
            <div className="flex items-center gap-2">
              {previewMode !== 'console' && (
                <>
                  ...
                  >
                    <RefreshCw className={`w-4 h-4 text-white ${isRunning ? 'animate-spin' : ''}`} />
                  </button>
      ...
      {(output || error) && (
        <div className={`px-4 py-2 text-xs border-b shrink-0 ${
          error ? 'bg-red-900/30 border-red-800 text-red-300' : 'bg-emerald-900/30 border-emerald-800 text-emerald-300'
        }`}>
          <div className="flex items-center gap-2">
            {error ? <XCircle className="w-3.5 h-3.5" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
      ...
      {!activeFile && (
        <div className="flex-1 flex items-center justify-center text-slate-400">
          <div className="text-center">
            ...
            <p className="text-xs mt-1">Open an HTML, React, or JS file to preview</p>
          </div>
        </div>
      )}
      </>
