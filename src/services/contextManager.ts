...
export class ContextManager {
  ...
  private static getTokenBudget(modelId: string): TokenBudget {
    ...
    return {
      ...
      availableForContext: maxPromptTokens - this.SYSTEM_INSTRUCTION_TOKENS - this.TOOLS_RESERVED_TOKENS
    };
  }
  ...
  static async extractRelevantContext(
    ...
    const latestAssistantMessage = messages.filter(m => m.role === 'model' || m.role === 'assistant').pop();
    ...
    const chunks: ContextChunk[] = messages
      .filter(msg => {
        ...
        return msg.role === 'user' && !!msg.content && msg.content.trim().length > 0;
      })
      .map(msg => ({
    ...
    if (useHybridRanking) {
      ...
      chunks.forEach(chunk => {
        chunk.relevanceScore = this.calculateRelevance(chunk.content, currentMessage);
    ...
    for (const msg of lastMessages) {
      if (!usedIds.has(msg.id) && totalTokens + (msg.tokens || 0) <= contextLimit) {
    ...
    for (const chunk of chunks) {
      if (!usedIds.has(chunk.id) && totalTokens + (chunk.tokens || 0) <= contextLimit) {
  ...
  private static async calculateHybridScore(
    ...
    const importanceScore = await this.calculateImportance(chunk.content, currentMessage);
    
    // 3. Semantic similarity (lightweight keyword-based)
    const semanticScore = this.calculateSemanticSimilarity(chunk.content, currentMessage);
    
    // 4. Structural relevance (file/intent matching)
    const structuralScore = this.calculateStructuralRelevance(chunk.content, currentMessage);
   ...
   */
  private static calculateSemanticSimilarity(chunkContent: string, currentMessage: string): number {
    const chunkWords = new Set(chunkContent.toLowerCase().split(/\W+/).filter(w => w.length > 3));
    const messageWords = new Set(currentMessage.toLowerCase().split(/\W+/).filter(w => w.length > 3));
   ...
   */
  private static calculateStructuralRelevance(chunkContent: string, currentMessage: string): number {
    // Extract file paths
    const filePattern = /[\w\-\.\/]+\.(tsx?|jsx?|css|html|json|md|py|java|cpp|rs)/gi;
   ...
   */
  private static calculateRelevance(chunkContent: string, currentMessage: string): number {
   ...
   */
  static async calculateImportance(content: string, query: string = ''): Promise<number> {
    // Check cache first
   ...
   */
  static async summarizeContext(chunks: ContextChunk[], maxTokens: number): Promise<string> {
    // Try AI summarization if available
    if (LocalAIModelService.getStatus() === 'READY' && chunks.length > 5) {
      try {
        const entries = chunks.map(c => ({ role: c.role, content: c.content }));
        return await this.createSummary(entries);
      } catch (error) {
        console.warn('[ContextManager] AI summarization failed, using fallback');
      }
    }
    ...
    for (const chunk of chunks) {
      const chunkTokens = chunk.tokens || TokenOptimizer.estimateTokens(chunk.content);
  ...
  static getLatestMessageOnly(messages: Message[]): Content[] {
    const latest = messages
      .filter(msg => {
        ...
        return (msg.role === 'user' || msg.role === 'model' || msg.role === 'assistant') 
          && contentText 
