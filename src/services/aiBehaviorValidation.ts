...
export type ClassificationResult = typeof ClassificationResult[keyof typeof ClassificationResult];
...
export class AIBehaviorValidation {
  ...
  static classifyResponse(
    ...
    if (!requestId || requestId.trim() === '') {
      const codeBugError = new Error('requestId is required but was missing or empty');
      this.logAttempt({
        requestId: 'MISSING',
        modelName,
        failureType: FailureType.CODE_BUG,
    ...
    if (!validation.isValid || !validation.containsCode || validation.isRefusal) {
      this.logAttempt({
        requestId,
        modelName,
        failureType: FailureType.MODEL_FAILURE,
  ...
  static validateCodeResponse(text: string): ValidationResult {
    ...
    const isRefusal = refusalPatterns.some(pattern => pattern.test(normalizedText));
    if (isRefusal) {
      return {
        isValid: false,
        containsCode: false,
    ...
    if (!containsCode) {
      ...
      const isExplanationOnly = explanationOnlyPatterns.some(pattern => 
        pattern.test(normalizedText.substring(0, 100))
      ) && !hasCodeBlocks && !hasInlineCode;
    ...
    return {
      ...
      isRefusal: false,
      reason: isTruncated ? 'Truncated response' : hasValidSyntax ? undefined : 'Invalid syntax'
    };
  }
  ...
  static isPermanentQuotaExhaustion(error: Error): boolean {
    ...
    if (errorMessage.includes('resource_exhausted') || errorString.includes('resource_exhausted')) {
      // If combined with limit: 0 or free_tier quota = 0, it's permanent
      if (errorMessage.includes('limit: 0') || 
          errorMessage.includes('limit = 0') ||
          errorMessage.includes('free_tier') && (errorMessage.includes('= 0') || errorMessage.includes(': 0'))) {
        return true;
      }
    }
    ...
    if (freeTierPattern.test(errorMessage) || freeTierPattern.test(errorString)) {
      return true;
    }
    
    // Check for retryDelay combined with zero quota (indicates permanent exhaustion)
    if ((errorMessage.includes('retrydelay') || errorMessage.includes('retry_delay')) &&
        (errorMessage.includes('limit: 0') || errorMessage.includes('limit = 0') || 
  ...
  static isInvalidRoleError(error: Error): boolean {
    const errorMessage = error.message.toLowerCase();
    return (errorMessage.includes('400') || errorMessage.includes('bad request')) &&
  ...
  static is400InvalidArgument(error: Error): boolean {
    ...
    const errorString = JSON.stringify(error).toLowerCase();
    if (errorString.includes('invalid_argument') || errorString.includes('invalid argument')) {
      return true;
    }
  ...
  static getHttpStatusCode(error: Error): number | null {
    ...
    const statusMatch = errorMessage.match(/\b(400|401|403|429|500|502|503|504)\b/);
    if (statusMatch) {
   ...
   */
  static isCodeBug(error: Error | null, requestId?: string, apiKey?: string, modelId?: ModelId, payload?: any): boolean {
    ...
    if (modelId !== undefined && !Object.values(ModelId).includes(modelId as ModelId)) {
      return true;
    }
    ...
    if (error) {
      ...
      if (errorName === 'typeerror' && !errorMessage.includes('network') && !errorMessage.includes('fetch')) {
        return true;
      }
      ...
      if (errorMessage.includes('is not a function') ||
          ...
          (errorMessage.includes('is not defined') && !errorMessage.includes('api key'))) {
        return true;
      }
    }
  ...
  static validateRequestPreFlight(
    ...
    const isValidModelId = typeof modelId === 'string' && modelId.trim() !== '' && modelId.startsWith('gemini') ||
                           Object.values(ModelId).includes(modelId as ModelId);
  ...
  static validateFinalPayload(contents: any[], requestId: string): void {
    if (!Array.isArray(contents)) {
      throw new Error(`CODE_BUG: Contents must be an array. requestId=${requestId}`);
    }
    ...
    for (let i = 0; i < contents.length; i++) {
      ...
      if (!content || typeof content !== 'object') {
        throw new Error(`CODE_BUG: Invalid content item at index ${i}. requestId=${requestId}`);
      }

      if (!content.role) {
        throw new Error(`CODE_BUG: Content item at index ${i} missing role. requestId=${requestId}`);
      }
      ...
      if (FORBIDDEN_ROLES.includes(role)) {
        throw new Error(`CODE_BUG: Forbidden role '${role}' at index ${i}. Gemini API only accepts 'user' or 'model' roles. requestId=${requestId}`);
      }

      // Check if role is not in allowed list
      if (!ALLOWED_ROLES.includes(role)) {
        throw new Error(`CODE_BUG: Invalid role '${role}' at index ${i}. Gemini API only accepts 'user' or 'model' roles. Unknown roles are forbidden. requestId=${requestId}`);
      }
    }
  }
  ...
  private static logAttempt(attempt: ClassificationAttempt): void {
    ...
    console.log(`[AIBehaviorValidation][requestId=${attempt.requestId}][model=${attempt.modelName}][attempt=${attempt.attemptNumber}]:`, {
      classification: attempt.failureType === null ? 'MODEL_SUCCESS' : attempt.failureType,
      ...logData
    });
  ...
  static shouldShowNotification(context: NotificationContext): boolean {
    ...
    try {
      ...
      if (!FailureType || typeof FailureType !== 'object' || !ClassificationResult || typeof ClassificationResult !== 'object') {
        // Safe default: only show for final phase
        return context.executionPhase === 'final' || context.allModelsExhausted === true;
      }
      ...
      const isQuotaExhausted = errorTypeStr === ClassificationResult.QUOTA_EXHAUSTED ||
                               errorTypeStr === 'QUOTA_EXHAUSTED';
      if (isQuotaExhausted) {
        return true;
      }

      // ALLOWED: User action required (API key missing/invalid)
      const isConfigFailure = errorTypeStr === FailureType.CONFIG_FAILURE ||
                             errorTypeStr === ClassificationResult.CONFIG_FAILURE ||
                             errorTypeStr === 'CONFIG_FAILURE';
      if (context.requiresUserAction || isConfigFailure) {
        return true;
      }

      // ALLOWED: Invariant broken (CODE_BUG)
      const isCodeBug = errorTypeStr === FailureType.CODE_BUG ||
                       errorTypeStr === ClassificationResult.CODE_BUG ||
                       errorTypeStr === 'CODE_BUG';
      if (isCodeBug) {
        return true;
      }

      // ALLOWED: All models exhausted (final failure) - ONLY if executionPhase is 'final'
      if (context.executionPhase === 'final' && context.allModelsExhausted === true) {
        return true;
      }
      ...
      const isInfraFailure = errorTypeStr === FailureType.INFRA_FAILURE ||
                             errorTypeStr === ClassificationResult.INFRA_FAILURE ||
                             errorTypeStr === 'INFRA_FAILURE';
      const isModelFailure = errorTypeStr === FailureType.MODEL_FAILURE ||
      ...
      console.error('[AIBehaviorValidation] shouldShowNotification error (non-throwing):', error);
      return context.executionPhase === 'final' || context.allModelsExhausted === true;
    }
  }
  ...
  static verifyCompletion(
    ...
    if (response?.text) {
      ...
      if (validation.isValid) {
        ...
        failures.push(`Code syntax validation failed: ${validation.reason || 'Invalid syntax'}`);
      }
    }
    ...
    if (response?.text) {
      ...
      const codeOnlyLength = codeBlocks.reduce((sum, block) => sum + block.length, 0);
      const totalLength = text.length;
      // At least 20% of response should be non-code (explanation)
      const explanationRatio = (totalLength - codeOnlyLength) / totalLength;
    ...
    if (!error) {
      ...
      const isClassified = this.isConfigFailure(error) || 
  ...
  static getUserMessage(
    ...
    if (!ClassificationResult || typeof ClassificationResult !== 'object') {
      console.error('[AIBehaviorValidation] ClassificationResult not available at runtime');
      return 'An error occurred. Please try again.';
    }

    // Defensive check: Ensure FailureType is available at runtime
    if (!FailureType || typeof FailureType !== 'object') {
      console.error('[AIBehaviorValidation] FailureType not available at runtime');
      return 'An error occurred. Please try again.';
    }
    ...
    if (notificationContext.executionPhase === 'retry' || notificationContext.executionPhase === 'fallback') {
    ...
    try {
      ...
      console.warn('[AIBehaviorValidation] NotificationDeduplicator not available:', error);
    }

    // CONFIG_FAILURE: User action required (API key)
    if (classification === ClassificationResult.CONFIG_FAILURE || classification === 'CONFIG_FAILURE') {
      return 'API key is missing or invalid. Please configure your API key in Settings.';
    }

    // CODE_BUG: Invariant broken
    if (classification === ClassificationResult.CODE_BUG || classification === 'CODE_BUG') {
    ...
    if (classification === ClassificationResult.QUOTA_EXHAUSTED || classification === 'QUOTA_EXHAUSTED') {
    ...
    if (classification === ClassificationResult.INFRA_FAILURE && context?.errorMessage) {
    ...
    if (context) {
      ...
      if (errorMessage.includes('quota') && (errorMessage.includes('exhausted') || errorMessage.includes('exceeded'))) {
    ...
    if (allModelsExhausted) {
      ...
      console.warn('[AIBehaviorValidation] getUserMessage called with allModelsExhausted=true - this should be a FatalAIError');
