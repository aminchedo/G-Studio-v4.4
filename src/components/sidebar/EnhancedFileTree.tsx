...
export const EnhancedFileTree: React.FC<EnhancedFileTreeProps> = memo(
  ({
    ...
    const isDark =
      effectiveTheme === "dark" || effectiveTheme === "high-contrast";
    ...
    return (
      <div
        className={`flex flex-col h-full ${isDark ? "bg-slate-800" : "bg-white"}`}
      >
        {/* Header */}
        <div
        ...
        {/* Search */}
        <div
          className={`px-2 py-1 border-b ${isDark ? "border-slate-700" : "border-gray-200"}`}
        >
          <div
          ...
          >
            <Search
              className={`w-3 h-3 ${isDark ? "text-slate-400" : "text-gray-400"}`}
            />
            <input
              type="text"
              value={searchQuery}
