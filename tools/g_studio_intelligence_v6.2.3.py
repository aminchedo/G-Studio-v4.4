...
# =============================================================================
# FILE PARSER (with real tree‑sitter support – enhanced v6.2.0)
# =============================================================================
class FileParser:
...
def __init__(self, verbose: bool = False):
    ...
    def parse_file(self, file_path: Path, content: str) -> Dict[str, Any]:
        ...
        if self.ts_parser and self.tsx_parser:
            try:
                ...
                # NEW in v6.2.0: log warning and fall back to regex
                if self.verbose:
    ...
    # -------------------------------------------------------------------------
    # ENHANCED in v6.1.0 and v6.2.0: Much richer AST extraction
    # -------------------------------------------------------------------------
    def _parse_with_tree_sitter(self, content: str, parser: Any) -> Dict[str, Any]:
        """Parse using tree-sitter with precise AST traversal."""
        # NEW in v6.2.0: wrap entire function in try-except for robustness
        try:
                            ...
                            for spec in import_clause.children:
                                ...
                                if spec.type == 'identifier':
                                    imported_names.append({
                                        ...
                                        'alias': None,
                                        'type_only': False,
                                        # NEW in v6.2.0: additional fields
                                    ...
                                    for n in spec.children:
                                        if n.type == 'import_specifier':
                                            ...
                                            # NEW in v6.2.0: detect type imports (simple heuristic)
                                            node_text = get_node_text(n)
                                            is_type = 'type' in node_text and name_node and 'type' in content[name_node.start_byte-6:name_node.start_byte]  # rough check
                                            imported_names.append({
                                                'name': name,
                                                'alias': alias,
                                                'type_only': is_type,
            ...
            # NEW in v6.2.0: catch any AST parsing error and fall back to regex
            if self.verbose:
...
import {{ useGemini }} from '@/hooks/core/useGemini';""",
    ...
    def _check_context_usage(self, file_path: str, line_num: int, line: str) -> List[WiringIssue]:
        ...
        if match:
            ...
            # NEW in v6.2.0: skip whitelisted contexts
...
class ArchitecturalAnalyzer:
    ...
    # NEW in v6.2.0: small-cycle detection
    def _detect_cycles(self) -> Optional[ArchitecturalInsight]:
        """Detect cycles of length 2–5 in the dependency graph."""
        # Build reverse lookup: file_path -> FileInfo, but we already have self.files
        # We need the dependency graph. Since this is called after graph is built,
        # we can assume that the `depends_on` list in each FileInfo is populated.
        # We'll do a DFS from each node, limit depth to 5.
        ...
        def dfs(start: str, current: str, depth: int, path: List[str], visited: Set[str]) -> List[List[str]]:
            ...
            if current in path:
                ...
                if 2 <= len(cycle) <= 5:
                    # Normalise cycle representation (smallest index first)
            ...
            if file_info:
                for dep in file_info.depends_on:
                    if dep in self.files:  # ensure it's still in graph
        ...
        # Flatten all files involved in cycles
        affected_files = set()
        for cycle in all_cycles:
            affected_files.update(cycle)
        
        # Health penalty: 10 points per cycle, max -50
...
# =============================================================================
# HTML REPORT GENERATOR (enhanced with real tables & styling – v6.2.0)
# =============================================================================
class HTMLReportGenerator:
    ...
    def generate(result: AnalysisResult, output_path: Path, with_git: bool = False):
        ...
        # NEW in v6.2.0: compute average health score
        avg_health = 0.0
        if result.insights:
        ...
        <div class="content">
            ...
            <div class="section">
                ...
                <div class="stats-grid">
                    ...
                    <!-- NEW in v6.2.0: average health score card -->
...
# =============================================================================
# MAIN ANALYZER (with new features: content-based layer, changed-only, CSV export, min-score)
# =============================================================================
class GStudioAnalyzer:
    ...
    def __init__(self, project_path: Path, output_dir: Path, 
        ...
        self.min_score = min_score  # NEW in v6.2.0
        ...
        # NEW in v6.2.0: pass verbose flag to FileParser
    ...
    def _find_valuable_unused(self, unused_paths: List[str]):
        ...
        for path in unused_paths:
            ...
            # Only include if valuable (score > 30) and meets min_score threshold
            # NEW in v6.2.0: apply min_score filter
...
<{name} />"""
    ...
    def _detect_wiring_issues(self):
        ...
        # NEW in v6.2.0: pass verbose flag to WiringIssueDetector
...
Examples:
  ...
  %(prog)s . --min-score 50              # only show unused components with score >= 50
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Minimum value score for valuable unused components (default: 0, always keeps >30)'
    )
    ...
    try:
        ...
        analyzer = GStudioAnalyzer(
            ...
            csv_export=args.csv,
            min_score=args.min_score   # NEW in v6.2.0
        )
