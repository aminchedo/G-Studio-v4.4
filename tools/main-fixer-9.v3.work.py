...
def main():
    ...
    if args.non_interactive or args.analyze_only or args.json_output:
        ...
        try:
            postprocess_script = Path(__file__).resolve().parent / "report_postprocess.py"
            subprocess.run(
                [sys.executable, str(postprocess_script), str(platform.report_folder)],
                cwd=str(Path(project_path).resolve()),
                check=False
            )
        except Exception as e:
            if platform.logger:
                platform.logger.info(f"Postprocess failed: {e}")
    else:
