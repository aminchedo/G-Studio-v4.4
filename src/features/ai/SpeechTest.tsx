 ...
 */
export const SpeechTest: React.FC<SpeechTestProps> = () => {
  ...
  const startListening = () => {
    ...
    if (!SpeechRecognition) {
      addLog(
        "SpeechRecognition API not available in this environment",
        "error",
      );
      setStatus("error");
      return;
  ...
  const stopListening = () => {
    if (recognitionRef.current) {
      try {
        ...
        console.warn(
          "Speech recognition stop failed (may already be stopped):",
          e,
        );
      }
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 min-h-[120px] max-h-[200px] overflow-y-auto">
            {transcript ? (
              ...
              <p className="text-sm text-slate-400 italic">
                No transcript yet. Start listening to capture speech...
              </p>
            )}
          </div>
        </div>
