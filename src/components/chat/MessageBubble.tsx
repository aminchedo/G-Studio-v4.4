...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  return (
    <div
      className={`flex ${isUser ? "justify-end" : "justify-start chat-assistant-inward"} chat-message-gap group chat-message-bubble-enter`}
    >
      <div
        className={`flex gap-2 sm:gap-3 chat-bubble-row ${isUser ? "flex-row-reverse" : "flex-row chat-assistant-row"}`}
      >
        {/* Avatar - photo style: user = yellow tint, AI = light green (smaller for assistant) */}
        <div className="flex-shrink-0">
          {isUser ? (
            <div className="w-8 h-8 rounded-full bg-amber-200/90 flex items-center justify-center shadow-md border border-amber-300/30">
              <svg
            ...
            <div className="w-6 h-6 rounded-full bg-emerald-200/90 flex items-center justify-center shadow-md border border-emerald-300/30 chat-assistant-avatar">
              <svg
        ...
        <div className="flex-1 min-w-0">
          {/* Header + actions */}
          <div
            className={`flex items-center gap-2 mb-1 ${isUser ? "justify-end" : "justify-start"} ${isAI ? "chat-assistant-header" : ""}`}
          >
            <span
              className={
                isUser
            ...
            <span
              className={
                isUser ? "text-xs text-slate-500" : "text-[11px] text-slate-500"
              }
            >
              {formattedTime}
            </span>
          ...
          {/* Message Bubble - photo style: AI = green, User = yellow */}
          <div
            className={`relative rounded-2xl transition-shadow duration-200 chat-message-body ${
              isUser
                ? "px-4 py-3 bg-amber-400 text-slate-900 chat-bubble-user"
                : "px-3 py-2.5 rounded-xl bg-emerald-500 text-white chat-bubble-assistant chat-assistant-bubble"
          ...
          >
            {/* Regenerate icon on bubble - last assistant only */}
            {isLastAssistant && isAI && !message.isLoading && onRegenerate && (
              <button
                type="button"
                onClick={onRegenerate}
                className="absolute bottom-2 right-2 p-1.5 rounded-full bg-white/20 hover:bg-white/30 text-white/90 hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-white/40"
                title="Regenerate"
              >
                <svg
                  className="w-3.5 h-3.5"
            ...
            {message.isLoading ? (
              <div className="flex items-center gap-2">
                <div className="flex gap-1">
                  <div
                    className={`w-2 h-2 rounded-full chat-typing-dot ${isUser ? "bg-slate-600" : "bg-white/90"}`}
                ...
                <span
                  className={`text-sm ${isUser ? "text-slate-700" : "text-white/90"}`}
                >
                  AI is thinking...
                </span>
              </div>
              ...
              <div
                className={`prose prose-invert max-w-none chat-message-content text-left ${isUser ? "prose-sm text-xs" : "prose-sm chat-assistant-content text-[11px] leading-snug"}`}
                dir="ltr"
              >
                <ReactMarkdown
                  components={{
                    code(props: {
                      ...
                      const { node, className, children, inline } = props;
                      ...
                      return !inline && match ? (
                        <div className="relative group/code my-4 chat-code-block-wrap">
                          ...
                          {/* Code Block - scrollable when long */}
                          <SyntaxHighlighter
                            style={vscDarkPlus}
                            language={match[1]}
                            PreTag="div"
                            className="!mt-0 !mb-0 !rounded-xl !bg-slate-900/50 !border !border-white/10 chat-code-inner"
                            customStyle={{
                              margin: 0,
                              padding: "1rem",
                              paddingTop: "2.5rem",
