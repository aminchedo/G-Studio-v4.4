...
# Setup MCP servers (optional - skip if directory doesn't exist)
...
if [ -d "mcp-servers" ]; then
  ...
  if [ -f "install.sh" ]; then
    ...
    echo -e "${YELLOW}⚠️  install.sh not found in mcp-servers${NC}"
  fi
  cd ..
else
  echo -e "${YELLOW}⚠️  mcp-servers directory not found - skipping${NC}"
fi
echo ""

# Setup configuration
echo -e "${BLUE}[4/6]${NC} Setting up configuration..."
if [ -d "config" ] && [ -f "config/mcp-config.example.json" ] && [ ! -f "config/mcp-config.json" ]; then
  cp config/mcp-config.example.json config/mcp-config.json
  echo -e "${GREEN}✓${NC} Configuration file created"
elif [ -f "config/mcp-config.json" ]; then
  echo -e "${GREEN}✓${NC} Configuration file already exists"
else
  echo -e "${YELLOW}⚠️  config/mcp-config.example.json not found - skipping${NC}"
fi
echo ""
