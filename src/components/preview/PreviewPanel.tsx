...
interface PreviewPanelProps {
  ...
  activeFile?: string;
  onClose?: () => void;
  errors?: Array<{ message: string; line?: number; column?: number; timestamp?: Date | number }>;
  onErrorsChange?: (errors: Array<{ message: string; line?: number; column?: number; timestamp?: Date | number }>) => void;
}
...
<body>
  ...
  // Forward internal preview errors to parent if requested
  useEffect(() => {
    if (typeof onErrorsChange === 'function') {
      if (error) {
        onErrorsChange([{ message: error, timestamp: Date.now() }]);
      } else {
        onErrorsChange([]);
      }
    }
  }, [error, onErrorsChange]);
