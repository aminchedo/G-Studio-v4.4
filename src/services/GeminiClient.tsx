...
export class GeminiClient implements IGeminiClient {
  ...
  async sendRequest(request: GeminiRequest): Promise<GeminiResponse> {
    ...
    try {
      ...
      if (error instanceof Error && error.name === "AbortError") {
        throw new GeminiError("Request timeout", GeminiErrorCode.TIMEOUT);
      }
  ...
  private parseFinishReason(reason?: string): FinishReason {
    ...
    if (lowerReason.includes("length") || lowerReason.includes("max_tokens"))
      return "length";
  ...
  private async processStream(
    ...
    if (!reader) {
      throw new GeminiError("No response body", GeminiErrorCode.NETWORK_ERROR);
    }
