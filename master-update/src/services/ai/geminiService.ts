...
import { ModelId, Message, ToolCall } from "@/types/types";
import { FILE_TOOLS } from "@/config/constants";
...
import {
  ...
  NetworkErrorType
} from "../network/networkReliabilityService";

import { NetworkReliabilityVerification } from "../network/networkReliabilityVerification";
import { RuntimeGuardrails } from "../security/runtimeGuardrails";
...
import {
  ...
  FailureType,
  NotificationContext
} from "../security/aiBehaviorValidation";

import {
  ModelFallbackManager,
  FallbackState
} from "./modelFallbackManager";

import { CircuitBreaker } from "../network/circuitBreaker";
import { ProviderLimit, ProviderLimitResult } from "../network/providerLimit";
...
class RobustAPIClient {
  ...
  async request(endpoint: string, options: { 
    ...
    const finalErrorInfo = ErrorCategorizer.categorize(lastError || new Error('Max retries exceeded'));
...
export async function runActiveModelDiagnostic(
  apiKey: string
): Promise<{ ok: true; responseText: string; modelId: string } | { ok: false; error: string; modelId?: string }> {
  ...
  if (!activeModel) {
    return {
      ok: false,
      error: 'No active model found. Please run "API Model Test" first to discover models.',
    };
  }
...
export class GeminiService {
  /**
   * Compatibility wrapper: instance-based API for legacy tests and code
   * Provides: constructor(apiKey), generateContent(prompt), streamContent(prompt, onChunk), getTokenUsage()
   */
  private apiKeyInstance: string | null = null;
  private optionsInstance: any = {};
  private usageInstance = { prompt: 0, response: 0, total: 0 };

  constructor(apiKey?: string, options: any = {}) {
    if (apiKey === undefined || (typeof apiKey === 'string' && apiKey.trim() === '')) {
      // Preserve original behavior: throw when empty string provided
      if (typeof apiKey === 'string' && apiKey.trim() === '') {
        throw new Error('API key is required. Please set your Gemini API key.');
      }
      // Allow undefined for static-only usage
    }
    this.apiKeyInstance = apiKey || null;
    this.optionsInstance = options || {};
  }

  /**
   * Minimal non-streaming helper used by legacy tests
   */
  async generateContent(prompt: string): Promise<string> {
    if (!this.apiKeyInstance) throw new Error('API key is required. Please set your Gemini API key.');
    // Simple retry on 429 (single retry) to match tests
    const doFetch = async () => {
      ...
      if (data?.usageMetadata) {
        ...
        this.usageInstance.prompt = um.promptTokenCount || 0;
        this.usageInstance.response = um.candidatesTokenCount || 0;
        this.usageInstance.total = um.totalTokenCount || (this.usageInstance.prompt + this.usageInstance.response);
      }
      return text;
    };
    ...
    try {
      ...
      if (err && (err.status === 429 || (err.message || '').toLowerCase().includes('too many requests'))) {
        // retry once
        try {
          return await doFetch();
        } catch (err2) {
  ...
  /**
   * Minimal streaming helper used by legacy tests
   */
  async streamContent(prompt: string, onChunk: (chunk: string) => void): Promise<void> {
    ...
    if (body && typeof body.getReader === 'function') {
      ...
      while (true) {
        ...
        // Support simple SSE-style lines like: data: {"text":"Hello"}\n
        const lines = chunkStr.split(/\r?\n/).filter(Boolean);
        for (const line of lines) {
          const dataMatch = line.trim().replace(/^data:\s*/i, '');
          try {
            const obj = JSON.parse(dataMatch);
            if (obj && typeof obj.text === 'string') {
              onChunk(obj.text);
            } else if (obj && obj.candidates && obj.candidates[0] && obj.candidates[0].content && obj.candidates[0].content.parts) {
              const t = obj.candidates[0].content.parts.map((p: any) => p.text).join('');
              onChunk(t);
            }
          } catch {
            // Not JSON - pass raw chunk
            onChunk(line);
          }
        }
      }
    ...
    // Fallback: attempt to parse response.json() for non-stream responses
  ...
  getTokenUsage() {
    return { total: this.usageInstance.total, prompt: this.usageInstance.prompt, response: this.usageInstance.response };
  }
   ...
   * Cache TTL: 5 minutes
   */
  private static modelDiscoveryCache: Map<string, { models: string[], timestamp: number }> = new Map();
  ...
  private static async testModelResponse(
  ...
  ): Promise<{ success: boolean; failureReason?: 'quota_exhausted' | 'permission_denied' | 'model_disabled' | 'not_found' | 'network_error' | 'timeout' | 'unknown' }> {
    ...
    try {
      ...
      if (result.success && result.data?.candidates?.[0]?.content?.parts?.[0]?.text) {
        console.log(`[GeminiService] ✓ Model ${modelId} verified - responded successfully`);
        return { success: true };
      }

      // Use ErrorCategorizer for better error handling
      const errorInfo = result.errorInfo || ErrorCategorizer.categorize({
        status: result.status,
        message: result.error || 'Unknown error'
      });

      // Map error categories to failure reasons
      if (errorInfo.category === 'rate_limit' || errorInfo.category === 'authentication' && result.status === 429) {
        const errorMsg = (result.error || '').toLowerCase();
        const isPermanent = errorMsg.includes('limit: 0') || errorMsg.includes('limit = 0');
        
        if (isPermanent) {
          console.warn(`[GeminiService] ✗ Model ${modelId} has PERMANENT quota exhaustion (limit: 0) - TERMINAL`);
          return { success: false, failureReason: 'quota_exhausted' };
        } else {
          console.warn(`[GeminiService] ✗ Model ${modelId} has rate limit (429) - marking as unavailable`);
          return { success: false, failureReason: 'quota_exhausted' };
        }
      }

      if (errorInfo.category === 'authentication') {
        console.warn(`[GeminiService] ✗ Model ${modelId} permission denied - marking as unavailable`);
        return { success: false, failureReason: 'permission_denied' };
      }

      if (errorInfo.category === 'not_found') {
        console.warn(`[GeminiService] ✗ Model ${modelId} not found or disabled - marking as unavailable`);
        return { success: false, failureReason: 'model_disabled' };
      }

      if (errorInfo.category === 'network' || errorInfo.type === 'dns_network_error') {
        console.warn(`[GeminiService] ✗ Model ${modelId} network error: ${errorInfo.message}`);
        return { success: false, failureReason: 'network_error' };
      }

      // Timeout handling
      if (result.error?.includes('timeout') || result.error?.includes('AbortError')) {
        console.warn(`[GeminiService] ✗ Model ${modelId} test timeout - marking as failed`);
        return { success: false, failureReason: 'timeout' };
      }

      console.warn(`[GeminiService] ✗ Model ${modelId} test failed: ${errorInfo.message}`);
      return { success: false, failureReason: 'unknown' };
    } catch (error: any) {
      // Fallback error handling
      const errorInfo = ErrorCategorizer.categorize(error);
      
      if (error.name === 'AbortError' || errorInfo.category === 'network' && errorInfo.type === 'dns_network_error') {
        console.warn(`[GeminiService] ✗ Model ${modelId} test timeout - marking as failed`);
        return { success: false, failureReason: 'timeout' };
      } else if (errorInfo.category === 'network') {
        console.warn(`[GeminiService] ✗ Model ${modelId} network error: ${error.message}`);
        return { success: false, failureReason: 'network_error' };
      } else {
        console.warn(`[GeminiService] ✗ Model ${modelId} test error: ${error.message}`);
        return { success: false, failureReason: 'unknown' };
      }
    }
  }
   ...
   */
  static getModelConfig(modelId: string): { family: string; tier: string; capabilities: string[] } | null {
    const config = GEMINI_MODEL_CONFIG.MODELS[modelId as keyof typeof GEMINI_MODEL_CONFIG.MODELS];
    if (config) {
      return config;
    }
   ...
   */
  static getRecommendedModel(useCase: 'speed' | 'quality' | 'balance' | 'latest'): string | null {
    const key = `bestFor${useCase.charAt(0).toUpperCase() + useCase.slice(1)}` as keyof typeof GEMINI_MODEL_CONFIG.RECOMMENDATIONS;
  ...
  static async discoverAvailableModels(apiKey: string): Promise<string[]> {
    ...
    if (providerStatus === 'exhausted') {
      const { DegradedMode } = await import('../network/degradedMode');
      const { ProviderLimit } = await import('../network/providerLimit');
      if (!DegradedMode.isDegraded('gemini')) {
        ...
        console.warn(`[GeminiService] Provider exhausted - entering degraded mode`);
      }
      return [];
    }
    ...
    if (validatedModels.length > 0) {
      console.log(`[GeminiService] Using ${validatedModels.length} validated models from store:`, validatedModels);
      return validatedModels;
    }

    // No validated models - return empty array
    // Models must be tested via "API Model Test" button first
    console.warn(`[GeminiService] No validated models found. Please run "API Model Test" first.`);
    return [];
  }
  ...
  static async getModelDetails(
    ...
    try {
      ...
      return availableModels.map(modelId => {
        const model = data.models?.find((m: any) => m.name === `models/${modelId}`);
        return {
          id: modelId,
  ...
  private static formatHistory(
    ...
    const result = validMessages.map(msg => {
      ...
      if (!ALLOWED_ROLES.includes(role as "user" | "model")) {
        throw new Error(`CODE_BUG: Invalid role '${role}' in message history. Gemini API only accepts 'user' or 'model' roles. Forbidden roles: system, function, tool, assistant, and any unknown role.`);
      }
  ...
  static async *streamChat(
    ...
    requestId?: string
  ): AsyncGenerator<{ text?: string, toolCalls?: ToolCall[], usage?: UsageMetadata, providerLimit?: ProviderLimitResult }, void, unknown> {
    ...
    if (!requestId) {
      throw new Error('requestId is required. It must be generated in UI layer and passed explicitly through the pipeline.');
    }
    ...
    const { StreamingMonitor } = await import('../monitoring/streamingMonitor');
    ...
    if (StreamLifecycleManager.isInCooldown(apiKey || '')) {
      const remaining = StreamLifecycleManager.getRemainingCooldown(apiKey || '');
      console.log(`[GeminiService][requestId=${finalRequestId}]: Cooldown active - using non-streaming directly (HTML tester mode). Remaining: ${remaining}ms`);
    ...
    StreamingMonitor.trackRequestStart(finalRequestId, 'gemini-api-stream', 'POST');
    ...
    const releaseLock = await StreamLifecycleManager.acquireStreamLock(apiKey || '');
    
    // Register this stream (cancels any previous stream for this API key)
    const abortController = StreamLifecycleManager.registerStream(finalRequestId, apiKey || '');
    ...
    if (!apiKey || apiKey.trim() === '') {
      throw new Error('API key is required. Please set your Gemini API key in Agent Settings (Connection tab).');
    }

    // Check if API Model Test has been executed
    if (!ModelValidationStore.hasTestBeenExecuted(apiKey)) {
      console.error(`[GeminiService][requestId=${finalRequestId}]: API Model Test not executed`);
    ...
    if (usableModels.length === 0) {
      const providerStatus = ModelValidationStore.getProviderStatus(apiKey);
      if (providerStatus === 'exhausted') {
        console.error(`[GeminiService][requestId=${finalRequestId}]: Provider exhausted`);
        throw FatalAIError.PROVIDER_EXHAUSTED();
      }
      console.error(`[GeminiService][requestId=${finalRequestId}]: Zero usable models`);
      throw FatalAIError.ZERO_USABLE_MODELS();
    }

    // SINGLE SOURCE OF TRUTH: Get model from Model Ribbon selection
    const selectionResult = await ModelSelectionService.getValidatedModel(apiKey);
    
    if (!selectionResult.isValid) {
      const errorMsg = selectionResult.error || 'Selected model is no longer available. Please reselect a model.';
      console.error(`[GeminiService][requestId=${finalRequestId}]: Model selection invalid: ${errorMsg}`);
    ...
    if (!activeModelInfo) {
      console.error(`[GeminiService][requestId=${finalRequestId}]: Active model info missing`);
      throw new Error('Active model information is missing. Please run "API Model Test" first.');
    }
    ...
    if (activeModelInfo.id !== ribbonSelectedModel) {
      console.error(`[GeminiService][requestId=${finalRequestId}]: Model ID mismatch: activeModelInfo.id="${activeModelInfo.id}" vs ribbonSelectedModel="${ribbonSelectedModel}"`);
    ...
    if (!usableModels.includes(ribbonSelectedModel)) {
      console.error(`[GeminiService][requestId=${finalRequestId}]: Ribbon model "${ribbonSelectedModel}" not in usable models`);
      throw new Error(`Selected model "${ribbonSelectedModel}" is no longer available. Please reselect a model.`);
    }
    ...
    StreamingMonitor.registerStream(finalRequestId, currentModel, apiKey || '');
    
    console.log(`[GeminiService][requestId=${finalRequestId}]: Starting streamChat with ribbon-selected model`, { 
      ...
      if (useCache && newPrompt) {
        const cached = ResponseCache.getExactMatch(newPrompt, currentModel as any, systemInstruction);
        if (cached) {
          ...
          if (classification === ClassificationResult.MODEL_SUCCESS) {
            ...
            yield { 
              ...
              usage: cached.tokenUsage ? {
                ...
                ...({ candidatesTokenCount: cached.tokenUsage.response } as any),
                totalTokenCount: cached.tokenUsage.prompt + cached.tokenUsage.response
              } : undefined
            };
            return;
          }
      ...
      if (DegradedMode.isDegraded('gemini')) {
        console.log(`[GeminiService][requestId=${finalRequestId}][PROVIDER_LIMIT]: Already in degraded mode - skipping request`);
        const providerLimit = ProviderLimit.create('gemini', 30);
        yield { providerLimit };
        return;
      }
      ...
      let modelAttemptResponse: { text?: string; toolCalls?: ToolCall[]; usage?: UsageMetadata } | null = null;
      ...
      const currentModelInfo = validatedModelInfos.find(m => m.id === currentModel);
      if (currentModelInfo) {
      ...
      while (attemptNumber <= maxAttempts) {
        console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][attempt=${attemptNumber}]: Starting request with ribbon-selected model`);
        try {
          ...
          if (!preflight.isValid) {
            ...
            console.error(`[GeminiService][requestId=${finalRequestId}][PREFLIGHT_FAILED]: CODE_BUG detected`, {
            ...
            const codeBugError = new Error(`Pre-flight validation failed: ${preflight.reason || 'Unknown CODE_BUG'}`);
            try {
              const { CompletionReporter } = await import('../monitoring/completionReporter');
            ...
            const completionVerification = AIBehaviorValidation.verifyCompletion(null, codeBugError, finalRequestId);
            const systemStatus = {
              ui: 'OK' as const,
              ai: 'BLOCKED' as const,
              infra: 'OK' as const
            ...
            try {
              ...
              if (process.env.NODE_ENV === 'development') {
                console.error('[CompletionReporter] CODE_BUG report:', CompletionReporter.formatReportAsMarkdown(report));
              }
            } catch {
              // Silently fail if reporter not available
            }
            ...
            try {
              const runtimeUIVerification = await import('../runtimeUIVerification');
              if (runtimeUIVerification && typeof (runtimeUIVerification as any).markCodeBug === 'function') {
            ...
            const userMessage = AIBehaviorValidation.getUserMessage(ClassificationResult.CODE_BUG, false, context);
            throw new Error(userMessage || `Internal error: ${preflight.reason || 'Code bug detected'}`);
          }
          // ==================== END PRE-FLIGHT VALIDATION ====================

          if (!useStreaming) {
            // Fallback to non-streaming mode
            console.log(`[NETWORK][requestId=${finalRequestId}] [FALLBACK_APPLIED] Using non-streaming fallback`, { 
              attemptNumber, 
              timeout,
            ...
            if (classification === ClassificationResult.MODEL_SUCCESS) {
              ...
              if (useCache && newPrompt && result.text) {
                ResponseCache.set(
                  ...
                  result.usage ? {
                    ...
                    response: (result.usage as any).candidatesTokenCount || 0
                  } : undefined,
                  systemInstruction
                );
              }
              ...
              try {
                if (result.usage) {
                  recordMetric('tokens_used', result.usage.totalTokenCount || 0, { model: currentModel });
                  recordMetric('tokens_prompt', result.usage.promptTokenCount || 0, { model: currentModel });
                  recordMetric('tokens_completion', (result.usage as any).candidatesTokenCount || 0, { model: currentModel });
                }
                const latency = Date.now() - startTime;
              ...
              AIBehaviorValidation.classifyResponse(result, null, finalRequestId, currentModel as ModelId, attemptNumber);
          ...
          let contents = this.formatHistory(history, newPrompt, currentModel as ModelId, useMinimalContext);

          // RUNTIME GUARD: Ensure contents is always an array
          if (!Array.isArray(contents)) {
            console.warn(`[GeminiService][requestId=${requestId}] formatHistory returned non-array, normalizing:`, typeof contents);
            contents = Array.isArray(contents) ? contents : [];
          }

          // RUNTIME ASSERTION: Verify contents structure (NON-REMOVABLE)
          RuntimeGuardrails.guardGeminiServiceInput(contents, finalRequestId, 'stream');
          ...
          if (newPrompt) {
            ...
            if (!newPrompt || typeof newPrompt !== 'string') {
              throw new Error(`Invalid prompt payload: expected string, got ${typeof newPrompt}. requestId=${finalRequestId}`);
            }
          ...
          const hasText = contents.some(c => c.parts?.some((p: any) => p.text));
          const hasImage = contents.some(c => c.parts?.some((p: any) => p.inlineData));
          if (!hasText && !hasImage) {
            throw new Error(`Invalid payload: No text or image parts found. requestId=${finalRequestId}`);
          }
          ...
          console.debug('[GenConfig]', {
            ...
            hasText,
            hasImage,
            partsCount: contents.reduce((sum, c) => sum + (c.parts?.length || 0), 0),
            requestId: finalRequestId
          });
          ...
          if (StreamLifecycleManager.isInCooldown(apiKey || '')) {
            const remaining = StreamLifecycleManager.getRemainingCooldown(apiKey || '');
            console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}]: In cooldown, skipping API call. Remaining: ${remaining}ms`);
            
            // Record cooldown status with real-time monitor
            StreamingMonitor.recordCooldown(apiKey || '', remaining);
            
            modelAttemptError = new Error(`Rate limited. Please wait ${Math.ceil(remaining / 1000)} seconds before trying again.`);
          ...
          const retryDelay = attemptNumber > 1 ? Math.min(1000 * (attemptNumber - 1), 2000) : 0; // Additional delay for retries
          const totalDelay = baseDelay + retryDelay;
          
          console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][attempt=${attemptNumber}]: Adding ${totalDelay}ms delay before API call to respect rate limits (base: ${baseDelay}ms, retry: ${retryDelay}ms)`);
          ...
          try {
            responseStream = await ai.models.generateContentStream({
              ...
              config: {
                ...
                ...(isGemini3 ? {
                  thinkingConfig: {
                    thinkingBudget: currentModel.includes('pro') ? 32768 : 16384
                  }
                } : {})
              }
            });
            ...
            const httpStatus = AIBehaviorValidation.getHttpStatusCode(streamInitError);
            const is429 = errorCode === 429 || streamInitError?.status === 429 || httpStatus === 429 || errorMsg.includes('429') || errorMsg.includes('too many requests');
            ...
            if (is429) {
              ...
              console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][429]: Rate limit exceeded during stream init - entering cooldown and disabling streaming`);
              
              // Get cooldown duration before handling
              const cooldownBefore = StreamLifecycleManager.getRemainingCooldown(apiKey || '');
              StreamLifecycleManager.handle429(apiKey || '');
              const cooldownAfter = StreamLifecycleManager.getRemainingCooldown(apiKey || '');
              
              // Record 429 event with real-time monitor
              StreamingMonitor.record429(finalRequestId, apiKey || '', cooldownAfter);
              StreamingMonitor.trackRequestEnd(finalRequestId, 429, 'Too Many Requests', 'Rate limit exceeded');
              ...
              console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][429]: Switching to non-streaming fallback`);
              useStreaming = false;
              attemptNumber++;
            ...
            if (AIBehaviorValidation.isPermanentQuotaExhaustion(streamInitError)) {
              console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][PROVIDER_LIMIT]: Quota exhaustion during stream init - entering degraded mode`);
              // Track model unavailability (for telemetry only - no model switching)
              ModelFallbackManager.markModelUnavailable({} as any, currentModel as ModelId, 'QUOTA_EXHAUSTED');
              const circuitKey = `model:${currentModel}:${finalApiKey.substring(0, 8)}`;
          ...
          const emptyResponse = ChaosTesting.injectEmptyResponse(finalRequestId);
          if (emptyResponse) {
            console.warn(`[GeminiService][requestId=${finalRequestId}][CHAOS]: Empty response injected`);
            ...
            if (!guardResult.isValid) {
              ...
              console.log(`[GeminiService][requestId=${finalRequestId}][FALLBACK]: Empty response detected, switching to non-streaming`);
              useStreaming = false;
              attemptNumber++;
              continue; // Retry with non-streaming
            }
          ...
          try {
            ...
            if (streamAborted || ModelFallbackManager.hasQuotaExhaustedModels(state)) {
              break; // Exit immediately if quota exhausted
            }
            
            // CRITICAL: Check if stream was aborted by lifecycle manager
            if (abortController?.signal?.aborted) {
              console.log(`[GeminiService][requestId=${finalRequestId}]: Stream aborted by lifecycle manager before processing`);
              break;
            }
            ...
            for await (const chunk of responseStream) {
              ...
              if (abortController?.signal?.aborted) {
                console.log(`[GeminiService][requestId=${finalRequestId}]: Stream aborted during processing`);
                break;
              }
              ...
              const malformedChunk = ChaosTesting.injectMalformedChunk(finalRequestId, chunkIndex);
              const c = malformedChunk || (chunk as GenerateContentResponse);
              
              // RUNTIME ASSERTION: Validate chunk structure
              if (!c || typeof c !== 'object') {
                console.error(`[GeminiService][requestId=${finalRequestId}]: Invalid chunk received`, { chunkType: typeof c, chunkIndex });
                throw new Error('Invalid response chunk received. Please try again.');
              }

              // CHAOS TESTING: Inject partial network drop (DEV-ONLY)
              const networkDropError = ChaosTesting.injectPartialNetworkDrop(finalRequestId, bytesReceived);
              if (networkDropError) {
                console.warn(`[GeminiService][requestId=${finalRequestId}][CHAOS]: Partial network drop injected`);
              ...
              if (c.functionCalls) {
                if (!Array.isArray(c.functionCalls)) {
                  console.error(`[GeminiService][requestId=${finalRequestId}]: Invalid functionCalls type`, { chunkIndex });
                  throw new Error('Invalid response format. Please try again.');
                }
                for (const fc of c.functionCalls) {
                  if (!fc || typeof fc !== 'object' || !fc.name) {
                    console.error(`[GeminiService][requestId=${finalRequestId}]: Invalid function call`, { chunkIndex, fc });
              ...
              if (typeof text !== 'string') {
                console.error(`[GeminiService][requestId=${finalRequestId}]: Invalid text type`, { chunkIndex, textType: typeof text, chunk: c });
              ...
              const normalizedText = typeof text === 'string' 
                ? text 
                : (textObj?.text || textObj?.content || textObj?.delta || String(text || ''));
              ...
              StreamingMonitor.recordChunk(finalRequestId, normalizedText.length, normalizedText);
              ...
              if (normalizedText || allToolCalls.length > 0 || finalUsage) {
                ...
                const guardResult = RuntimeGuardrails.guardGeminiServiceResponse(responseChunk, finalRequestId, 'stream');
                if (!guardResult.isValid) {
                  console.error(`[GeminiService][requestId=${finalRequestId}]: Invalid response chunk`, { chunkIndex, error: guardResult.error });
            ...
            const httpStatus = AIBehaviorValidation.getHttpStatusCode(streamError);
            const is429 = errorCode === 429 || streamError?.status === 429 || httpStatus === 429 || errorMsg.includes('429') || errorMsg.includes('too many requests');
            
            if (is429) {
              // CRITICAL: 429 = RESOURCE_EXHAUSTED = "You've exceeded the rate limit"
              // According to Google docs: Verify you're within the model's rate limit
              // Enter cooldown with exponential backoff
              console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][429]: Rate limit exceeded during stream processing - entering cooldown and disabling streaming`);
              ...
              const currentModelApiName = this.modelIdToApiName(currentModel as ModelId);
              if (ModelValidationStore.isModelRejected(finalApiKey, currentModelApiName)) {
                const reason = ModelValidationStore.getRejectionReason(finalApiKey, currentModelApiName) || 'quota_exhausted';
                console.error(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}]: 429 on model rejected during test - TERMINAL`);
                const { FatalAIError } = await import('../fatalAIError');
                throw FatalAIError.MODEL_REJECTED_DURING_TEST(currentModelApiName, reason);
              }
            ...
            if (AIBehaviorValidation.isPermanentQuotaExhaustion(streamError)) {
              console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][PROVIDER_LIMIT]: Quota exhaustion in stream - entering degraded mode`);
              // Track model unavailability (for telemetry only - no model switching)
              ModelFallbackManager.markModelUnavailable({} as any, currentModel as ModelId, 'QUOTA_EXHAUSTED');
              const circuitKey = `model:${currentModel}:${finalApiKey.substring(0, 8)}`;
              CircuitBreaker.recordFailure(circuitKey, true);
            ...
            if (AIBehaviorValidation.isCorsError(streamError)) {
              console.error(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][CORS]: CORS error detected - TERMINAL`);
              const circuitKey = `model:${currentModel}:${finalApiKey.substring(0, 8)}`;
              CircuitBreaker.recordFailure(circuitKey, true);
              modelAttemptError = new Error('Network connectivity issue detected. This may be due to CORS restrictions or network configuration.');
            ...
            if (streamError.code === 'ECONNRESET' || streamError.message?.includes('reset') || 
                streamError.message?.includes('abort') || streamError.message?.includes('timeout')) {
              console.warn(`[GeminiService][requestId=${finalRequestId}][STREAM_ABORTED]: Partial completion detected`, {
                bytesReceived,
                chunkIndex,
              ...
              if (classification === ClassificationResult.MODEL_SUCCESS && fullResponse.length > 0) {
              ...
              if (AIBehaviorValidation.isInfraFailure(streamError) && 
                  !AIBehaviorValidation.isPermanentQuotaExhaustion(streamError)) {
                console.log(`[GeminiService][requestId=${finalRequestId}][FALLBACK]: Switching to non-streaming after stream abort`);
                useStreaming = false;
                attemptNumber++;
            ...
            // Normalize to PROVIDER_LIMIT (non-exceptional state)
            if (AIBehaviorValidation.isPermanentQuotaExhaustion(streamError)) {
              console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][PROVIDER_LIMIT]: Quota exhaustion in stream error - entering degraded mode`);
              // Track model unavailability (for telemetry only - no model switching)
              ModelFallbackManager.markModelUnavailable({} as any, currentModel as ModelId, 'QUOTA_EXHAUSTED');
              const circuitKey = `model:${currentModel}:${finalApiKey.substring(0, 8)}`;
              CircuitBreaker.recordFailure(circuitKey, true);
            ...
            if (classification === ClassificationResult.INFRA_FAILURE && attemptNumber < maxAttempts) {
          ...
          if (!finalResponse.text && !finalResponse.toolCalls?.length) {
            console.error(`[GeminiService][requestId=${finalRequestId}]: Empty final response`, {
              textLength: finalResponse.text?.length || 0,
              toolCallsCount: finalResponse.toolCalls?.length || 0,
              model: currentModel
            });
            throw new Error('Empty model response: No text or tool calls received.');
          }
          
          const guardResult = RuntimeGuardrails.guardGeminiServiceResponse(finalResponse, finalRequestId, 'stream');
          if (!guardResult.isValid) {
            console.error(`[GeminiService][requestId=${finalRequestId}]: Final response validation failed`, { error: guardResult.error });
            ...
            if (classification === ClassificationResult.MODEL_FAILURE) {
              modelAttemptError = new Error(guardResult.error || 'Empty response received');
          ...
          if (classification === ClassificationResult.MODEL_SUCCESS) {
            ...
            try {
              if (finalUsage) {
                recordMetric('tokens_used', finalUsage.totalTokenCount || 0, { model: currentModel });
                recordMetric('tokens_prompt', finalUsage.promptTokenCount || 0, { model: currentModel });
                recordMetric('tokens_completion', (finalUsage as any).candidatesTokenCount || 0, { model: currentModel });
          ...
          if (AIBehaviorValidation.isPermanentQuotaExhaustion(error)) {
            console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][PROVIDER_LIMIT]: Permanent quota exhaustion detected - entering degraded mode`);
            ...
            state = ModelFallbackManager.markModelUnavailable(state, currentModel as ModelId, 'QUOTA_EXHAUSTED');
            const circuitKey = `model:${currentModel}:${finalApiKey.substring(0, 8)}`;
            CircuitBreaker.recordFailure(circuitKey, true);
            const providerLimit = ProviderLimit.create('gemini', 30);
            DegradedMode.enterDegradedMode('gemini', providerLimit.cooldownUntil!);
          ...
          if (httpStatus === 429) {
            ...
            modelAttemptError = new Error('Rate limited. Please wait a few seconds before trying again.');
          ...
          if (AIBehaviorValidation.isInvalidRoleError(error)) {
            console.error(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][CODE_BUG]: Invalid role detected - should have been caught in pre-flight`);
            modelAttemptError = new Error('CODE_BUG: Invalid role in request. This should have been caught in pre-flight validation.');
            break; // Exit immediately - no retry, no fallback
          }

          // CRITICAL: Check for 400 INVALID_ARGUMENT - NOT retriable, NOT fallback
          if (AIBehaviorValidation.is400InvalidArgument(error)) {
            console.error(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][400_INVALID_ARGUMENT]: 400 error detected - no retry, no fallback`);
            modelAttemptError = new Error('Invalid request argument. Please check your request format.');
            break; // Exit immediately - no retry, no fallback
          }

          // CRITICAL: Log error details before classification for debugging
          console.error(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][ERROR_DETAILS]:`, {
          ...
          if (failureType === FailureType.CODE_BUG) {
            ...
            const userMessage = AIBehaviorValidation.getUserMessage(classification, false, context);
            modelAttemptError = new Error(userMessage || 'Internal error detected');
            break; // Exit immediately - no retry, no fallback
          }

          // CONFIG_FAILURE: Don't retry, fail immediately with user message
          if (failureType !== null && failureType === FailureType.CONFIG_FAILURE) {
            const context: NotificationContext = {
          ...
          const isQuotaExhausted = AIBehaviorValidation.isPermanentQuotaExhaustion(error);
          ...
          const isConfigFailure = classification === ClassificationResult.CONFIG_FAILURE;
          ...
          if (isQuotaExhausted || isCodeBug || isConfigFailure || is400Error || isInvalidRole) {
          ...
          const isTransientInfraFailure = failureType === FailureType.INFRA_FAILURE &&
                                          (httpStatus === 500 || httpStatus === 502 || httpStatus === 503 || httpStatus === 504 ||
                                           (httpStatus === null && (error.message.includes('500') || error.message.includes('502') || error.message.includes('503') || error.message.includes('504'))));
          ...
          const shouldRetry = attemptNumber < maxAttempts && isTransientInfraFailure;
          if (shouldRetry) {
            ...
            console.log(`[GeminiService][requestId=${finalRequestId}][model=${currentModel}][INFRA_RETRY]: Retrying same model with ${backoffMs}ms backoff`, {
              attemptNumber,
              backoffMs,
            ...
            attemptNumber++;
            useStreaming = NetworkReliabilityService.determineFallback(networkError, {
              apiKeyValidated,
              requestType: useStreaming ? 'streaming' : 'non-streaming',
              attemptNumber
            }).useStreaming;
            timeout = NetworkReliabilityService.determineFallback(networkError, {
              apiKeyValidated,
              requestType: useStreaming ? 'streaming' : 'non-streaming',
              attemptNumber
            }).timeout;
      ...
      if (attemptNumber >= maxAttempts) {
        ...
        const failedModelInfo = validatedModelInfos.find(m => m.id === currentModel);
        if (failedModelInfo) {
        ...
        const lastErrorIsQuota = modelAttemptError && AIBehaviorValidation.isPermanentQuotaExhaustion(modelAttemptError);
      ...
      if (error.message && !error.message.includes('Your API key is valid')) {
        ...
        if (error.message.includes('thought_signature') || error.message.includes('thought-signature')) {
          throw new Error('خطا در فراخوانی تابع: امضای تفکر (thought signature) مورد نیاز است.\n\nاین خطا برای مدل‌های Gemini 3.0 با ابزارها رخ می‌دهد. لطفاً از مدل دیگری استفاده کنید یا با پشتیبانی تماس بگیرید.');
        }
      }
  ...
  static async chatNonStreaming(
    ...
    requestId?: string
  ): Promise<{ text: string; toolCalls?: ToolCall[]; usage?: UsageMetadata; providerLimit?: ProviderLimitResult }> {
    const startTime = Date.now();
    // CRITICAL: requestId MUST be provided - never generate as fallback
    if (!requestId) {
      throw new Error('requestId is required. It must be generated in UI layer and passed explicitly through the pipeline.');
    ...
    const finalRequestId = requestId;
    console.log(`[GeminiService][requestId=${finalRequestId}]: Starting chatNonStreaming`, { modelId, timeout });
    ...
    // Check if API Model Test has been executed
    if (!ModelValidationStore.hasTestBeenExecuted(apiKey)) {
      console.error(`[GeminiService][requestId=${finalRequestId}]: API Model Test not executed (non-streaming)`);
      throw FatalAIError.API_TEST_NOT_EXECUTED();
    }
    ...
    console.log(`[GeminiService][requestId=${finalRequestId}][DEBUG_MODELS_NON_STREAMING]:`, {
      usableModelsCount: usableModels.length,
      usableModels: usableModels,
      hasTestBeenExecuted: ModelValidationStore.hasTestBeenExecuted(apiKey),
      providerStatus: ModelValidationStore.getProviderStatus(apiKey),
    ...
    if (usableModels.length === 0) {
      const providerStatus = ModelValidationStore.getProviderStatus(apiKey);
      if (providerStatus === 'exhausted') {
        console.error(`[GeminiService][requestId=${finalRequestId}]: Provider exhausted (non-streaming)`);
        throw FatalAIError.PROVIDER_EXHAUSTED();
      }
      console.error(`[GeminiService][requestId=${finalRequestId}]: Zero usable models (non-streaming)`);
      throw FatalAIError.ZERO_USABLE_MODELS();
    }
    ...
    if (!usableModels.includes(requestedModelApiName)) {
      ...
      if (ModelValidationStore.isModelRejected(apiKey, requestedModelApiName)) {
        const reason = ModelValidationStore.getRejectionReason(apiKey, requestedModelApiName) || 'unknown';
        console.error(`[GeminiService][requestId=${finalRequestId}]: Model ${requestedModelApiName} was rejected during test (non-streaming): ${reason}`);
        throw FatalAIError.MODEL_REJECTED_DURING_TEST(requestedModelApiName, reason);
      } else {
        console.error(`[GeminiService][requestId=${finalRequestId}]: Model ${requestedModelApiName} not in usable models (non-streaming)`);
        throw FatalAIError.MODEL_NOT_ALLOWED_BY_API_TEST(requestedModelApiName, 'Model was not validated during API Model Test');
      }
    }
    ...
    try {
      ...
      if (!preflight.isValid) {
        // CODE_BUG detected - abort immediately
        console.error(`[GeminiService][requestId=${finalRequestId}][PREFLIGHT_FAILED]: CODE_BUG detected in chatNonStreaming`, {
          reason: preflight.reason,
          model: modelId
        });
      ...
      console.log(`[GeminiService][requestId=${finalRequestId}][NON_STREAMING]: Using Smart Layer for API request with bypass/DNS`);
      ...
      // RUNTIME GUARD: Ensure contents is always an array
      if (!Array.isArray(contents)) {
        console.warn(`[GeminiService][requestId=${finalRequestId}] formatHistory returned non-array in non-streaming, normalizing:`, typeof contents);
        contents = Array.isArray(contents) ? contents : [];
      }

      // RUNTIME ASSERTION: Verify contents structure (NON-REMOVABLE)
      RuntimeGuardrails.guardGeminiServiceInput(contents, finalRequestId, 'non-stream');
      ...
      const { ModelValidationStore } = await import('./modelValidationStore');
      const validatedModelInfos = ModelValidationStore.getValidatedModelInfos(finalApiKey);
      const currentModelInfo = validatedModelInfos.find(m => m.id === modelId);
      ...
      if (FILE_TOOLS.length > 0 || systemInstruction) {
        (requestBody as any).config = {
          ...(FILE_TOOLS.length > 0 ? { tools: [{ functionDeclarations: FILE_TOOLS }] } : {}),
          ...(systemInstruction ? { systemInstruction } : {}),
          generationConfig: generationConfig,
          ...(isGemini3 ? {
            thinkingConfig: {
      ...
      const responseText = c?.candidates?.[0]?.content?.parts
        ?.map((p: any) => p.text)
        ?.filter((t: any) => t)
        ?.join('') || '';
      
      if (!responseText && !c?.functionCalls?.length) {
        console.error(`[GeminiService][requestId=${finalRequestId}]: Empty model response`, {
        ...
        throw new Error('Empty model response received. The model returned no text or function calls.');
      }
      
      // RUNTIME ASSERTION: Validate response structure
      if (!c || typeof c !== 'object') {
        console.error(`[GeminiService][requestId=${finalRequestId}]: Invalid response object`, { responseType: typeof c });
      ...
      if (c.functionCalls) {
        if (!Array.isArray(c.functionCalls)) {
          console.error(`[GeminiService][requestId=${finalRequestId}]: Invalid functionCalls type`, { functionCallsType: typeof c.functionCalls });
          throw new Error('Invalid response format. Please try again.');
        }
        for (const fc of c.functionCalls) {
          if (!fc || typeof fc !== 'object' || !fc.name) {
            console.error(`[GeminiService][requestId=${finalRequestId}]: Invalid function call`, { fc });
            continue; // Skip invalid function call
          }
          toolCalls.push({
            id: Math.random().toString(36).substring(7),
      ...
      const guardResult = RuntimeGuardrails.guardGeminiServiceResponse(result, finalRequestId, 'non-stream');
      if (!guardResult.isValid) {
        console.error(`[GeminiService][requestId=${finalRequestId}]: Final response validation failed`, { error: guardResult.error });
        throw new Error(guardResult.error || 'Empty response received. The AI did not return any content. Please try again.');
      }
      ...
      // Normalize to PROVIDER_LIMIT (non-exceptional state)
      if (AIBehaviorValidation.isPermanentQuotaExhaustion(error)) {
        console.log(`[GeminiService][requestId=${finalRequestId}][model=${modelId}][PROVIDER_LIMIT]: Permanent quota exhaustion detected in non-streaming - entering degraded mode`);
      ...
      // CRITICAL: Check for invalid role error (CODE_BUG)
      if (AIBehaviorValidation.isInvalidRoleError(error)) {
        console.error(`[GeminiService][requestId=${finalRequestId}][model=${modelId}][CODE_BUG]: Invalid role detected in non-streaming - should have been caught in pre-flight`);;
        throw new Error('CODE_BUG: Invalid role in request. This should have been caught in pre-flight validation.');
      }

      // CRITICAL: Check for 400 INVALID_ARGUMENT - NOT retriable, NOT fallback
      if (AIBehaviorValidation.is400InvalidArgument(error)) {
        console.error(`[GeminiService][requestId=${finalRequestId}][model=${modelId}][400_INVALID_ARGUMENT]: 400 error detected in non-streaming - no retry, no fallback`);
        throw new Error('Invalid request argument. Please check your request format.');
      }
  ...
  static async validateApiKey(key: string): Promise<boolean> {
    ...
    if (ModelTestingService.isTestInProgress(key)) {
      console.log('[GeminiService] validateApiKey: Test in progress - using ModelValidationStore results');
  ...
  private static normalizeContentToText(content: any): string {
    ...
    if (content && typeof content === 'object') {
      ...
      const textFields = Object.values(content).filter(v => typeof v === 'string');
  ...
  private static async sendRequestToGemini(
    ...
    return await client.request(endpoint, {
      ...
      maxRetries: options.maxRetries || GEMINI_MODEL_CONFIG.GEMINI_API.maxRetries,
      body: options.body,
  ...
  static async checkQuota(apiKey?: string): Promise<{
    ...
    try {
      ...
      if (!finalApiKey || finalApiKey.trim() === '') {
        return {
          isValid: false,
          message: 'API key is not set. Please set your Gemini API key in Agent Settings.',
          quotaLimits: {
            requestsPerDay: 1500,
      ...
      const { ModelValidationStore } = await import('./modelValidationStore');
      const validatedModels = ModelValidationStore.getValidatedModels(finalApiKey);
      
      if (validatedModels.length === 0) {
        // No validated models - return default limits
        return {
          isValid: false,
          message: 'No validated models available. Please run "API Model Test" first.',
      ...
      const result = await this.sendRequestToGemini(finalApiKey, `models/${quotaCheckModel}:generateContent`, {
        method: 'POST',
        body: {
      ...
      if (result.success && result.data) {
        const usage = result.data.usageMetadata as UsageMetadata & { candidatesTokenCount?: number };
        
        return {
          isValid: true,
          message: 'API is working correctly. This test request used tokens. Check Google Cloud Console for detailed quota usage.',
          usageInfo: usage ? {
        ...
        return {
          isValid: false,
          message: errorInfo.message || 'Failed to check API quota. Please verify your API key.',
          quotaLimits: defaultQuotaLimits
        };
      }
    } catch (error: any) {
