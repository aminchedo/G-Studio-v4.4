#!/usr/bin/env python3
"""
G-Studio Enterprise Code Intelligence Platform v7.4.0
======================================================
...
Adds optional production‑ready features:
  ...
  • Advanced Potentially‑Unused File Intelligence Engine (v7.4.0)

All new features are disabled by default – activate with dedicated flags.

CHANGES IN v7.4.0:
-------------------
• Added multi‑signal unused file detection with confidence scoring.
• Files are classified as DEAD_CODE, ORPHANED_FEATURE, HIGH_VALUE_UNWIRED,
  LEGACY_LEFTOVER, or ARCHIVE_CANDIDATE.
• Deletion simulation ensures safe removal suggestions.
• Structural cross‑check suggests merge candidates for similar active files.
• New CLI flags: --unused-scan, --fast-unused-scan, --dry-run-unused,
  --unused-threshold.
• Extended AnalysisResult with potentially_unused_files and summary stats.
• All existing features and cache remain fully compatible.
...
# =============================================================================
# CONFIGURATION – Merged from v7 and v6, extended with unused detection
# =============================================================================
class Config:
    ...
    # ---------- v7.4.0 unused detection ----------
    UNUSED_CONFIDENCE_THRESHOLD = 70   # default threshold for flagging
    # Weights for each signal (positive = contributes to "unused" confidence)
    UNUSED_SIGNAL_WEIGHTS = {
        ...
        'has_exports': -10,               # exports are good
        'complexity_high': 10,             # complexity > threshold
        'lines_gt_min': 5,                 # lines > MIN_LINES_FOR_UNWIRED
        'not_test': 5,
        'not_config': 5,
        'not_framework_reserved': 5,
        'git_old': 15,                      # last commit > 180 days
        'git_few_commits': 10,               # commit count <= 2
        'structural_similarity_to_active': -15,  # similar to active file
        'value_score_low': 20,                # value_score < VALUE_MIN_SCORE
        'has_side_effects': -20,               # side effects make deletion risky
    }
...
# =============================================================================
# ENUMS – Unified from both versions, extended for unused detection
# =============================================================================
...
class PatternType(Enum):
    ...
    UNUSED_ARCHITECTURAL_FRAGMENT = "unused_architectural_fragment"  # v7.4.0
...
# =============================================================================
# DATA CLASSES – Merged from v7 and v6, extended for unused detection
# =============================================================================
@dataclass
...
# ---------- v7.4.0 Unused file detail ----------
@dataclass
class UnusedFileDetail:
    """Detailed information about a potentially unused file."""
    path: str
    confidence: float
    classification: UnusedClassification
    recommended_action: RecommendedAction
    merge_target: Optional[str] = None          # if MERGE_WITH
    signals: Dict[str, bool] = field(default_factory=dict)   # which signals triggered
    reasons: List[str] = field(default_factory=list)
    estimated_savings_lines: int = 0
    estimated_complexity_reduction: int = 0
    deletion_safe: bool = False                  # result of simulation
...
class AnalysisResult:
    ...
    # ---------- v7.4.0 unused intelligence ----------
    potentially_unused_files: List[UnusedFileDetail] = field(default_factory=list)
    unused_confidence_summary: Dict[str, float] = field(default_factory=dict)  # e.g., avg confidence
...
class CacheManager:
    ...
    def __init__(self, cache_path: Path, use_cache: bool = True, version: str = "7.4.0"):
...
class HTMLReportGenerator:
    ...
    def generate(result: AnalysisResult, output_path: Path, with_git: bool = False):
        ...
        # v7.4.0: potentially unused files section
        if result.potentially_unused_files:
            extra_sections += HTMLReportGenerator._build_unused_section(result)
...
<head>
    ...
    <title>G-Studio Code Analysis Report v7.4</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🎯 G-Studio Enterprise Code Analysis v7.4</h1>
...
"""
        ...
        <div class="footer">
            Generated by G-Studio Enterprise Code Intelligence v7.4<br>
...
<div class="section">
    ...
    @staticmethod
    def _build_unused_section(result: AnalysisResult) -> str:
        rows = ""
        for u in sorted(result.potentially_unused_files, key=lambda x: x.confidence, reverse=True)[:50]:
            merge_info = f" → {u.merge_target}" if u.recommended_action == RecommendedAction.MERGE_WITH and u.merge_target else ""
            action_display = u.recommended_action.value + merge_info
            rows += f"""
<tr>
...
def export_csv_reports(result: AnalysisResult, output_dir: Path):
    ...
    # v7.4.0: potentially unused files
    if result.potentially_unused_files:
...
# =============================================================================
# UNUSED DETECTION ENGINE (v7.4.0)
# =============================================================================
class UnusedSignalCollector:
    """Collects signals for a file that contribute to unused confidence."""
    ...
    def __init__(self, files: Dict[str, FileInfo], graph: Dict[str, List[str]], 
        ...
        # Precompute active files (those with dependents or entry points)
    ...
    def collect(self, path: str) -> Dict[str, bool]:
        """Collect boolean signals for a file."""
        fi = self.files[path]
        signals = {}
        
        # Basic dependency signals
        ...
        # Git signals (may be expensive, skip in fast mode)
        ...
        # Structural similarity to active files (max similarity)
        if not self.fast_mode and self.active_files:
            max_sim = 0.0
            # Limit to a sample to avoid O(n^2)
            sample_active = list(self.active_files)[:100]  # arbitrary limit
            for active in sample_active:
                if active == path:
                    continue
                fi_active = self.files[active]
                sim = jaccard_similarity(set(fi.exports + fi.imports), 
                                          set(fi_active.exports + fi_active.imports))
                if sim > max_sim:
                    max_sim = sim
...
class UnusedConfidenceModel:
    """Computes confidence score from signals using weighted sum."""
    ...
    def compute(self, signals: Dict[str, bool]) -> float:
        ...
        for signal, value in signals.items():
            if value:
                ...
                # Negative weights are subtracted when signal is false? Actually negative weights mean the signal is *good* (reduces confidence).
                # So if the signal is false (bad), we should add the absolute value? Let's keep it simple: positive weights add when true,
                # negative weights subtract when true. So we only apply weight if signal is true.
                # For signals with negative weights, they are "good" signals, so we want to *reduce* confidence when they are present.
                # So we add the negative weight when signal is true (reduces score).
                pass  # already applied only if true
        # Also, for signals with negative weights, we want to subtract when true (done)
        # Ensure 0-100
        return max(0, min(100, score))

class UnusedClassifier:
    """Classifies a file based on signals and confidence."""
    ...
    def classify(path: str, fi: FileInfo, signals: Dict[str, bool], confidence: float) -> Tuple[UnusedClassification, RecommendedAction, Optional[str]]:
        """Determine classification and recommended action."""
        # Default
        classification = UnusedClassification.DEAD_CODE
        action = RecommendedAction.ARCHIVE
        merge_target = None
        
        # If it has exports and reasonable complexity, might be valuable unwired
        ...
        # If it has no exports but is old and no commits, dead code
        ...
        # If it has structural similarity to active files
            ...
            # Find a merge target (first active file with high similarity)
            # (simplified: we'll set merge_target later in the main loop)
        # If it's an orphaned feature (exports, no dependents, but recent)
...
class UnusedDeletionSimulator:
    """Simulates deletion of a file to check safety."""
    ...
    def is_safe_to_delete(self, path: str) -> Tuple[bool, List[str]]:
        """Check if deleting path would cause issues."""
        blockers = []
        fi = self.files[path]
        
        # 1. Any dynamic imports referencing this file?
        if fi.is_dynamic_imported:
            blockers.append("File is dynamically imported")
        
        # 2. Any barrel re-exports?
        if fi.is_barrel_exported:
            blockers.append("File is re-exported from a barrel")
        
        # 3. Any tests that import it? (we consider test imports as usage)
        ...
        # 4. Would deletion break any indirect dependencies?
        # If any file depends on this one, we need to check if those dependents are still valid after removal.
        # For now, we consider any direct dependent as blocker.
        if fi.dependents:
            blockers.append(f"Has {len(fi.dependents)} direct dependents")
        
        # 5. Would it create a new circular dependency? (complex, skip)
        
        # 6. Any unresolved imports after removal? (would require full re-resolve, skip)
...
class GStudioAnalyzer:
    ...
    def __init__(self,
        ...
        # Cache
        self.cache = CacheManager(project_path / Config.CACHE_FILE, use_cache and not dry_run, version="7.4.0")
    ...
    def analyze(self) -> AnalysisResult:
        log_header("G-STUDIO ENTERPRISE CODE INTELLIGENCE v7.4.0")
        ...
        log_success(f"Found {len(unused)} potentially unused files (simple detection)")
...
<{name} />"""
    ...
    def _analyze_potentially_unused(self):
        """Run advanced unused detection and populate result.potentially_unused_files."""
        ...
        # We'll iterate over all files, but could be optimized to only those with low dependents.
        # For now, iterate all.
    ...
    def _evaluate_unused(self, path: str, collector: UnusedSignalCollector,
        ...
        """Evaluate a single file and return UnusedFileDetail if confidence >= threshold."""
        fi = self.files.get(path)
        if not fi:
            return None
        # Skip entry points, test files, configs early?
        # We'll let signals handle that.
        ...
        # If action is MERGE_WITH, find a merge target (first active file with high similarity)
        if action == RecommendedAction.MERGE_WITH:
            # Find most similar active file
            best_sim = 0.0
            best_target = None
        ...
        # Simulate deletion safety
        safe, blockers = simulator.is_safe_to_delete(path) if not self.dry_run_unused else (False, ["dry run"])

        # Build reasons list from signals that contributed positively (excluding negatives)
        reasons = []
        weights = Config.UNUSED_SIGNAL_WEIGHTS
        for sig, val in signals.items():
            if val and weights.get(sig, 0) > 0:
                reasons.append(sig.replace('_', ' ').title())
            elif not val and weights.get(sig, 0) < 0:
                # Negative weight means good signal is missing, e.g., no exports is bad
                # We can add reason like "Missing good signal: ..." but maybe not needed.
                pass

        # Estimated savings (lines + complexity)
        savings_lines = fi.lines
        complexity_reduction = fi.cyclomatic_complexity
        ...
        detail = UnusedFileDetail(
            ...
            signals=signals,
            reasons=reasons[:5],  # keep top 5
            estimated_savings_lines=savings_lines,
            estimated_complexity_reduction=complexity_reduction,
            deletion_safe=safe,
            blockers=blockers
        )
        return detail
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="G-Studio Enterprise Code Intelligence v7.4.0",
...
Examples:
  ...
  # Full enterprise analysis with unused detection
  %(prog)s . --enable-python --enable-duplicates --enable-recommendations --git-history --archive --csv --unused-scan

  # Dry run unused detection
  %(prog)s . --unused-scan --dry-run-unused
"""
    )
    ...
    # ---------- v7.4.0 unused detection ----------
    parser.add_argument('--unused-scan', action='store_true', help='Run advanced unused file intelligence')
    parser.add_argument('--fast-unused-scan', action='store_true', help='Skip expensive signals (git, structural)')
    parser.add_argument('--dry-run-unused', action='store_true', help='Do not simulate deletion, just report')
