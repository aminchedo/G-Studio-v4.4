...
export class RuntimeGuardrails {
  ...
  /** Whether non-critical guards are enabled (production guards always on) */
  static get enabled(): boolean {
    return this.PRODUCTION_ENABLED;
  }
  ...
  static setEnabled(enabled: boolean): void {
    ...
    console.warn(
      "[GUARD] Attempted to disable guards - production guards remain active",
    );
  }
   ...
   */
  static guardOrchestratorEntry(
    message: string,
    apiKey: string,
    requestId?: string,
  ): void {
    // NON-REMOVABLE: Always enforce in production
    const guardPrefix = requestId
    ...
    if (
      ...
      console.error(`${guardPrefix}: TRIGGERED - Invalid message input`, {
        messageType: typeof message,
        messageLength: message?.length,
      });
      throw new Error(
        "Invalid input: Message cannot be empty. Please provide a valid message.",
      );
    }

    // PRECONDITION: API key must be string (can be empty, but type must be correct)
    if (apiKey !== undefined && apiKey !== null && typeof apiKey !== "string") {
      console.error(`${guardPrefix}: TRIGGERED - Invalid API key type`, {
  ...
  static guardDecisionEngine(executionPlan: any, requestId?: string): void {
    ...
    if (!executionPlan || typeof executionPlan !== "object") {
      console.error(
        `${guardPrefix}: TRIGGERED - Decision engine not consulted`,
        { executionPlanType: typeof executionPlan },
      );
      throw new Error(
        "System error: Decision engine must be consulted before processing. Please try again.",
      );
    }

    if (
      typeof executionPlan.mode === "undefined" ||
      executionPlan.mode === null
    ) {
      console.error(`${guardPrefix}: TRIGGERED - Missing execution mode`, {
        executionPlan,
      });
      throw new Error(
        "System error: Execution mode not determined. Please try again.",
      );
    }
    ...
    if (!validModes.includes(executionPlan.mode)) {
      console.error(`${guardPrefix}: TRIGGERED - Invalid execution mode`, {
        mode: executionPlan.mode,
        validModes,
      });
      throw new Error(
        `System error: Invalid execution mode detected. Please refresh and try again.`,
      );
    }

    // POSTCONDITION: Decision engine validated
    console.log(`${guardPrefix}: PASSED (decision engine)`, {
      mode: executionPlan.mode,
    });
  }
   ...
   */
  static guardContextPersistence(
    sessionId: string | null,
    response: string,
  ): void {
    if (!this.enabled) return;
  ...
  static guardNoBypass(callerStack: string): void {
    ...
    if (
      !callerStack.includes("agentOrchestrator") &&
      !callerStack.includes("processUserMessage")
    ) {
  ...
  static guardGeminiServiceInput(
    ...
    if (!Array.isArray(contents)) {
      ...
      throw new Error(
        "Internal error: Invalid request format. Please try again.",
      );
    }

    // PRECONDITION: Contents must have at least one message
    if (contents.length === 0) {
      console.error(`${guardPrefix}: TRIGGERED - Empty contents array`, {
        operation,
      });
      throw new Error(
        "Internal error: Request cannot be empty. Please try again.",
      );
    }
    ...
    for (let i = 0; i < contents.length; i++) {
      ...
      if (!item || typeof item !== "object") {
        console.error(`${guardPrefix}: TRIGGERED - Invalid content item`, {
          index: i,
          itemType: typeof item,
        });
        throw new Error(
          "Internal error: Invalid message format. Please try again.",
        );
      }
      if (!item.role || !item.parts || !Array.isArray(item.parts)) {
        console.error(
          `${guardPrefix}: TRIGGERED - Content item missing required fields`,
          {
            index: i,
            hasRole: !!item.role,
            hasParts: !!item.parts,
        ...
        throw new Error(
          "Internal error: Message structure invalid. Please try again.",
        );
      }
    }

    console.log(`${guardPrefix}: PASSED (gemini service input)`, {
      operation,
  ...
  static guardGeminiServiceResponse(
    ...
    if (!hasText && !hasToolCalls && !hasUsage) {
      ...
      return {
        isValid: false,
        error:
          "Empty response content. The AI did not return any content. Please try again.",
      };
    }
  ...
  static guardNetworkReliabilityInput(
    ...
    if (!error) {
      console.error(
        `${guardPrefix}: TRIGGERED - No error provided to network reliability`,
        { context },
      );
      throw new Error(
        "Internal error: Network analysis requires error information.",
      );
    }

    // PRECONDITION: Context must be valid
    if (!context || typeof context !== "object") {
      console.error(`${guardPrefix}: TRIGGERED - Invalid context`, {
    ...
    if (typeof context.apiKeyValidated !== "boolean") {
      console.error(`${guardPrefix}: TRIGGERED - Invalid apiKeyValidated`, {
        context,
      });
    ...
    if (
      typeof context.attemptNumber !== "number" ||
      context.attemptNumber < 1
    ) {
      console.error(`${guardPrefix}: TRIGGERED - Invalid attempt number`, {
        context,
      });
