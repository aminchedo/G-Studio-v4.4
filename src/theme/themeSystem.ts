...
const darkTheme: ThemeTokens = {
  ...
  typography: {
    fontFamilySans: "'Inter', 'Vazir', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif",
    fontFamilyMono: "'JetBrains Mono', 'Fira Code', 'Consolas', monospace",
    fontFamilyDisplay: "'Inter', 'Vazir', -apple-system, BlinkMacSystemFont, sans-serif",
    fontSizeXs: '0.75rem',
    fontSizeSm: '0.875rem',
    fontSizeMd: '1rem',
    fontSizeLg: '1.125rem',
    ...
    (set, get) => ({
      ...
      setMode: (mode: ThemeMode) => {
        ...
        if (mode === 'high-contrast') {
          ...
          tokens = JSON.parse(
            JSON.stringify(systemPreference === 'dark' ? darkTheme : lightTheme)
          );
          effectiveTheme = systemPreference;
        } else {
          tokens = JSON.parse(JSON.stringify(mode === 'dark' ? darkTheme : lightTheme));
          effectiveTheme = mode;
        }
...
function applyThemeToCSSVariables(tokens: ThemeTokens) {
  ...
  Object.entries(tokens.typography).forEach(([key, value]) => {
    if (key.startsWith('fontSize') || key.startsWith('fontWeight') || key.startsWith('lineHeight')) {
...
function updateDocumentClasses(mode: ThemeMode, effectiveTheme: EffectiveTheme) {
  if (typeof document === 'undefined') return;
...
export function useIsHighContrast() {
  return useThemeStore((state) => state.getEffectiveTheme() === 'high-contrast');
}
