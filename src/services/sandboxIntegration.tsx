...
export class SandboxIntegration {
  ...
  static async executeToolWithSandbox(
    ...
    // If sandbox is disabled, use internal execution (bypasses sandbox check)
    // This avoids circular dependency with McpService.executeTool
    ...
    try {
      ...
      const result = await SandboxManager.execute({
        ...
        executor: async (executorArgs) => {
          // Call McpService.executeToolInternal to avoid circular dependency
          // (executeToolInternal bypasses SandboxIntegration)
          return McpService.executeToolInternal(
            tool,
            executorArgs,
            files,
            callbacks,
  ...
  private static getSandboxConfigForTool(
    ...
    const capabilities =
      toolConfig.capabilities || this.inferCapabilities(tool);
  ...
  static getToolConfig(
    tool: string,
  ): (typeof DEFAULT_CONFIG.toolConfigs)[string] | undefined {
