/**
 * Voice Chat Modal - Persian & English Voice Chat with Gemini
 * 
 * Features:
...
import { X, Mic, MicOff, Sparkles, Settings, Loader2, Globe } from 'lucide-react';
...
export const VoiceChatModal: React.FC<VoiceChatModalProps> = ({
  ...
  const [apiKey, setApiKey] = useState(() => propApiKey || localStorage.getItem('gemini_api_key') || '');
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      const languageInstruction = language === 'fa-IR' 
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center animate-in fade-in duration-200">
      <div className="bg-white w-[500px] h-[650px] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="h-16 border-b border-slate-200 flex items-center justify-between px-6 bg-gradient-to-r from-indigo-50 to-purple-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-slate-800 text-base">گفتگوی صوتی</h2>
              <p className="text-[10px] text-slate-500 font-medium">Voice Chat with Gemini</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              title={language === 'fa-IR' ? 'Switch to English' : 'تغییر به فارسی'}
            >
              <Globe size={16} />
              <span className="text-xs font-semibold">{language === 'fa-IR' ? 'FA' : 'EN'}</span>
            </button>
            <button
              onClick={onClose}
        ...
        {!apiKey && (
          <div className="p-6 flex flex-col flex-1 items-center justify-center text-center">
            ...
            <h3 className="text-lg font-bold text-slate-800 mb-2">تنظیم کلید API</h3>
              ...
              {(isListening || isLoading) && (
                <div className="flex justify-end">
                  <div className="bg-slate-200 text-slate-600 px-4 py-2 rounded-2xl rounded-br-sm text-xs flex items-center gap-2">
                    ...
                    {transcript || (isLoading ? 'در حال فکر کردن...' : 'در حال گوش دادن...')}
                  </div>
                </div>
              )}
            ...
            <div className="p-6 bg-white border-t border-slate-200 flex flex-col items-center relative">
              ...
              {(isListening || isSpeaking) && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-10 overflow-hidden">
                  <div className={`w-96 h-96 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full blur-3xl transition-all duration-500 ${
              ...
              <p className="mt-4 text-xs font-medium text-slate-500">
                {isListening 
                  ? (language === 'fa-IR' ? 'برای توقف ضبط کلیک کنید' : 'Tap to stop recording')
                  : (language === 'fa-IR' ? 'برای شروع ضبط کلیک کنید' : 'Tap microphone to speak')
                }
              </p>
            </div>
          </>
