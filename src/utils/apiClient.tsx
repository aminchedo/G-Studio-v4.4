...
const DEFAULT_CONFIG: Required<
  Omit<APIClientConfig, "onRequest" | "onResponse" | "onError">
> = {
  baseUrl: "",
  timeout: 30000,
  maxRetries: 3,
...
export function createAPIClient(config: APIClientConfig): APIClient {
  ...
  const makeRequest = async <T,>(
    ...
    try {
      ...
      // NOTE: This is a low-level API client utility that wraps fetch with retry logic.
      // For architectural compliance, prefer using mcpService.request() when possible.
      // This utility is kept for cases where advanced retry/backoff logic is needed.
  ...
  const request = async <T = unknown,>(
    ...
    const maxRetries = options.skipRetry
      ? 1
      : (options.maxRetries ?? finalConfig.maxRetries);
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        stats.total++;
        const { data, status, responseTime } = await makeRequest<T>(
          url,
          options,
          timeout,
        );
        ...
        if (
          attempt < maxRetries &&
          (status === 0 || isRetryableError(status))
        ) {
          stats.retries++;
