...
export const FeedbackBadge: React.FC<FeedbackBadgeProps> = ({
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${styles.bg} ${styles.text} text-xs font-medium animate-in fade-in slide-in-from-bottom-2 duration-200`}
    >
      {showIcon && <IconComponent className={`w-4 h-4 ${isAnimating ? 'animate-spin' : ''}`} />}
  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        ...
        onClick={handleClick}
        className={`flex items-center justify-center gap-2 rounded-lg font-medium transition-all duration-200 transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${getVariantClass()} ${getSizeClass()} ${className || ''}`}
      >
        {isLoadingState ? (
