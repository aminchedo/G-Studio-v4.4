...
class AgentVerifier {
  ...
  private static async verifyFiles(): Promise<void> {
    const criticalFiles = [
      'services/localAIModelService.ts',
      'services/contextDatabaseBridge.ts',
  ...
  private static async verifyExports(): Promise<void> {
    const exports = [
      { module: 'services/localAIModelService', export: 'LocalAIModelService' },
      { module: 'services/contextDatabaseBridge', export: 'ContextDatabaseBridge' },
      ...
      { module: 'services/contextManager', export: 'ContextManager' },
      { module: 'services/agentOrchestrator', export: 'AgentOrchestrator' },
    ];

    for (const exp of exports) {
      try {
        // Dynamic import to verify
        const module = await import(`../${exp.module}`);
  ...
  private static async verifyRuntimePaths(): Promise<void> {
    ...
    try {
      await (await import('./localAIModelService')).LocalAIModelService.initialize();
      this.verificationResults.push({
        passed: true,
    ...
    try {
      await (await import('./contextDatabaseBridge')).ContextDatabaseBridge.init();
      this.verificationResults.push({
        passed: true,
...
export { AgentVerifier, VerificationResult };
