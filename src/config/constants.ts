...
import { FunctionDeclaration, Type } from "@/services/geminiService/types";

export const SUPPORTED_MODELS: ModelOption[] = [
  {
    id: ModelId.Gemini3FlashPreview,
    label: "Gemini 3.0 Flash",
    value: ModelId.Gemini3FlashPreview,
  ...
  },
  {
    ...
    value: ModelId.Gemini3ProPreview,
    name: "Gemini 3.0 Pro",
    description:
  ...
  },
  {
    ...
    label: "Gemini Flash",
    value: ModelId.GeminiFlashLatest,
    name: "Gemini Flash",
  ...
  },
  {
    ...
    value: ModelId.GeminiFlashLiteLatest,
...
**Script integration (5 tools – project/analysis)**:
- project_analyzer: Analyze project structure (JS/TS/TSX), writes analysis.json
- src_directory_analyzer: Analyze src/ vs root, usage, recommendations
- file_optimizer: Unused/redundant file analysis (dry-run report)
- runtime_verification: Verify Local AI, Hybrid Engine, SQLite
- project_file_comparison: Compare src/ and root files (dry-run)
...
✅ Exposed to the model via FILE_TOOLS (Gemini functionDeclarations)
✅ Executed via MCP registry and executor
...
export const FILE_TOOLS: FunctionDeclaration[] = [
    ...
    parameters: {
      ...
      properties: {
        aliases: {
          type: Type.OBJECT,
          description:
            "Alias mappings, e.g., {'@components': '@/components', '@utils': '@/utils'}. Required.",
        },
        updateImports: {
          type: Type.BOOLEAN,
          description:
  ...
  // Script integration tools (MCP-wrapped scripts from /scripts)
  {
    ...
    parameters: {
      ...
      properties: {
        path: {
          type: Type.STRING,
          description:
            "Optional. Project path to analyze (relative to project root). Default: current directory.",
        },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "Analyze src/ directory: equivalent files in root, usage, recommendations. Generates report (reportPath).",
    parameters: {
      type: Type.OBJECT,
      properties: {
        verbose: {
  ...
  },
  {
    name: "file_optimizer",
    description:
      "Analyze project files for unused/redundant/outdated files and consolidation. Safe mode only (dry-run, report).",
    parameters: {
      type: Type.OBJECT,
      properties: {
        reportOnly: {
          type: Type.BOOLEAN,
          description:
            "Only generate report, no cleanup suggestions. Default true.",
        },
        reportPath: {
          type: Type.STRING,
          description:
  ...
  },
  {
    ...
    description:
      "Run runtime verification: Local AI Model Service, Hybrid Decision Engine, SQLite persistence. Returns pass/fail summary.",
    parameters: {
      type: Type.OBJECT,
      properties: {},
    },
  },
  {
    ...
    description:
      "Compare files in src/ and root: duplicates, completeness, actionable steps. Dry-run only.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        verbose: {
