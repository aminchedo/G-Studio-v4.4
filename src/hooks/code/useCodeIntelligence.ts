...
export interface UseCodeIntelligenceReturn {
  ...
  // Analysis Actions
  analyzeFile: (filePath: string, content: string, language?: string) => Promise<AnalysisResult>;
  analyzeProject: (files: Record<string, string>) => Promise<void>;
  trackChanges: (beforeCode: string, afterCode: string, filePath?: string) => Promise<ChangeAnalysis>;
  detectBreakingChanges: (oldVersion: string, newVersion: string) => Promise<BreakingChange[]>;
  
  // Query Actions
  getFileAnalysis: (filePath: string) => AnalysisResult | undefined;
  getFileDependencies: (filePath: string) => string[];
  getFileDependents: (filePath: string) => string[];
  findReferences: (symbol: string, filePath?: string) => Array<{ file: string; line: number }>;
...
const calculateMaintainabilityIndex = (
  ...
  const mi = Math.max(0, (171 - 5.2 * Math.log(volume) - 0.23 * complexity + 50 * Math.sin(Math.sqrt(2.4 * commentRatio))) * 100 / 171);
...
const detectCodeSmells = (code: string, language: string): CodeSmell[] => {
  ...
  lines.forEach((line, index) => {
    ...
    // Magic numbers
    if (/[^a-zA-Z_][0-9]{2,}[^a-zA-Z_0-9]/.test(line) && !/const|let|var|=/.test(line.split('//')[0])) {
      smells.push({
        type: 'magic-number',
        severity: 'low',
...
const parseImports = (code: string, language: string): ImportInfo[] => {
  ...
  lines.forEach((line, index) => {
    // ES6 imports
    const esImportMatch = line.match(/import\s+(?:(\{[^}]+\})|(\*\s+as\s+\w+)|(\w+))?\s*(?:,\s*(?:(\{[^}]+\})|(\w+)))?\s*from\s*['"]([^'"]+)['"]/);
    if (esImportMatch) {
      ...
      if (esImportMatch[1]) {
        // Named imports
        specifiers.push(...esImportMatch[1].replace(/[{}]/g, '').split(',').map(s => s.trim()));
      }
      if (esImportMatch[2]) {
        // Namespace import
      ...
      if (esImportMatch[4]) {
        specifiers.push(...esImportMatch[4].replace(/[{}]/g, '').split(',').map(s => s.trim()));
      }
      if (esImportMatch[5]) {
    ...
    if (requireMatch) {
      const specifiers: string[] = [];
      if (requireMatch[1]) {
        specifiers.push(...requireMatch[1].replace(/[{}]/g, '').split(',').map(s => s.trim()));
      }
      if (requireMatch[2]) {
 ...
 */
export function useCodeIntelligence(workspacePath?: string): UseCodeIntelligenceReturn {
  // State
  const [analysisResults, setAnalysisResults] = useState<Map<string, AnalysisResult>>(new Map());
  const [dependencies, setDependencies] = useState<DependencyGraph>({ nodes: [], edges: [] });
  const [projectMetrics, setProjectMetrics] = useState<CodeMetrics | null>(null);
  ...
  const analyzeFile = useCallback(async (
    ...
    try {
      ...
      const maintainabilityIndex = calculateMaintainabilityIndex(codeLines.length, complexity, commentRatio);
      ...
      if (metrics.cyclomaticComplexity > 10) {
        result.suggestions.push('Consider breaking down complex functions into smaller ones');
      }
      if (metrics.maintainabilityIndex < 50) {
        result.suggestions.push('Code maintainability is low. Consider refactoring');
      }
      if (metrics.codeSmells.length > 5) {
        result.suggestions.push(`Address ${metrics.codeSmells.length} code smells detected`);
      }
  ...
  const analyzeProject = useCallback(async (files: Record<string, string>): Promise<void> => {
    setIsAnalyzing(true);
    setError(null);
    ...
    try {
      ...
      if (allMetrics.length > 0) {
        ...
        const avgComplexity = allMetrics.reduce((sum, m) => sum + m.cyclomaticComplexity, 0) / allMetrics.length;
        const avgMaintainability = allMetrics.reduce((sum, m) => sum + m.maintainabilityIndex, 0) / allMetrics.length;
        const allSmells = allMetrics.flatMap(m => m.codeSmells);

        setProjectMetrics({
          linesOfCode: totalLOC,
          linesOfComments: allMetrics.reduce((sum, m) => sum + m.linesOfComments, 0),
          ...
          technicalDebt: allMetrics.reduce((sum, m) => sum + m.technicalDebt, 0),
  ...
  const trackChanges = useCallback(async (
    ...
    const additions = afterLines.filter(l => !beforeLines.includes(l)).length;
    const deletions = beforeLines.filter(l => !afterLines.includes(l)).length;
    const modifications = Math.min(additions, deletions);

    const breakingChanges: BreakingChange[] = [];

    // Detect breaking changes (simplified)
    const beforeExports = beforeCode.match(/export\s+(?:const|function|class|interface|type)\s+(\w+)/g) || [];
    const afterExports = afterCode.match(/export\s+(?:const|function|class|interface|type)\s+(\w+)/g) || [];

    for (const exp of beforeExports) {
      if (!afterExports.includes(exp)) {
    ...
    const riskLevel = breakingChanges.length > 0 ? 'high' : 
                      additions + deletions > 100 ? 'medium' : 'low';
  ...
  const detectBreakingChanges = useCallback(async (
    oldVersion: string,
    newVersion: string
  ...
  // Get file analysis
  const getFileAnalysis = useCallback((filePath: string): AnalysisResult | undefined => {
    return analysisResults.get(filePath);
  }, [analysisResults]);

  // Get file dependencies
  const getFileDependencies = useCallback((filePath: string): string[] => {
  ...
  // Suggest refactoring
  const suggestRefactoring = useCallback((filePath: string): string[] => {
