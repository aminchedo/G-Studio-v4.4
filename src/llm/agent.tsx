...
import {
  generateContextAbstraction,
  formatContextAbstraction,
} from "./contextAbstraction";
  ...
  try {
    ...
    if (ENABLE_CONTEXT_ABSTRACTION && useContextAbstraction && files) {
      try {
        ...
        console.warn(
          "[Agent] Context abstraction failed, continuing without it:",
          error,
        );
      }
    }
    ...
    if (enableCache) {
      const cachedText = getCached(optimized, undefined, model);
      cached = cachedText ? { text: cachedText, tokens: undefined } : null;
      if (cached) {
    ...
    const gatewayResponse = await callGateway(gatewayConfig, {
      ...
      useContextAbstraction:
        ENABLE_CONTEXT_ABSTRACTION && useContextAbstraction,
    });
