 ...
 */
function freezeResults(results: ModelTestResult[]): ModelTestResult[] {
...
export class ModelValidationStore {
  ...
  private static inMemoryCache: Map<string, ModelTestResult[]> = new Map();
  private static providerStatusCache: Map<
    string,
    {
      readonly status: "ok" | "exhausted" | "rate_limited";
      readonly testedAt: number;
    }
  > = new Map();
   ...
   */
  private static discoveryState: Map<
    string,
    {
  ...
  static getValidatedModels(apiKey: string): string[] {
    ...
    if (results.length > 0) {
      ...
      safeFetch(`${API_BASE}/ingest/e18d953a-ffb4-4ced-9fd1-6cbd5a7207b7`, {
        ...
        body: JSON.stringify({
          location: "modelValidationStore.ts:getValidatedModels",
          message: "getValidatedModels after test",
          data: {
            resultsLength: results.length,
            workingCount: working.length,
            returnedLength: working.length,
    ...
    safeFetch(`${API_BASE}/ingest/e18d953a-ffb4-4ced-9fd1-6cbd5a7207b7`, {
      ...
      body: JSON.stringify({
        location: "modelValidationStore.ts:getValidatedModels",
        message: "getValidatedModels no test - defaults",
        data: { resultsLength: 0 },
        timestamp: Date.now(),
        hypothesisId: "H4",
      }),
    }).catch(() => {});
  ...
  static setSelectionMode(apiKey: string, mode: "auto" | "manual"): void {
    ...
    try {
      const stored = localStorage.getItem(this.SELECTION_MODE_KEY);
      const allModes: Record<string, "auto" | "manual"> = stored
        ? JSON.parse(stored)
        : {};
  ...
  static setActiveModel(apiKey: string, model: ModelInfo): void {
    ...
    try {
      const stored = localStorage.getItem(this.ACTIVE_MODEL_KEY);
      const allActive: Record<string, ModelInfo> = stored
        ? JSON.parse(stored)
        : {};
  ...
  static getRejectionReason(apiKey: string, modelId: string): string | null {
    ...
    const result = results.find(
      (r) => r.modelId === modelId && r.status === "failed",
    );
  ...
  static recordTestResult(
    ...
    if (this.testCompletedLock.has(apiKeyHash)) {
      console.warn(
        `[ModelValidationStore] IMMUTABILITY_VIOLATION: Attempted to modify results after test completion for ${apiKeyHash}`,
      );
      throw new Error(
        "INVARIANT_VIOLATION: Cannot modify test results after API Model Test completion. Run a new test to update results.",
      );
    }
  ...
  static recordTestResults(
    ...
    safeFetch(`${API_BASE}/ingest/e18d953a-ffb4-4ced-9fd1-6cbd5a7207b7`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        location: "modelValidationStore.ts:recordTestResults",
        message: "API Model Test results saved",
        data: {
  ...
  static markTestComplete(apiKey: string): void {
    ...
    if (validatedModels.length > 0) {
      ...
      if (
        !currentActive ||
        !validatedModels.find((m) => m.id === currentActive.id)
      ) {
        const bestDefault = selectBestDefaultModel(validatedModels);
        if (bestDefault) {
          this.setActiveModel(apiKey, bestDefault);
          console.log(
            `[ModelValidationStore] Auto-selected best default model: ${bestDefault.id}`,
          );
        }
      }
    }
    ...
    console.log(
      `[ModelValidationStore] Test completed and LOCKED for ${apiKeyHash} - results are now immutable`,
    );
  }
  ...
  static clearResults(apiKey: string): void {
    ...
    try {
      ...
      if (stored) {
        const allResults: Record<string, ModelTestResult[]> =
          JSON.parse(stored);
      ...
      if (modeStored) {
        const allModes: Record<string, "auto" | "manual"> =
          JSON.parse(modeStored);
      ...
      console.warn(
        "[ModelValidationStore] Failed to clear results from localStorage:",
        e,
      );
    }

    console.log(
      `[ModelValidationStore] Results cleared and UNLOCKED for ${apiKeyHash}`,
    );
  }
    ...
    try {
      ...
      if (stored) {
        const allStatuses: Record<
          string,
          { status: "ok" | "exhausted" | "rate_limited"; testedAt: number }
        > = JSON.parse(stored);
  ...
  static setProviderStatus(
    ...
    if (status === "ok" && this.testCompletedLock.has(apiKeyHash)) {
      console.warn(
        `[ModelValidationStore] IMMUTABILITY_VIOLATION: Attempted to set provider status to 'ok' after test completion`,
      );
      throw new Error(
        "INVARIANT_VIOLATION: Cannot change provider status to ok after test completion. Run a new test.",
      );
    }
  ...
  private static loadResults(apiKeyHash: string): ModelTestResult[] {
    ...
    try {
      ...
      if (stored) {
        ...
        const freshResults = results.filter(
          (r) => now - r.testedAt <= this.CACHE_TTL_MS,
        );
      ...
      console.warn(
        "[ModelValidationStore] Failed to load results from localStorage:",
        e,
      );
    }

    return Object.freeze([]) as ModelTestResult[];
  }

  private static saveResults(
    apiKeyHash: string,
    ...
    this.inMemoryCache.set(apiKeyHash, toCache as ModelTestResult[]);

    // Save to localStorage
    try {
      const stored = localStorage.getItem(this.STORAGE_KEY);
      const allResults: Record<string, ModelTestResult[]> = stored
        ? JSON.parse(stored)
        : {};
      ...
      console.warn(
        "[ModelValidationStore] Failed to save results to localStorage:",
        e,
      );
    }
  }
