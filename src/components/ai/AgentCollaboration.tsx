...
interface AgentCollaborationProps {
  onAgentsChange?: (roles: AgentRole[]) => void;
...
export const AgentCollaboration: React.FC<AgentCollaborationProps> = ({
  ...
  const [agents] = useState<Agent[]>(MultiAgentService.getAllAgents());
  ...
  useEffect(() => {
    MultiAgentService.setActiveAgents(Array.from(selectedAgents));
  ...
  const selectAll = () => {
    setSelectedAgents(new Set(agents.map((a) => a.id as AgentRole)));
  };
  ...
  const getWorkflowOrder = () => {
    ...
    if (selectedAgents.has(AgentRole.ARCHITECT)) {
      order.push(agents.find((a) => a.id === AgentRole.ARCHITECT)!);
    }
    if (selectedAgents.has(AgentRole.CODER)) {
      order.push(agents.find((a) => a.id === AgentRole.CODER)!);
    }
    if (selectedAgents.has(AgentRole.REVIEWER)) {
      order.push(agents.find((a) => a.id === AgentRole.REVIEWER)!);
    }
    if (selectedAgents.has(AgentRole.DEBUGGER)) {
      order.push(agents.find((a) => a.id === AgentRole.DEBUGGER)!);
    }
    if (selectedAgents.has(AgentRole.DOCUMENTER)) {
      order.push(agents.find((a) => a.id === AgentRole.DOCUMENTER)!);
    }
    ...
    <div className="flex flex-col h-full bg-slate-900">
      ...
      <div className="flex-1 overflow-auto p-6 bg-slate-900">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {agents.map((agent) => {
            const isSelected = selectedAgents.has(agent.id as AgentRole);

            return (
              <button
                key={agent.id}
                onClick={() => toggleAgent(agent.id as AgentRole)}
