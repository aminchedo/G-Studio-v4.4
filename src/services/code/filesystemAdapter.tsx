...
import { McpToolResult, McpToolCallbacks } from "@/mcp/runtime/types";
...
import { executeViaExecutor } from "../mcp/executorAdapter";
...
// Phase 2 intentionally allows only read and write operations
const FILESYSTEM_ALLOWED_TOOLS = ["read_file", "write_file"];
...
export class FilesystemAdapter {
  ...
  static async execute(
    ...
    if (mode === "memory") {
      // Step 2: Route through executor adapter (with fallback to old implementation)
      return executeViaExecutor(tool, args, files, callbacks);
    }
  ...
  private static async executeFilesystemTool(
    ...
    try {
      ...
      const sandboxResult = await this.checkSandboxPermissions(
        tool,
        args,
      ...
      switch (tool) {
        case "read_file":
          result = await this.executeFilesystemRead(
            filePath,
            pathValidation.absolute,
          );
          break;
  ...
  private static validatePath(filePath: string): {
    ...
    try {
      ...
      // Check for suspicious patterns first (quick reject)
      if (filePath.includes("..") || filePath.includes("~")) {
        const errorMsg = filePath.includes("..")
          ? "Path contains suspicious patterns (traversal detected)"
  ...
  private static async executeFilesystemRead(
    ...
    const readResult = await FileOperations.readFile(absolutePath);
