#!/usr/bin/env python3
"""
G-Studio Enterprise Code Intelligence Platform v9.2.0
======================================================
...
All previous features integrated:
  • Hybrid AST + CFG + DFG + PDG (parallel construction)
  • AST‑based structural clone detection (Type‑1,‑2,‑3) with fallback Jaccard
  • Full dependency graph with transitive closure, Tarjan’s SCC, Kahn’s topological sort
  • Incremental parsing with Tree‑sitter – cached syntax trees for re‑analysis
  • Program Dependence Graph (PDG) combining control & data dependencies
  • SQLite caching with O(1) lookups and transaction‑safe updates
  • Framework‑aware entry point detection (Next.js, Remix, Vite, CRA, Gatsby, Astro)
  • Multi‑signal unused file detection (confidence scoring, classification, deletion simulation)
  • Wiring issue detector (direct SDK imports, test imports, layer violations, raw fetch)
  • Architectural insights (AI, MCP, voice, context, hooks, cycles)
  • Recommendation engine (merge/archive/wire/refactor) with wiring suggestions
  • Archive manager (zip + metadata, dry‑run)
  • Git intelligence (commit count, authors, recency)
  • Parallel processing with batching
  • Rich HTML/JSON/CSV reports with detailed guidance
