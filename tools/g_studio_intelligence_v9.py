...
class RegexParser:
    ...
    def parse_ts_js(self, content: str, result: ParsedData):
        ...
        # Complexity & structural hash (if requested)
        # (caller will set compute_structural_hash and call these separately if needed)
...
# Note: WiringIssue, FileInfo, LayerType, Severity, Config are expected to exist in your codebase.
# This class will use them. If not present, provide compatible definitions elsewhere.
...
class WiringIssueDetector:
    ...
    - allows construction with or without initial `files`/`project_path`
    - provides `configure()` for late-binding
    - uses safe fallbacks when Config or enums are missing
    """
    ...
    def __init__(
        ...
        # Accept both None and actual maps; allow later configure() call
        self.files: Dict[str, Any] = files or {}
        self.project_path: Path = project_path or Path.cwd()
        self.verbose: bool = bool(verbose)

        # internal mappings
        self.ctx_to_hook_map: Dict[str, str] = {}
        self._ai_sdk_files: Dict[str, Set[str]] = {}

        # Attempt to build ctx->hook map if files were provided
        if self.files:
            try:
                self.ctx_to_hook_map = self._build_context_hook_map()
            except Exception as e:
                if self.verbose:
                    print(f"[WiringIssueDetector] _build_context_hook_map failed: {e}")

    # Allow late configuration (useful if someone calls WiringIssueDetector() with no args)
    def configure(self, files: Dict[str, Any], project_path: Optional[Path] = None, verbose: Optional[bool] = None):
        self.files = files or {}
        ...
        try:
            ...
            if self.verbose:
                print(f"[WiringIssueDetector] configure: _build_context_hook_map failed: {e}")
            self.ctx_to_hook_map = {}

    def _build_context_hook_map(self) -> Dict[str, str]:
        """Scan hook files (LayerType.HOOKS) to map raw context identifiers to hook names.

        Expected FileInfo to expose:
          - layer (comparable to LayerType.HOOKS)
          - path (or use file_path key)
        """
        ctx_map: Dict[str, str] = {}
        if not self.files:
            return ctx_map

        for file_key, info in self.files.items():
            try:
                # Try to detect layer flag in a defensive way
                layer_val = getattr(info, "layer", None)
                if layer_val is None:
                    # maybe info is a dict
                    layer_val = info.get("layer") if isinstance(info, dict) else None

                # Compare to LayerType.HOOKS if available; otherwise use filename heuristics
                is_hooks_layer = False
                try:
                    from your_project_types import LayerType  # replace with actual import if needed
                    is_hooks_layer = (layer_val == LayerType.HOOKS)
                except Exception:
                    # fallback: file path or key contains "hook" or "hooks"
                    is_hooks_layer = ("hook" in str(file_key).lower() or "hook" in str(getattr(info, "path", "") or "").lower())

                if not is_hooks_layer:
                    continue

                # determine actual file path on disk
                rel_path = getattr(info, "path", None) or file_key
                full_path = (self.project_path / rel_path).resolve()
                if not full_path.exists():
                    # try fallback: file_key might already be absolute
                    candidate = Path(file_key)
                    if candidate.exists():
                        full_path = candidate
                    else:
                        if self.verbose:
                            print(f"[WiringIssueDetector] hook file not found: {rel_path}")
                        continue

                content = full_path.read_text(encoding="utf-8", errors="ignore")

                # capture hooks: look for function names exported as default or named hooks
                # also capture useContext usages inside the hook file to map contexts -> hook name
                # Hook name fallback: file stem if startswith 'use'
                hook_candidate = Path(rel_path).stem
                if not hook_candidate.startswith("use"):
                    # try to find exported hook function name inside
                    m = re.search(r"export\s+(?:const|function)\s+(use[A-Za-z0-9_]+)", content)
                    if m:
                        hook_candidate = m.group(1)

                for ctx in re.findall(r"useContext\(\s*([A-Za-z0-9_$.]+)\s*\)", content):
                    # normalize ctx (strip possible non-alphanumeric wrapper)
                    ctx_name = ctx.strip()
                    if hook_candidate and hook_candidate.startswith("use"):
                        ctx_map[ctx_name] = hook_candidate
                    else:
                        ctx_map[ctx_name] = f"use{Path(rel_path).stem.capitalize()}"

            except Exception as exc:
                if self.verbose:
                    print(f"[WiringIssueDetector] _build_context_hook_map error for {file_key}: {exc}")
                # continue scanning other files

        return ctx_map

    @staticmethod
    def _severity_to_score(sev: Any) -> int:
        """Convert a Severity enum/value to integer score using Config.SEVERITY_SCORES if available."""
        try:
            # Prefer using Config mapping
            from your_project_config import Config  # replace if your module name differs
            # Expect Config.SEVERITY_SCORES to be a dict like {'low': (1,2), ...}
            key = getattr(sev, "value", str(sev)).lower()
            low_high = Config.SEVERITY_SCORES.get(key)
            if low_high and isinstance(low_high, (list, tuple)) and len(low_high) == 2:
                low, high = int(low_high[0]), int(low_high[1])
                return (low + high) // 2
        except Exception:
            # fallback: map simple names to numbers
            pass

        # graceful fallback mapping
        name = getattr(sev, "name", None) or str(sev)
        name_lower = str(name).lower()
        fallback_map = {"critical": 100, "high": 75, "medium": 50, "low": 25}
        return fallback_map.get(name_lower, 10)

    def detect_issues(self, file_path: str, content: str) -> List[Any]:
        """Detect wiring issues inside a single file content.

        Returns a list of WiringIssue instances (or compatible objects).
        """
        issues: List[Any] = []

        # capture import module paths per line (robust for single-line import statements)
        # Common forms:
        # import X from 'module'
        # import {A, B} from "module"
        # const X = require('module')
        import_from_pattern = re.compile(r"""^\s*(?:import\b.*\bfrom\s+['"](?P<mod>[^'"]+)['"]|.*require\(\s*['"](?P<req>[^'"]+)['"]\s*\)).*""")
        lines = content.splitlines()

        for line_num, line in enumerate(lines, start=1):
            # attempt to extract import module for this line
            m = import_from_pattern.match(line)
            import_path = None
            if m:
                import_path = m.group("mod") or m.group("req")

            # examine import-based checks only when an import/module path is present
            if import_path:
                try:
                    issues.extend(self._check_direct_sdk_import(file_path, line_num, import_path, line))
                    issues.extend(self._check_test_file_import(file_path, line_num, import_path))
                    issues.extend(self._check_layer_violation(file_path, line_num, import_path, line))
                    issues.extend(self._check_multiple_ai_providers(file_path, line_num, import_path, line))
                except Exception as e:
                    if self.verbose:
                        print(f"[WiringIssueDetector] import checks failed for {file_path}:{line_num} ({import_path}): {e}")

            # checks that don't require explicit import module
            try:
                ...
                if self.verbose:
                    print(f"[WiringIssueDetector] content checks failed for {file_path}:{line_num}: {e}")

        return issues

    def _get_config_list(self, attr_name: str, default: Optional[List[str]] = None) -> List[str]:
        """Helper to safely fetch list-like config items from Config with fallback."""
        default = default or []
        try:
            from your_project_config import Config  # adjust import path to your config module
            val = getattr(Config, attr_name, None)
            if isinstance(val, (list, tuple, set)):
                return list(val)
            return default
        except Exception:
            # fallback: try global Config if available
            try:
                val = globals().get("Config", None)
                if val:
                    v = getattr(val, attr_name, None)
    ...
    def _check_direct_sdk_import(self, file_path: str, line_num: int, import_path: str, line: str) -> List[Any]:
        issues: List[Any] = []
        ai_sdks = self._get_config_list("AI_SDK_IMPORTS", default=["@google/generative-ai", "openai"])
        for sdk in ai_sdks:
            if sdk in import_path:
                # skip some legitimate service or hook files based on heuristics
                lower_fp = file_path.lower()
                if "services" in lower_fp and "gemini" in lower_fp:
                    # service implementation importing sdk is allowed
                    if self.verbose:
                        print(f"[WiringIssueDetector] skipping sdk import check for service/gemini file: {file_path}")
                    return issues
                if "hooks" in lower_fp and ("usegemini" in lower_fp or "use_" in lower_fp):
                    # a hook that intentionally wraps sdk is allowed
                    return issues

                severity = getattr(__import__("builtins"), "object", None)  # dummy to keep type checker happy
                # Determine severity object if available in project
                try:
                    from your_project_types import Severity  # replace with actual import
                ...
                # propose better alternative based on sdk string
                if "@google/" in sdk or "generative" in sdk:
                ...
                # Build WiringIssue object (assumes WiringIssue dataclass exists)
                try:
                    issue = WiringIssue(
                        file_path=file_path,
                    ...
                    # If WiringIssue is not available or signature differs, create a lightweight dict
                    issue = {
                        "file_path": file_path,
                        "line_number": line_num,
    ...
    def _check_test_file_import(self, file_path: str, line_num: int, import_path: str) -> List[Any]:
        issues: List[Any] = []
        test_indicators = [".test.", ".spec.", "__tests__", "__test__"]
        if any(ind in import_path for ind in test_indicators):
            try:
                from your_project_types import Severity
    ...
    def _check_layer_violation(self, file_path: str, line_num: int, import_path: str, line: str) -> List[Any]:
        issues: List[Any] = []
        # Heuristic: service files importing components -> violation
        lower_fp = file_path.lower()
        lower_import = import_path.lower()
        if "services" in lower_fp and ("components" in lower_import or "/components" in lower_import):
            try:
                from your_project_types import Severity
                severity = Severity.HIGH
            except Exception:
    ...
    def _check_multiple_ai_providers(self, file_path: str, line_num: int, import_path: str, line: str) -> List[Any]:
        """Accumulate AI SDK imports per file; actual reporting happens in post_process_ai_providers()."""
        ai_sdks = self._get_config_list("AI_SDK_IMPORTS", default=["@google/generative-ai", "openai"])
        sdk_detected = None
        for sdk in ai_sdks:
            if sdk in import_path:
                ...
                break
        if sdk_detected:
            self._ai_sdk_files.setdefault(file_path, set()).add(sdk_detected)
        return []

    def _check_context_usage(self, file_path: str, line_num: int, line: str) -> List[Any]:
        issues: List[Any] = []
        m = re.search(r"useContext\(\s*([A-Za-z0-9_$.]+)\s*\)", line)
        if not m:
            return issues

        ctx_name = m.group(1)
        # check if whitelisted raw contexts exist
        try:
            from your_project_config import Config
            allow_raw = getattr(Config, "ALLOW_RAW_CONTEXTS", [])
            if ctx_name in allow_raw:
                if self.verbose:
                    print(f"[WiringIssueDetector] Skipping raw useContext check for {ctx_name} (whitelisted) in {file_path}:{line_num}")
                return issues
        except Exception:
            # fallback to global Config if present
            allow_raw = getattr(globals().get("Config", None), "ALLOW_RAW_CONTEXTS", []) if globals().get("Config") else []
            if ctx_name in allow_raw:
                return issues
        ...
        if ctx_name in self.ctx_to_hook_map:
            ...
            try:
                ...
                severity = "LOW"
            severity_score = self._severity_to_score(severity)
            hook_suggestion = f"use{ctx_name}" if not str(ctx_name).startswith("use") else ctx_name
            try:
                issue = WiringIssue(
                    file_path=file_path,
                    line_number=line_num,
                    ...
                    issue_type="Direct useContext Usage",
                    current_import=f"useContext({ctx_name})",
                    better_alternative=f"Create custom hook {hook_suggestion}()",
                    reasoning=[
                        "Encapsulate context logic in a custom hook",
                        "Improves reusability and testability"
                    ],
                    refactor_sample=(
                        f"// Create a custom hook:\nexport const {hook_suggestion} = () => {{\n"
                        f"  const context = useContext({ctx_name});\n  if (!context) throw new Error('...');\n  return context;\n}};"
                    ),
                    auto_fixable=False
                )
            except Exception:
    ...
    def _check_raw_fetch(self, file_path: str, line_num: int, line: str) -> List[Any]:
        issues: List[Any] = []
        # detect direct fetch/axios usage outside services/hooks
        if (("fetch(" in line or "axios." in line) and
            ("services" not in file_path.lower() and "hooks" not in file_path.lower())):
            try:
                from your_project_types import Severity
                severity = Severity.MEDIUM
            except Exception:
    ...
    def post_process_ai_providers(self) -> List[Any]:
        """After scanning all files, produce issues for files that import multiple AI SDKs."""
        issues: List[Any] = []
            ...
            if len(sdks) > 1:
                ...
                try:
                    issue = WiringIssue(
                        ...
                        current_import=", ".join(sorted(sdks)),
                        better_alternative="Standardize on one AI provider",
                        reasoning=[
                            f"File imports multiple AI SDKs: {', '.join(sorted(sdks))}",
...
<head>
    ...
    <title>G-Studio Code Analysis Report v7.2</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🎯 G-Studio Enterprise Code Analysis v7.2</h1>
...
"""
        ...
        <div class="footer">
            Generated by G-Studio Enterprise Code Intelligence v7.2<br>
...
class GStudioAnalyzer:
    ...
    def analyze(self) -> AnalysisResult:
        ...
        self.result.analysis_duration_seconds = time.time() - self._start_time if hasattr(self, '_start_time') else 0
        ...
        try:
            ...
            if cached:
                ...
                for field in ['structural_hash', 'cyclomatic_complexity', 'any_count',
                              ...
                              'recommendation_confidence', 'issues']:
