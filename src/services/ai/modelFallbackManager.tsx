...
import { FailureType } from "../security/aiBehaviorValidation";
...
export class ModelFallbackManager {
  ...
  static getFallbackSequence(
    ...
    const baseSequences: Record<string, ModelId[]> = {
  ...
  private static applyPriorityOrder(availableModels: string[]): string[] {
    ...
    for (const model of availableModels) {
      if (
        !this.MODEL_PRIORITY.includes(model) &&
        !prioritized.includes(model)
      ) {
        others.push(model);
      }
    }
  ...
  static initializeState(
    ...
    const initialModel =
      sequence.length > 0 && sequence.includes(primaryModel)
        ? primaryModel
        : sequence.length > 0
          ? sequence[0]
          : primaryModel;
    ...
    return {
      ...
      >(), // Initialize unavailable models Map
  ...
  static getNextModel(
    ...
    const sequence = this.getFallbackSequence(
      state.primaryModel,
    ...
    if (
      this.shouldRetrySameModel(
        failureType,
        state.attemptsOnCurrentModel,
        state.maxAttemptsPerModel,
      )
    ) {
    ...
    for (let i = currentIndex + 1; i < sequence.length; i++) {
      ...
      if (
        !state.triedModels.has(nextModel) &&
  ...
  static updateStateAfterAttempt(
    ...
    if (
      ...
      (failureType === FailureType.INFRA_FAILURE &&
        newAttempts >= state.maxAttemptsPerModel)
    ) {
      // Move to next model
  ...
  static allModelsExhausted(state: FallbackState): boolean {
    ...
    const eligibleModels = sequence.filter(
      (model) => !this.isModelUnavailable(state, model),
    );
