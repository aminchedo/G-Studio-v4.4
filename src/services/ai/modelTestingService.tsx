...
export class ModelTestingService {
  ...
  private static async probeModel(
    ...
    try {
      ...
      const result = await client.request(`models/${modelId}:generateContent`, {
        ...
        timeout: timeout,
        maxRetries: 2, // Allow one retry on 429/5xx so backoff can reduce rate-limit hits
      });
      ...
      if (result.status === 429 || result.error?.includes("429")) {
        console.warn(
          `[ModelTestingService] 429 detected on ${modelId} - short-circuiting immediately`,
        );
        return {
          success: false,
  ...
  private static async fetchModelDetails(
    ...
    try {
      ...
      const result = await client.request(`models/${modelId}?key=${apiKey}`, {
        timeout: 10000,
        maxRetries: 1,
      });
      ...
      return {
        maxInputTokens:
          typeof data.inputTokenLimit === "number"
  ...
  private static async discoverModelsFromAPI(
    ...
    try {
      ...
      console.log(
        `[ModelTestingService] ✅ Discovered ${discoveredModels.length} models from API using UltimateGeminiTesterService`,
      );
      return discoveredModels;
    } catch (error: any) {
      console.warn(
        `[ModelTestingService] Model discovery failed: ${error.message}`,
      );
      return [];
    }
  }
  ...
  static async testAllModels(
    ...
    console.log(
      `[ModelTestingService] USER_ACTION: Starting fresh API Model Test - previous results cleared, degraded mode exited`,
    );
    ...
    console.log(
      `[ModelTestingService] Initializing UltimateGeminiTester with bypass/DNS...`,
    );
    ...
    console.log(
      `[ModelTestingService] ✅ UltimateGeminiTester initialized with bypass: ${tester.getResults().bypassMethod}`,
    );

    // Test only the candidate (priority) list — skip full API discovery for speed.
    // Candidate list is already short (~8 models): 2.5 Flash, 2.0 Flash, 2.5 Pro, 2.0 Pro, etc.
    const modelsToTest = candidateModels;
    console.log(
      `[ModelTestingService] Testing ${modelsToTest.length} priority models (no full API discovery)`,
    );

    // Log bypass status (browser will use system proxy automatically if configured)
    console.log(
      `[ModelTestingService] Bypass: Browser will use system proxy automatically if configured`,
    );
    ...
    try {
      // Brief delay before first probe to avoid burst after discovery (reduces 429 risk)
      ...
      for (let i = 0; i < modelsToTest.length; i++) {
        ...
        if (result.isRateLimited) {
          ...
          console.log(
            `[ModelTestingService] 429 detected on "${modelId}" (${i + 1}/${candidateModels.length}) - model is VALID (rate limited), adding to usable models`,
          );
          ...
          const modelDetails = await this.fetchModelDetails(
            apiKey,
            modelId,
            tester,
          );
          ...
          // Back off after 429 to avoid cascading rate limits (Gemini is strict)
          await new Promise((resolve) => setTimeout(resolve, 4000)); // 4s after 429
        ...
        if (result.success) {
          ...
          if (failureReason === "rate_limited") {
            ...
            console.log(
              `[ModelTestingService] Filtered 429 error for "${modelId}" - treating as noise`,
            );
            continue;
          }
          ...
          if (
            (failureReason === "timeout" ||
              failureReason === "network_error" ||
              failureReason === "unknown") &&
            i < modelsToTest.length
          ) {
            console.log(
              `[ModelTestingService] Retrying "${modelId}" after ${failureReason} (attempt 2/2 with 15s timeout)...`,
            );
            await new Promise((resolve) => setTimeout(resolve, 2000)); // Wait 2 seconds before retry

            const retryResult = await this.probeModel(
              apiKey,
              modelId,
              15000,
              tester,
            ...
            if (retryResult.success || retryResult.isRateLimited) {
              ...
              if (retryResult.isRateLimited) {
                console.log(
                  `[ModelTestingService] Retry successful: "${modelId}" is VALID (429 on retry)`,
                );
                usableModels.push(modelId);
                const modelDetails = await this.fetchModelDetails(
                  apiKey,
                ...
                console.log(
                  `[ModelTestingService] Retry successful: "${modelId}" works!`,
                );
                usableModels.push(modelId);
                const modelDetails = await this.fetchModelDetails(
                  apiKey,
        ...
        // Delay between every probe to avoid 429 (use modelsToTest.length, not candidateModels)
        if (i < modelsToTest.length - 1) {
          await new Promise((resolve) => setTimeout(resolve, 2000)); // 2s between probes to stay under rate limit
        }
      }

      // Discovery is complete when all models in modelsToTest have been scanned
      console.log(
        `[ModelTestingService] Discovery complete: scanned ${modelsToTest.length}/${modelsToTest.length} models, found ${usableModels.length} usable models`,
      );
      ...
      if (usableModels.length === 0 && rateLimitedCount > 0) {
        ...
        console.warn(
          `[ModelTestingService] Discovery complete (full scan): No usable models found after ${rateLimitedCount} rate limits`,
        );
      } else if (usableModels.length > 0) {
        // Success: Found usable models despite any rate limits
        console.log(
          `[ModelTestingService] Discovery complete (full scan): Found ${usableModels.length} usable models (${rateLimitedCount} rate limits encountered, but discovery succeeded)`,
        );
      } else {
        // No rate limits but also no usable models - likely other issues
        console.warn(
          `[ModelTestingService] Discovery complete (full scan): No usable models found (no rate limits detected)`,
        );
      }
  ...
  static async getCandidateModels(apiKey: string): Promise<string[]> {
    // Use short priority list so "Test connection" finishes in seconds (~8 models)
    const { PRIORITY_TEST_MODELS } = await import("../defaultModels");
    return [...PRIORITY_TEST_MODELS];
  }

  /**
   * Fast API check: single GET to list models (same as the simple HTML checker).
   * Returns in 1–3 seconds instead of probing each model. Use this for "Discover" when
   * you only need to verify the key and get the list of available models.
   */
  static async quickListModels(apiKey: string): Promise<{
    valid: boolean;
    modelIds?: string[];
    error?: string;
  }> {
    ...
    try {
      ...
      if (err?.name === "AbortError") {
        return { valid: false, error: "Request timed out (15s). Try again." };
      }
  ...
  /**
   * Mark the given model IDs as validated (working) without probing.
   * Use after quickListModels() so chat can use any listed model immediately.
   */
  static setValidatedModelsFromList(apiKey: string, modelIds: string[]): void {
    ModelValidationStore.clearResults(apiKey);
    ModelValidationStore.startDiscovery(apiKey, modelIds.length);
    ModelValidationStore.recordTestResults(
      apiKey,
      modelIds.map((modelId) => ({ modelId, status: "working" as const })),
    );
    ModelValidationStore.setProviderStatus(apiKey, "ok");
    ModelValidationStore.markTestComplete(apiKey);
    ModelValidationStore.endDiscovery(apiKey);
  }
}
